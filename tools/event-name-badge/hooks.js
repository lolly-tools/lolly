 ...
 *   • track chips with per-chip contrast,
 *   • an optional QR code (inline vector SVG), and
 *   • the default SUSE logo (auto light/dark) when no event logo is supplied.
 *
 * The QR is produced by the qrcode-svg library (MIT, papnkukn/qrcode-svg),
 * inlined verbatim below so the tool is self-contained inside the sandboxed
 * Function() scope (no imports, no globals beyond ECMAScript built-ins + host).
 */
...
// QR as inline vector SVG (single joined path, crisp at print). Always dark-on-
// white inside its own card, so it scans regardless of the badge colour.
function makeQr(content) {
  try {
    var qr = new QRCode({
      content: content, color: '#0c322c', background: '#ffffff',
      ecl: 'M', padding: 1, join: true, width: 600, height: 600, pretty: false
    });
    return qr.svg({ container: 'svg-viewbox' });
  } catch (e) {
    if (host.log) host.log('warn', 'event-name-badge: QR failed', { error: String(e) });
    return '';
  }
}
