...
Перемикач діє на рівні розгортання (`LOLLY_DISABLE_RENDER_GET=1`): на екземплярі, де його не встановлено, рендеринг за прямим посиланням активний, тож той самий запит там повертає зображення - ця відмінність є результатом роботи прапорця, а не непослідовністю.
