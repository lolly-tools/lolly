 ...
 *
 * Run as: pnpm run build:cargo-licenses  (or directly: node scripts/build-cargo-licenses.ts)
 *
 ...
 *     regenerated cargo-licenses.json AND sbom.cdx.json (pnpm run build:sbom).
 */
...
const doc = {
  '//': [
    ...
    'Generated by scripts/build-cargo-licenses.ts (pnpm run build:cargo-licenses).',
    'Do not edit by hand - regenerate after any Cargo.lock change, then run',
    'pnpm run build:sbom and commit both files.',
    '',
