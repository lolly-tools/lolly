...
/** One entry in an asset's `formats` (or per-locale format) array. */
...
interface CatalogAsset {
  ...
  locales?: Record<string, CatalogAssetFormat[]>;
}

interface CatalogAssetsIndex {
  assets: CatalogAsset[];
}
...
export function sriForFile(absPath: string): { checksum: string; size: number } | null {
...
function run(): void {
  ...
  // JSON trust boundary: catalog/assets/index.json is schema-validated separately
  // (schemas/asset.schema.json via validate-catalog.js).
  const index = parsed as CatalogAssetsIndex;
  let updated = 0;
