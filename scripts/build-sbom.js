...
const workspaceSubcomponents = (rootPkg.workspaces ?? [])
  .map((ws) => {
    ...
    const component = { type: 'library', 'bom-ref': purl, name: pkg.name, version: v, purl };
...
function fileHashes(absPath, algs = ['sha512']) {
...
function licensesFromPackage(pkg) {
...
function licensesFromLegacy(list) {
  const out = [];
...
function addNpmShellDeps(shellDir) {
