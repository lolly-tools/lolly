...
async function deliverCatalog(index: AssetIndex, sb: SignerBundle): Promise<void> {
  ...
  console.log(
    `Delivered: signed ${signed} in place · ${already} already signed · ${failed} failed`
  );
  console.log(
    `Not stampable (no C2PA container, left as-is): ${unstampable} - ${Object.entries(
      skippedFormats
    )
      .map(([k, v]) => `${k}:${v}`)
      .join(', ')}`
...
async function main(): Promise<void> {
  ...
  if (index.defaultFavourites)
    index.defaultFavourites = index.defaultFavourites.filter((id) => !id.startsWith(ID_PREFIX));
  const stripped = before - index.assets.length;
  if (stripped)
    console.log(
      `Stripped ${stripped} stale ${ID_PREFIX}* entr${stripped === 1 ? 'y' : 'ies'} from the index.`
    );

  if (!USE_CATALOG) {
    console.log(
      'Nothing to deliver (pass --catalog to sign every stampable asset). Index cleanup only.'
    );
