 ...
 *
 * Run as: node scripts/build-brand-tokens.js
 *
...
type TokenGroup = { $type?: 'color'; [key: string]: TokenGroup | ColorToken | 'color' | undefined };

const slug = (s: string): string => String(s).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');

// Nest a token at a dotted path inside `root`, creating groups as needed.
function place(root: TokenGroup, path: string, token: ColorToken): boolean {
  const segs = path.split('.');
  let node = root;
  for (let i = 0; i < segs.length - 1; i++) {
    const seg = segs[i] as string;
    node[seg] ??= {};
    node = node[seg] as TokenGroup;
  }
...
function tokenFor(entry: PaletteEntry): ColorToken {
  const token: ColorToken = { $value: String(entry.hex).toLowerCase(), $description: entry.label };
...
function pathFor(entry: PaletteEntry): string {
  ...
  if (ramp) return `ramp.${slug(ramp[1] as string)}.${ramp[2] as string}`;
  if (entry.group === 'spectrum') return `spectrum.${slug(entry.label)}`;
  return `brand.${slug(entry.label)}`;
}

const color: TokenGroup = { $type: 'color' };
for (const entry of PALETTE) {
  place(color, pathFor(entry), tokenFor(entry));
}

const doc = {
  $description: 'SUSE brand colour tokens — generated from shells/web/src/palette.js by scripts/build-brand-tokens.js. Edit the palette and re-run; do not hand-edit.',
  color,
};
...
const count = (node: TokenGroup): number => Object.values(node).reduce(
  (n: number, v) => (v && typeof v === 'object' ? ('$value' in v ? n + 1 : n + count(v as TokenGroup)) : n), 0,
);
