...
export function entryFromManifest(manifest) {
  ...
  // Reference a committed demo preview (tools/<id>/preview.svg|png) when one exists,
  // so the gallery can show a default thumbnail for a tool with no saved session yet
  // — a fuller, prettier gallery on a fresh install. Unlike the icon (inlined), the
  // preview is a path served by the shell's /tools static handler — it can be a
  // sizeable PNG, so inlining it would bloat the index every shell fetches. SVG is
  // preferred (vector tools), PNG for raster. Generated by `npm run thumbs`
  // (scripts/build-thumbs.js); disk-derived like `icon`, so it isn't drift-checked.
  for (const ext of ['svg', 'png']) {
    if (existsSync(join(ROOT, 'tools', manifest.id, `preview.${ext}`))) {
      entry.preview = `/tools/${manifest.id}/preview.${ext}`;
      break;
    }
  }
  return entry;
