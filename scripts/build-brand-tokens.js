...
function place(root, path, token) {
...
function tokenFor(entry) {
  const token = { $value: String(entry.hex).toLowerCase(), $description: entry.label };
...
const count = (node) => Object.values(node).reduce(
  (n, v) => (v && typeof v === 'object' ? ('$value' in v ? n + 1 : n + count(v)) : n), 0,
);
