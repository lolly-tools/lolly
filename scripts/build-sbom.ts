 ...
 *     `security/cargo-licenses.json` map (regenerate with `pnpm run build:cargo-licenses`);
...
// Crate license lookup ("name@version" → SPDX expression), generated by
// scripts/build-cargo-licenses.ts from `cargo metadata`. Optional: without the
// file every crate is licensed `unknown`, exactly as before.
const cargoLicenseDoc = readJsonOptional('security/cargo-licenses.json');
...
// add them as pkg:cargo components. Cargo.lock records no license data, so each
// crate's license is resolved from the committed security/cargo-licenses.json map; a crate
// not in the map is marked `unknown` rather than fabricated. Absent file → skip.
  ...
  const flush = () => {
    if (name && version) {
      ...
      if (!byPurl.has(purl)) {
        ...
        const note = cargoLicenseNotes[`${name}@${version}`];
        if (note) {
          component.properties.push({ name: 'lolly:license-note', value: note });
          if (/not linked into the shipped binary/.test(note)) component.scope = 'excluded';
        }
        byPurl.set(purl, component);
      }
    }
