 ...
 *
 * Usage:
 *   pnpm run translate --corpus spa --lang de
 *   pnpm run translate --corpus tools --all
 *   pnpm run translate --corpus docs --lang de              # every DOCS_PAGES page
 *   pnpm run translate --corpus docs --only privacy --all   # one page, all 26 languages
 *   pnpm run translate --check              # exit non-zero on stale/missing, no API calls
 *   pnpm run translate --corpus spa --all --regenerate   # rewrite catalogs from cache, no API calls
 *
...
// environment variable WINS over the file, so `ANTHROPIC_API_KEY=… pnpm run
...
// they were "generated by pnpm run translate --corpus site" described an
...
async function exportPending(corpusIds: string[], langs: Lang[], cache: Cache, glossary: Glossary, dir: string, only?: string): Promise<void> {
  ...
  console.log('Add a `translations` array to each file - one {id, text} per item, ids as in `items`, text in the target language (leave `items` as the English source) - then: pnpm run translate --import <dir-or-file>');
}
...
async function runCorpus(client: Anthropic | null, corpus: CorpusDef, lang: Lang, cache: Cache, glossary: Glossary): Promise<{ translated: number; cached: number; failed: number }> {
  ...
  if (corpus.id === 'site' && client && !cache.site?.[lang]) {
    throw new Error(
      ...
      + 'this corpus; translating now would overwrite it. Run `pnpm run translate --seed-site-cache` once, review '
...
async function main(): Promise<void> {
  ...
  if (seedSiteCache) {
    ...
    console.log('Review the cache.json diff, commit it, THEN run: pnpm run translate --corpus site --all');
    return;
  }
