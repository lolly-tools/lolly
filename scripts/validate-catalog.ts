 ...
 * Catalog validator.
 *
 * Run as: pnpm run validate:catalog
 *
 ...
 *
 * Index and checksums are generated by `pnpm run build:catalog`. This validator
...
for (const entry of toolsIndex.tools) {
  ...
  for (const field of INDEX_FIELDS) {
    if (entry[field] !== manifest[field]) {
      errors.push(`tools/index.json: "${entry.id}" ${field} "${entry[field]}" ≠ manifest "${manifest[field]}" - run \`pnpm run build:catalog\``);
    }
  }
  ...
  // both are manifest-derived, so a forgotten `pnpm run build:catalog` after adding
  ...
  if (!!entry.personalized !== !!derived.personalized) {
    errors.push(`tools/index.json: "${entry.id}" personalized ${!!entry.personalized} ≠ derived ${!!derived.personalized} - run \`pnpm run build:catalog\``);
  }
  ...
  for (const tmpl of (entry.templates ?? []) as Array<Record<string, unknown>>) {
    if (tmpl && typeof tmpl === 'object' && 'values' in tmpl) {
      errors.push(`tools/index.json: "${entry.id}" template "${tmpl.id}" carries "values" - the index must be metadata-only (values live in tools/<id>/templates/<tid>.json); run \`pnpm run build:catalog\``);
    }
  }
  ...
  if (JSON.stringify(entry.templates ?? null) !== JSON.stringify(derived.templates ?? null)) {
    errors.push(`tools/index.json: "${entry.id}" templates drifted from tools/${entry.id}/templates/*.json - run \`pnpm run build:catalog\``);
  }
}
...
for (const id of toolManifests.keys()) {
  if (!indexedIds.has(id)) {
    errors.push(`tools/index.json is missing tool "${id}" - run \`pnpm run build:catalog\``);
  }
}
...
if (!slimIndex) {
  ...
  if (slimIndex.version !== toolsIndex.version || slimIndex.generatedAt !== toolsIndex.generatedAt) {
    errors.push('tools/index.slim.json: version/generatedAt ≠ tools/index.json - run `pnpm run build:catalog`');
  }
  if (slimTools.length !== toolsIndex.tools.length) {
    errors.push(`tools/index.slim.json lists ${slimTools.length} tools, tools/index.json lists ${toolsIndex.tools.length} - run \`pnpm run build:catalog\``);
  }
  ...
  slimTools.forEach((entry, i) => {
    ...
    if (!full) {
      errors.push(`tools/index.slim.json: "${entry?.id}" is not in tools/index.json - run \`pnpm run build:catalog\``);
      return;
    }
    if (entry.preview !== full.preview) {
      errors.push(`tools/index.slim.json: "${entry.id}" preview ${entry.preview} ≠ tools/index.json ${full.preview} - run \`pnpm run build:catalog\``);
    }
    if (toolsIndex.tools[i]?.id !== entry.id) {
      errors.push(`tools/index.slim.json: "${entry.id}" is at position ${i}, tools/index.json has "${toolsIndex.tools[i]?.id}" there - the order IS the gallery's layout; run \`pnpm run build:catalog\``);
    }
    if (JSON.stringify(entry) !== JSON.stringify(slimEntry(full))) {
      errors.push(`tools/index.slim.json: "${entry.id}" drifted from its tools/index.json entry - run \`pnpm run build:catalog\``);
    }
    ...
    for (const heavy of ['i18n', 'templates', 'examples']) {
      if (entry[heavy] !== undefined) {
        errors.push(`tools/index.slim.json: "${entry.id}" carries "${heavy}" - the slim index must stay first-paint sized; run \`pnpm run build:catalog\``);
      }
    }
  });
  for (const t of toolsIndex.tools) {
    if (!slimTools.some(s => s?.id === t.id)) {
      errors.push(`tools/index.slim.json is missing tool "${t.id}" - run \`pnpm run build:catalog\``);
    }
  }
}
...
if (previewBundle) {
  for (const [id, manifest] of toolManifests) {
    ...
    looks.forEach((look: any, i: number) => {
      ...
      if (!entry) {
        warnings.push(`[${id}] example look ${i} is absent from catalog/previews/bundle.json - the gallery will LIVE-RENDER it on first paint; run \`pnpm run previews\` then \`pnpm run build:catalog\``);
      } else if (entry.sig !== sig) {
        errors.push(`[${id}] example look ${i} bundle sig is stale - the gallery will silently fall back to a live render on first paint; run \`pnpm run previews\` then \`pnpm run build:catalog\``);
      }
    });
  }
}
...

{
  ...
  // A checkout that has never run `pnpm run previews` has no dir at all. Skip the whole
...
// because the report no longer matches the file and the gate says so.
{
  ...
  if (images.length && !report) {
    errors.push(
      ...
      `nothing is measuring whether any of them is an empty card; run \`pnpm run check:blank-previews\``,
    );
    ...
    if (!thresholdsAgree) {
      errors.push(
        ...
        `its verdicts answer a different question; re-run \`pnpm run check:blank-previews\``,
      );
    }
    if (unmeasured.length || orphaned.length) {
      errors.push(
        ...
        ` - a re-captured tile is unmeasured until the probe runs again, which is exactly when a blank one would slip back in; run \`pnpm run check:blank-previews\``,
      );
    }
      ...
      for (const row of blankRows) {
        ...
        if (usable.length) {
          errors.push(
            ...
            `re-capture with \`node scripts/build-previews.ts --only=${base}\` against a running dev server, then \`pnpm run check:blank-previews\``,
          );
        } else {
          backlog.push(base);
        }
...
for (const asset of assetsIndex.assets) {
  ...
  if (asset.added !== expectedAdded) {
    errors.push(`[asset ${asset.id}] added "${asset.added ?? '(absent)'}" does not match the committed date map's "${expectedAdded ?? '(absent)'}" - run \`pnpm run build:catalog\``);
  }
  ...
  for (const formats of formatLists) {
    for (const fmt of formats ?? []) {
      ...
      if (fmt.checksum === 'sha256-PLACEHOLDER') {
        warnings.push(`[asset ${asset.id}] format "${fmt.format}" has placeholder checksum - run \`pnpm run build:catalog\``);
      } else {
        const actual = `sha256-${createHash('sha256').update(bytes).digest('base64')}`;
        if (fmt.checksum !== actual) {
          errors.push(`[asset ${asset.id}] format "${fmt.format}" checksum stale - run \`pnpm run build:catalog\``);
        }
      }
      ...
      if (expectedDepth == null && fmt.depth !== undefined) {
        errors.push(`[asset ${asset.id}] format "${fmt.format}" declares depth ${fmt.depth} but its bytes state no depth (not a raster, or an unsniffable container) - run \`pnpm run build:catalog\``);
      } else if (expectedDepth != null && fmt.depth !== expectedDepth) {
        errors.push(`[asset ${asset.id}] format "${fmt.format}" depth ${fmt.depth ?? '(absent)'} does not match the file's ${expectedDepth}-bit header - run \`pnpm run build:catalog\``);
      }
...
// maintained by `pnpm run sync:shared`. A hand-edited consumer region (or a
