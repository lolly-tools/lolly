...
/** A tool's `render` block, at least the sub-shape tools/index.json entries mirror. */
type ManifestField = 'name' | 'description' | 'version' | 'status' | 'category';

interface ToolsIndexFile {
  version?: string;
  generatedAt?: string;
  tools: CatalogIndexEntry[];
}

interface AssetFormatEntry {
  format: string;
  url: string;
  checksum?: string;
  size?: number;
...
interface CatalogAssetEntry {
  ...
  locales?: Record<string, AssetFormatEntry[]>;
}

interface AssetsIndexFile {
  assets: CatalogAssetEntry[];
}

interface CanonicalInputDef {
  type: string;
  label?: string;
  description?: string;
  min?: number;
...
interface CanonicalInputsFile {
  inputs: Record<string, CanonicalInputDef>;
}

// Fields tools/index.json mirrors from each manifest — kept in sync with
// scripts/build-catalog-index.ts.
const INDEX_FIELDS: ManifestField[] = ['name', 'description', 'version', 'status', 'category'];
...
const toolSchema = readJson<AnySchemaObject>('schemas/tool.schema.json');
const assetSchema = readJson<AnySchemaObject>('schemas/asset.schema.json');
const tokensSchema = readJson<AnySchemaObject>('schemas/tokens.schema.json');
...
const assetsIndex = readJson<AssetsIndexFile>('catalog/assets/index.json');
const toolsIndex = readJson<ToolsIndexFile>('catalog/tools/index.json');
...
const toolManifests = new Map<string, ToolManifest>(); // id → manifest

for (const dir of toolDirs) {
  const manifestPath = `tools/${dir}/tool.json`;
  const manifest = readJsonOptional<ToolManifest>(manifestPath);
  if (!manifest) {
...
for (const entry of toolsIndex.tools) {
  ...
  for (const field of INDEX_FIELDS) {
    if (entry[field] !== manifest[field]) {
      errors.push(`tools/index.json: "${entry.id}" ${field} "${String(entry[field])}" ≠ manifest "${String(manifest[field])}" — run \`npm run build:catalog\``);
    }
  }
...
const canonicalInputs = readJsonOptional<CanonicalInputsFile>('schemas/canonical-inputs.json')?.inputs ?? {};
for (const [toolId, manifest] of toolManifests) {
  for (const input of manifest.inputs) {
    ...
    const numKeys: ('min' | 'max' | 'step')[] = canon.type === 'number' ? ['min', 'max', 'step'] : [];
    ...
    if (canon.type === 'select' && Array.isArray(canon.options)) {
      if (normOptions(input.options) !== normOptions(canon.options)) {
...
/** Normalise a select input's options (SelectOption[] or canonical raw values)
 *  into a comparable string, regardless of whether each entry is a plain value
 *  or a `{ value, ... }` object. */
function normOptions(opts: unknown): string {
  if (!Array.isArray(opts)) return '';
  return opts
    .map(o => (o && typeof o === 'object' && 'value' in o) ? String((o as { value: unknown }).value) : String(o))
    .join('|');
}

// ─── Palette consistency ─────────────────────────────────────────────────────
// The CMYK-aware PDF export builds a hex→CMYK lookup (buildCmykPaletteMap in
// shells/web/src/bridge/export/pdf-cmyk.ts) keyed on each swatch's hex. The brand palette
...
{
  const cmykByHex = new Map<string, { cmyk: unknown; label: string }>(); // normalised hex → { cmyk, label }
...
const assetById = new Map<string, CatalogAssetEntry>();
...
function readJson<T = unknown>(rel: string): T {
  // JSON trust boundary: callers name the expected shape via the type parameter.
  return JSON.parse(readFileSync(join(ROOT, rel), 'utf8')) as T;
}

function readJsonOptional<T = unknown>(rel: string): T | null {
  ...
  try {
    ...
    errors.push(`[${rel}] not valid JSON: ${message}`);
    return null;
  }
}
