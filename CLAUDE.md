...
The three genuine blockers, diagnosed in `plans/69-svg-snapshot-without-print.md` §16: the **gallery** shots (the walker drops fixed-position chrome and flattens the Cover Flow covers, whose 3-D `rotateY` `parseCssMatrix` refuses); the **audiogram** shots (canvas by implementation, not necessity — vector-expressible, but it needs a change to the tool); and **`incl-neuro-viz`**, which is a real physical limit (a WebGL2 fragment shader computes a per-pixel colour field — there is no geometry to recover).
