...
### Web shell views are an orchestrator plus feature modules (2026-09-09)

The six largest web views used to be one giant closure each (`initFreeCanvas` alone was 15,600 lines holding 474 nested functions). They are now an **orchestrator file plus a directory of feature modules that share one explicit context object**: `views/free-canvas.ts` + `views/free-canvas/*.ts` (`fc: FcCtx`), `views/timeline-panel.ts` + `views/timeline-panel/` (`tp: TpCtx`), `views/catalog.ts` + `views/catalog/` (`cat: CatCtx`), `views/tool.ts` + `views/tool/` (`tview: ToolViewCtx`), `views/tool-actions.ts` + `views/tool-actions/` (`ta: ActionsCtx`), `lib/brand-editor.ts` + `lib/brand-editor/` (`bedit: BrandEditorCtx`). The rules, which the generator enforces:

- `<dir>/context.ts` declares the context interface: every former closure variable as a property (let-like state is read and written as `fc.x`; a constant is aliased at function entry, `const { cfg } = fc;`), plus one **namespace per module** (`fc.pen.enterPen()`), each an `<name>Ops(fc)` factory of bound wrappers. Cross-module calls and every use of a function as a value (event listeners) go through the namespace, so **no feature module imports another** - the only edges are module → context and module → `<dir>/shared.ts` (the module-level types, constants and helpers the closure used, moved verbatim).
- Every module function takes the context first: `export function commit(fc: FcCtx, boxes: Box[])`. Same-module calls are direct (`commit(fc, boxes)`); a call from another module is `fc.select.commit(boxes)`.
- Mount-time statement runs (setup, wiring) became `export function wireX(ctx)` blocks in a module, called in order from the orchestrator (`await tview.setup.templatePick();`).
- The split is mechanical and reproducible: `scripts/split-closure.ts <plan.json> [--apply]` with the plans in `scripts/data/closure-splits/`. Checker-resolved, so every reference is exact; it refuses to run when the context name is already declared inside the closure or a block name shadows an import. Use it on the next hotspot the maintainability ratchet names rather than growing a closure again.
- A module that is itself a closure splits again in place: the plan sets `"outDir"` to the feature directory and a `"prefix"` (`catalog/details.ts` → `catalog/details-{context,shared,panels,inline-modes,sheet}.ts`, context `dt`), and the parent's context (`cat`) arrives as a published parameter. The context creation is emitted after the block removals so a block that opens the body cannot swallow it; an optional parameter's context field is `T | undefined`.
- **Tests that pin source text must read the whole feature**, not one file: the pattern is to concatenate `views/<name>.ts` with `views/<name>/*.ts` (see `tool-template-mount.test.ts`, `design-units-contract.test.ts`, `primitive-guards.test.ts`). Expect a moved function to read `x(fc, …)` or `fc.<module>.x(…)` where the old code read `x(…)`.
