...
Lives at `shells/tauri-desktop/` in the [`lolly`](https://github.com/lolly-tools/lolly) repository. See the [note](#builds-inside-the-repository-it-lives-in) at the bottom. Before 2026-09-11 this was its own repository, `lolly-desktop`, mounted as a git submodule; its history came across intact and is archived at the old URL with a redirect notice.
...
- **`bundleRepoDirs`** answers `/tools/` and `/catalog/` in dev through the content resolver, and on build writes a real tree into `dist/` (`materializeInto`, or the neutral seed), because those are URL namespaces rather than directories and the WebView needs real files offline.
...
contract-tested from this repo. If one changes, change all package formats together.
