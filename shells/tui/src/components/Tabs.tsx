...
// Top navigation, mirroring the web shell's Tools | Projects | Assets toggle. Number
    ...
    <Box paddingX={1}>
      ...
      {TABS.map(t => (
        <Text key={t.id}>
          ...
          <Text bold={active === t.id} color={active === t.id ? theme.accentName : undefined}>{compact ? ({ tools: 'Tools', projects: 'Proj', profile: 'Me', catalog: 'Assets', system: 'Sys' } as const)[t.id] : t.label}</Text>
