...
- **Styles are plain CSS with cascade layers, and the layer order is declared exactly once.** `src/styles/app.css` opens with `@layer perf, vendor, base, primitives, chrome, views, overrides, a11y;`. Normal declarations gain priority toward the end; `!important` reverses that order. The first layer is reserved for the gated Performance UI policy, with explicit focus and tool/export exclusions. Lazy view styles belong in `views`; vendor CSS belongs in `vendor`. `fonts.css` and `tokens.css` stay unlayered so normal design-token declarations outrank normal layered styles. [Performance checks](PERFORMANCE.md) cover the real browser cascade and throttled interactions.
