...
export async function createBridge(): Promise<WebHost> {
  ...
  host.export = {
    checkLayout: async (node) => (await loadExport()).checkLayout!(node),
  ...
  // Decoder construction starts only when a tool reads formats or detects a frame.
  host.scan = createLazyScanAPI();
