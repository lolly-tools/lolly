...
/** Open failure tagged with recoverability so boot() can offer a retry. */
export class DbOpenError extends Error {
  code: 'DB_BLOCKED' | 'DB_OPEN_TIMEOUT';
  constructor(message: string, code: 'DB_BLOCKED' | 'DB_OPEN_TIMEOUT') {
    super(message);
    this.code = code;
  }
}
...
function openOnce(): Promise<IDBPDatabase> {
  ...
  // The open connection, captured so blocking() can close it. (The old code
  // called `this.close()`, but idb invokes its callbacks as plain functions —
  // `this` was undefined there, so cross-tab upgrades were silently blocked.)
  ...
  const timeout = new Promise<never>((_, reject) => {
    timer = setTimeout(() => {
      ...
      reject(new DbOpenError(
        ...
        wasBlocked ? 'DB_BLOCKED' : 'DB_OPEN_TIMEOUT',
      ));
    }, OPEN_TIMEOUT_MS);
  });
  const captured = opening.then(db => { dbHandle = db; return db; });
  return Promise.race([captured, timeout]).finally(() => clearTimeout(timer));
}
