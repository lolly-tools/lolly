...
import type { RuntimeHost } from '@lolly/engine';
import type { ComposeAPI, ComposeSpec, ComposeUrlOpts, AssetRef, ExportFormat } from '../../../../engine/src/bridge/host-v1.ts';
...
/** The web-only picker metadata for a pasted tool URL (see describeUrl below). */
export interface DescribedToolUrl {
  toolId: string;
  name: string;
  formats: string[];
  format: string;
...
/** The v1 ComposeAPI, plus the web-only `_describeUrl` picker helper. */
export interface WebComposeAPI extends ComposeAPI {
  /** Web-only host-UI chrome (the picker's detected-tool card), not part of the
   *  v1 ComposeAPI contract — underscore-prefixed like assets._* helpers. */
  _describeUrl(url: string): Promise<DescribedToolUrl | null>;
}
...
export function createComposeAPI(host: RuntimeHost): WebComposeAPI {
  ...
  async function render(spec: ComposeSpec): Promise<AssetRef> {
    ...
    // renderRowToBlob's own manifest-driven input model coerces/validates each
    // value on the way in, so this narrowing reflects an already-enforced
    // invariant rather than an unchecked cast.
    const values = inputs as Record<string, InputValue>;
    // ComposeSpec.unit is host-contract-loose (`string`, so a host-side caller
    // can't be rejected for a slightly-off spelling); renderRowToBlob narrows to
    // the closed Unit set. Manifests only ever declare the six Unit values, and
    // callers within this codebase (embed.ts, this module) only ever forward a
    // UrlState-derived unit, which is already Unit-typed at its source.
    const rowUnit = unit as Unit | undefined;
    const { blob, format: fmt } = await renderRowToBlob(
      { toolId, values },
      host,
      { format, width, height, unit: rowUnit, dpi, composeStack: [..._stack], watermark: false, embedMeta: false, thumbnail: true },
    );
    ...
    if (cache.size > CACHE_CAP) {
      ...
      if (oldestKey !== undefined) {
        ...
        if (old) { try { URL.revokeObjectURL(old.blobUrl); } catch { /* already gone */ } }
      }
    }
    return assetRef;
  }
  ...
  interface ResolvedSpec {
    parsed: NonNullable<ReturnType<typeof parseToolUrl>>;
  ...
  async function resolveSpec(url: string): Promise<ResolvedSpec | null> {
  ...
  async function describeUrl(url: string): Promise<DescribedToolUrl | null> {
  ...
  async function renderUrl(url: string, opts: ComposeUrlOpts = {}): Promise<AssetRef | null> {
    const r = await resolveSpec(url);
    if (!r) return null;
    ...
    try {
      ref = await render({
        ...
        // `format` here is normFmt's output ('svg'/'png'/'jpg'/'webp'), a superset
        // of host-v1's tool-facing ExportFormat (see the comment in bridge/embed.ts
        // on the same narrow/broad split); it flows unmodified into renderRowToBlob,
        // whose format param is a plain string, so this narrowing cast changes no
        // runtime behavior.
        format: format as ExportFormat, width, height, unit, dpi, _stack: opts._stack ?? [],
      });
    } catch (e) {
      host.log?.('warn', `renderUrl "${parsed.toolId}": ${e instanceof Error ? e.message : String(e)}`);
      return null;
    }
