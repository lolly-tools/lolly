 ...
 *
 *   - `data-pending`  no render has arrived yet. The dot is dim and small; under
 *                     motion it breathes. It is the progress indicator - there is
 *                     no spinner and no placeholder art.
 ...
 *      than two ready looks the arrows are `aria-disabled` and hold still - the
 ...
 *      is idempotent: call it after any load, any failure, or on wiring, and the
 *      dots and arrows tell the truth about that instant.
 *
 * `data-pending` is also read by lib/capture-neutral.ts: a docs capture waits for
...
export const CAR_PENDING_ATTR = PENDING_ATTR;
 ...
 * first paint of the tile. Every dot starts pending; `syncCarState` corrects any
 * that is already showing art (a lead frame) the moment the strip is wired.
 *
...
/**
 * The prev/next arrows for a strip of `count` looks. Both start `aria-disabled`:
 * on a cold tile nothing has rendered, so there is nowhere to step yet (the CSS
 * dims a disabled arrow and drops its hover state).
 */
export function carouselNavMarkup(count: number): string {
  if (count < 2) return '';
  return `<button class="gcar-nav gcar-prev" type="button" tabindex="-1" aria-hidden="true" aria-disabled="true" title="${esc(t('Previous example'))}">${CHEVRON_LEFT}</button>`
    + `<button class="gcar-nav gcar-next" type="button" tabindex="-1" aria-hidden="true" aria-disabled="true" title="${esc(t('Next example'))}">${CHEVRON_RIGHT}</button>`;
}
...
export function syncCarState(gcar: HTMLElement): void {
  ...
  gcar.querySelectorAll<HTMLElement>('.gcar-nav').forEach(b => { b.setAttribute('aria-disabled', String(inert)); });
}
 ...
 *  two looks have arrived, which is the state the arrows advertise as disabled. */
