...
import type { StateEntry } from '../../../../engine/src/bridge/host-v1.ts';
...
/**
 * The slice of a catalog index entry (see scripts/build-catalog-index.ts
 * CatalogIndexEntry, and folder-tiles.ts TileToolInfo) that this view reads.
 * Kept local rather than imported: the index is a denormalised, gallery-facing
 * projection of the tool manifest, not a domain type the engine owns.
 */
...
const catLabel = (c: string | undefined): string => (c ? CAT_LABEL[c] : undefined) ?? (c ? c[0]!.toUpperCase() + c.slice(1) : 'Other');
const statusLabel = (s: string | undefined): string => ({ official: 'Official', community: 'Community', experimental: 'Experimental' } as Record<string, string>)[s ?? ''] ?? (s ?? '');
...
const fmtLabel = (f: string): string => FMT_LABEL[f] ?? String(f).toUpperCase();
...
function byRowThenColumn(a: Element, b: Element): number {
...
function revealCards(masonry: HTMLElement | null, animate: boolean): IntersectionObserver | null {
  ...
  if (!masonry || !animate || typeof IntersectionObserver === 'undefined') {
  ...
  const io = new IntersectionObserver((entries, obs) => {
    entries.filter(e => e.isIntersecting).map(e => e.target).sort(byRowThenColumn)
      .forEach((el, i) => { reveal(el as HTMLElement, i); obs.unobserve(el); });
...
function armPreviewReveal(masonry: HTMLElement | null, animate: boolean): void {
  ...
  masonry.querySelectorAll<HTMLElement>('.gtile-hero--preview').forEach(hero => {
    ...
    if (img instanceof HTMLImageElement && img.complete && img.naturalWidth > 0) hero.classList.add('is-ready');
  });
}

export async function mountGallery(viewEl: HTMLElement, host: WebHost): Promise<void> {
  document.title = 'Lolly';
  const w: Window & { __toolIndex?: { tools: GalleryTool[] } } = window;
  const index = w.__toolIndex ?? { tools: [] };
  const [savedEntries, profile, sessionSizes, cachedPreviews] = await Promise.all([
    host.state.list() as Promise<SavedEntry[]>,
    host.profile.get(),
  ...
  const sortedSaved: SavedEntry[] = [...savedEntries].sort((a, b) => String(b.updatedAt).localeCompare(String(a.updatedAt)));
  ...
  // see armPreviewReveal) and fade in only once the image has actually decoded, so
  ...
  // Entrance reveal runs the cascade once, on cold mount — not when returning
  // from a tool (cards are already known) nor on filter/search re-renders (those
  // show instantly). Tracked here so render() can decide whether to disconnect cleanly.
  const isReturning = viewEl.classList.contains('is-returning');
  const prefersReduced = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches ?? false;
  let firstPaint = true;
  let revealObserver: IntersectionObserver | null = null;

  function matchingTools(): GalleryTool[] {
    const q = query.trim();
    // Alphabetical by name (case-insensitive). With category pills + search making
    // any tool easy to reach, a stable A–Z order is a more predictable scan than
    // the catalog's authoring order. Sorts a filtered copy, not index.tools.
    return index.tools
      .filter(t => {
  ...
  function renderPills(): void {
    ...
    // Show the filter FAB as "has active filter" when a non-default pill is selected.
    filterFab?.classList.toggle('has-active', !allActive);
  }
    ...
    container.querySelectorAll<HTMLElement>('[data-history]').forEach(el => {
      el.addEventListener('click', (e) => {
        ...
        showHistoryDialog(tool, entriesByTool.get(tool.id) ?? [], sessionSizes, host, {
          // Update in-memory state (per-tool list + FAB count) as the dialog deletes,
          // then re-render once it closes so the gallery reflects any change.
          onDelete: (slot) => {
          ...
          onClose: () => {
            // Re-render so the card's history count / continue state stays accurate.
            render();
  ...
  if (previewSig && host.previews) {
    ...
    if (toRegenerate.length) {
      ...
      (viewEl as HTMLElement & { _cleanup?: () => void })._cleanup = cancel;
    }
  }
}
...
function showHistoryDialog(
  ...
  host: WebHost,
  { onDelete, onClose }: ShowHistoryDialogOpts = {},
): void {
  if (!tool) return;
  const dialog = document.createElement('dialog');
  ...
  dialog.querySelectorAll<HTMLElement>('[data-delete]').forEach(el => {
    el.addEventListener('click', async (e) => {
      ...
      onDelete?.(slot);
      // update in-memory state only — render happens on close
      changed = true;
...
function savedItem(entry: SavedEntry, bytes: number | undefined, toolName = ''): string {
  ...
  const thumb = batch
    ...
    : entry.thumb
      ? `<img class="saved-thumb" src="${escape(entry.thumb)}" aria-hidden="true">`
      : `<span class="saved-thumb saved-thumb--empty"></span>`;
  const when = fmtDateTime(new Date(entry.updatedAt));
  ...
  // The tool name is already in the row's title (h4) just above, so the sub-line only needs
  ...
  return `
    <li class="saved-row${batch ? ' saved-row--batch' : ''}" data-search="${escape(searchText)}">
      ...
      ${thumb}
      <span class="saved-label"><h4>${escape(title)}</h4><small>${escape(subtitle)}</small></span>
      ${size}
