...
/** One collapsible section of the page (a `<details>` panel of cards). */
...
function card({ icon, title, features }: CapCard): string {
  return `
...
export async function mountCapabilities(viewEl: HTMLElement, _host: WebHost): Promise<void> {
  ...
  const toolCount = w.__toolIndex?.tools?.length ?? null;

  const stat = (n: number, label: string): string => `<span class="plat-stat"><strong>${escape(String(n))}</strong>${escape(label)}</span>`;
  const chip = (label: string): string => `<span class="plat-chip">${escape(label)}</span>`;

  const panel = (flag: string, defaultOpen: boolean, id: string, title: string, desc: string, body: string): string => `
