...
import type { AssetRef, ExportFormat, InputValue } from '@lolly/engine';
import { createRuntime, serializeUrlState, buildEmbedUrl } from '@lolly/engine';
import type { WebHost } from '../bridge/index.ts';
import type { WebAssetPickerOpts, UserAssetRecord } from '../bridge/assets.ts';
import type { DescribedToolUrl } from '../bridge/compose.ts';
import { getTool } from '../bridge/tool-loader.ts';
import { downscaleRaster } from '../bridge/image-resize.ts';
...
/**
 * A slice of a runtime catalog-index tool entry (window.__toolIndex — see
 * catalog/sync.ts's ToolIndex, gallery.ts's local GalleryTool) that this view
 * reads. Kept local rather than imported: the index is a denormalised,
 * gallery/picker-facing projection of the tool manifest, not a domain type
 * the engine owns.
 */
...
type TabId = 'library' | 'sessions' | 'tools';
interface Tab {
  id: TabId;
  label: string;
}

/** Narrow an unknown catch value to an Error carrying a `code` property (see
 *  bridge/assets.ts's userAssetError — cap/quota errors set one). */
function hasErrorCode(e: unknown): e is Error & { code: string } {
  return e instanceof Error && typeof (e as { code?: unknown }).code === 'string';
}

export function openPicker(host: WebHost, opts: WebAssetPickerOpts = {}): Promise<AssetRef | null> {
...
async function render(
  ...
  const w: Window & { __toolIndex?: { tools: PickerTool[] } } = window;
  const toolIndex = (typeof window !== 'undefined' && w.__toolIndex?.tools) || [];
  ...
  // body/libraryEl/toolcardHost are unconditionally present in the template just
  // set above — asserted non-null (mirrors gallery.ts's identical convention for
  // elements guaranteed by the just-rendered static markup).
  ...
  function focusCard(el: HTMLElement | undefined): void {
  ...
  function moveSelection(cur: HTMLElement, key: string): void {
    ...
    if (key === 'ArrowRight') { focusCard(cards[i + 1]); return; }
    if (key === 'ArrowLeft')  { focusCard(cards[i - 1]); return; }
  ...
  root.querySelector('.asset-picker-panel')?.addEventListener('keydown', (e) => {
    ...
    const cur = e.target instanceof HTMLElement
      ? e.target.closest<HTMLElement>('[data-asset-id],[data-tool-id],[data-session-slot]')
      : null;
  ...
  root.querySelector('.asset-picker-tabs')?.addEventListener('click', (e) => {
    const btn = e.target instanceof HTMLElement ? e.target.closest<HTMLElement>('[data-tab]') : null;
    const tabId = btn?.dataset.tab;
    if (tabId === 'library' || tabId === 'sessions' || tabId === 'tools') setTab(tabId);
  });
  ...
  body.addEventListener('click', async (e) => {
    ...
    const sess = target.closest<HTMLElement>('[data-session-slot]');
    if (sess?.dataset.sessionSlot) { embedSession(sess.dataset.sessionSlot); return; }
    const tool = target.closest<HTMLElement>('[data-tool-id]');
    if (tool?.dataset.toolId) { embedTool(tool.dataset.toolId); return; }
    const pick = target.closest<HTMLElement>('[data-asset-id]');
    if (pick?.dataset.assetId) {
      try {
        ...
        alert(`Could not resolve asset: ${err instanceof Error ? err.message : String(err)}`);
      }
    }
  });
  ...
  function setTab(id: TabId): void {
    ...
    if (searchInput) searchInput.placeholder = placeholderFor(id);
  ...
  function renderUserAssets(): void {
    ...
    for (const a of userAssets) {
      ...
      if (f) {
        if (!groups.has(f.id)) groups.set(f.id, { name: f.name, items: [] });
        groups.get(f.id)?.items.push(a);
      } else {
        ungrouped.push(a);
      }
    }
  ...
  if (opts.allowUpload) {
    ...
    fileInput?.addEventListener('change', async () => {
      ...
      try {
        ...
        // Cap/quota errors (see bridge/assets.ts's userAssetError) carry a
        // user-ready message on a `code` property; prefix only the rest.
        const message = e instanceof Error ? e.message : String(e);
        alert(hasErrorCode(e) ? message : `Upload failed: ${message}`);
      } finally {
  ...
  function restoreLibrary(q: string): void {
    ...
    renderLibrary(libraryCandidates.filter(c =>
      String(c.meta?.name ?? c.id).toLowerCase().includes(q) || c.id.toLowerCase().includes(q)
    ));
  }
  ...
  // (see views/tool/embed-editor.ts's identical asExportFormat).
  ...
  function showToolCard(desc: DescribedToolUrl, url: string, { editUrl }: { editUrl?: string } = {}): void {
    ...
    if (!cardEl || !fmtSel || !wEl || !hEl || !previewEl || !useBtn) return;
    ...
    let pending: AssetRef | null = null;     // the AssetRef the Use button will commit
    ...
    const renderPreview = async (): Promise<void> => {
      ...
      // renderUrl is optional on ComposeAPI (older hosts may lack it), but this
      // card only ever shows when allowToolUrl already confirmed it's present.
      const renderUrl = host.compose.renderUrl;
      if (!renderUrl) { previewEl.innerHTML = `<p class="asset-picker-error">Couldn't render this link.</p>`; return; }
      const ref = await renderUrl(url, {
  ...
  async function embedSession(slot: string): Promise<void> {
    ...
    try {
      ...
      // SavedStateData's index signature is `InputValue | undefined` (a slot may
      // simply lack a given input); createRuntime's initialState wants only
      // present values, so drop the (rare) undefined ones rather than widen its
      // contract (mirrors pro/render-export.ts's own Record<string, InputValue> seed).
      const values: Record<string, InputValue> = {};
      for (const [k, v] of Object.entries(data)) if (v !== undefined) values[k] = v;
  ...
  if (allowToolUrl) {
    host.state.list()
      .then(list => {
        sessions = (list ?? [])
          .filter(e => e.slot && !isBatchSlot(e.slot)) // single-tool only (see batch-slots.ts)
  ...
  try {
    ...
    libraryEl.innerHTML = `<p class="asset-picker-error">Failed to load: ${escape(e instanceof Error ? e.message : String(e))}</p>`;
  }
}
...
function card(ref: AssetRef): string {
  ...
  return `
    <button type="button" class="asset-picker-card" data-asset-id="${escape(ref.id)}">
      ...
      <span class="asset-picker-name" title="${escape(String(name))}">${escape(String(name))}</span>
...
function sessionThumb(thumb: string | null | undefined, iconSvg: string | null | undefined): string {
...
function userCard(ref: AssetRef): string {
  ...
  return `
    <div class="asset-picker-card asset-picker-card-user">
      ...
      <button type="button" class="asset-picker-card-delete" data-delete-id="${escape(ref.id)}" title="Delete" aria-label="Delete ${escape(String(name))}">×</button>
      ${formatBadge(ref)}
    </div>
  `;
}
...
async function sanitizeSvgFile(file: Blob): Promise<Blob> {
  try {
 ...
 */
function openWebcamCapture(host: WebHost): Promise<AssetRef | null> {
  return new Promise((resolve) => {
    ...
    if (!videoEl || !statusEl || !shootBtn) return;
    ...
    (async () => {
      try {
        ...
        showError(e instanceof DOMException && e.name === 'NotAllowedError'
...
/**
 * The host slice storeUserUpload needs: write + resolve a user asset. Narrower
 * than WebHost so callers with a partial host view (e.g. views/tool/drop.ts's
 * DropHost) can satisfy it without needing the full bridge surface.
 */
...
export async function storeUserUpload(host: UploadHost, file: File): Promise<AssetRef> {
  ...
  let blob: Blob = file;
  let format: string = extFromMime(file.type);
  ...
  if (isVector) {
    ...
    ({ width, height } = await readDimensions(blob).catch(() => ({ width: undefined, height: undefined })));
  } else {
