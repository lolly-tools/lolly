...
export function projectsTopRight(folderId: string | null): string {
  ...
  return `${viewOptionsButtonHtml('projects-viewopts', { popup: 'dialog' })}
    <a class="history-fab" href="${escapeHtml(historyHref)}" aria-label="${escapeHtml(t('History'))}" title="${escapeHtml(t('History'))}">${icon('history')}</a>`;
}
