...
/**
 * The Assets view side of the inline video Grade/Trim mode (plans/130) - a SOURCE
 ...
 *
 * Run directly:
 *   node --test shells/web/src/views/video-edit-assets.test.ts
 */
...
const catalog = [readFileSync(new URL('./assets.ts', import.meta.url), 'utf8'), ...readdirSync(catDir).filter((n) => n.endsWith('.ts')).sort().map((n) => readFileSync(new URL(n, catDir), 'utf8'))].join('\n');
// The mode's rules are eager (asset-shared.css, since views/video-edit-inline.ts renders them);
// the view-only takeover rules may sit in the lazy half (assets.css). Read both as one sheet.
const css = ['asset-shared.css', 'assets.css'].map((f) => readFileSync(new URL(`../styles/parts/${f}`, import.meta.url), 'utf8')).join('\n');
