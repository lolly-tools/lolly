...
const WEIGHT_RAMP: number[] = [100, 300, 400, 500, 700, 900];
...
const isTransparent = (hex: string): boolean => !hex || hex.toLowerCase() === 'transparent';
const cmykText = (cmyk: PaletteEntry['cmyk']): string =>
...
type PaletteRamp = [string, PaletteEntry[]];

interface GroupedPalette {
  brand: PaletteEntry[];
...
function groupPalette(palette: readonly PaletteEntry[]): GroupedPalette {
  ...
  for (const c of palette) {
    if (c.group === 'spectrum') {
      ...
      if (m && m[1] !== undefined) addToRamp(m[1], c);
      else brand.push(c);
    }
  }
...
function swatch(c: PaletteEntry): string {
...
const liveValue = (key: string): string => LIVE_VALUES[key]?.() ?? DASH;
...
function fmtBytes(n: number): string {
  ...
  return `${i === 0 ? n : n.toFixed(n >= 10 ? 0 : 1)} ${unit}`;
}
...
function browserIcon(name: string): string | null {
  ...
  if (/edge/i.test(name)) return ICONS.edge ?? null;
  if (/opera|opr\b/i.test(name)) return ICONS.opera ?? null;
  if (/samsung/i.test(name)) return null; // no distinct mark → generic browser
  if (/firefox/i.test(name)) return ICONS.firefox ?? null;
  if (/chrom/i.test(name)) return ICONS.chrome ?? null; // Chrome / Chromium
  if (/safari/i.test(name)) return ICONS.safari ?? null;
  return null;
}
function osIcon(os: string): string | null {
  if (!os || os === DASH) return null;
  if (/chrome\s*os|cros/i.test(os)) return ICONS.chrome ?? null; // before the generic mac/linux checks
  if (/windows/i.test(os)) return ICONS.windows ?? null;
  if (/mac|ios|ipad/i.test(os)) return ICONS.apple ?? null;
  if (/android/i.test(os)) return ICONS.android ?? null;
  if (/linux/i.test(os)) return ICONS.linux ?? null;
  return null;
}
...
function gpuIcon(vendor: string): string | null {
  if (vendor && vendor !== DASH && /apple/i.test(vendor)) return ICONS.apple ?? null;
  return null;
}
...
/** Narrows the broad `RenderingContext` union `getContext('experimental-webgl')`
 *  returns (lib.dom has no dedicated overload for that legacy identifier) down
 *  to the WebGL contexts this function actually needs — a runtime check rather
 *  than a cast, so no `any`/`as` is required. */
function isWebglContext(
  ctx: RenderingContext | null,
): ctx is WebGLRenderingContext | WebGL2RenderingContext {
  return !!ctx && 'getExtension' in ctx;
}
...
function readGpu(): GpuInfo | null {
  try {
    ...
    const rawGl: RenderingContext | null = gl2 || canvas.getContext('webgl') || canvas.getContext('experimental-webgl');
    if (!isWebglContext(rawGl)) return null;
    const gl = rawGl;
    const dbg = gl.getExtension('WEBGL_debug_renderer_info');
    // getParameter's return type is intrinsically `any` in lib.dom (it depends
    // on the pname argument, which the DOM types can't express) — annotating
    // these locals as `string`/`number` is the honest shape at this boundary,
    // without adding an explicit `any` or a cast of our own.
...
function describeGpu(rawVendor: string, rawRenderer: string): DescribedGpu {
  ...
  if (vParen && vParen[1] !== undefined && vParen[2] !== undefined) {
  ...
  if (wrap && wrap[1] !== undefined && wrap[2] !== undefined && /angle/i.test(wrap[1])) {
    translation = wrap[1].trim();
    const parts = wrap[2].split(',').map((p) => p.trim());
    if (parts.length) {
      if (!vParen) hwVendor = parts[0] ?? hwVendor; // no paren vendor → take ANGLE's vendor field
...
/** `navigator.userAgentData` (UA-Client-Hints; Chromium-only today; absent elsewhere). */
interface NavigatorUAData {
  mobile?: boolean;
  getHighEntropyValues?: (hints: string[]) => Promise<UaHighEntropyValues>;
}

/** `navigator.connection` (Network Information API; Chromium-only, vendor-prefixed history). */
...
/** The extra, non-standard-or-optional navigator surface this view reads.
 *  lib.dom doesn't declare these (experimental / vendor-prefixed APIs), so
 *  they're widened here rather than cast — same pattern as capabilities.ts's
 *  ChromiumProbe. */
...
async function collectClientInfo(): Promise<ClientGroup[]> {
  ...
  let hints: UaHighEntropyValues | null = null;
  try {
  ...
  groups.push({
    ...
    rows: [
      ...
      {
        k: 'Dynamic range',
        v: ({ high: 'High (HDR)', standard: 'Standard' } as Record<string, string>)[
  ...
  try {
    ...
    if (est && Number.isFinite(est.quota)) {
      capRows.push({ k: 'Storage', v: `${fmtBytes(est.usage || 0)} of ${fmtBytes(est.quota ?? 0)}` });
    }
  } catch {
    /* ignore */
  }
...
/** Denormalised projection of a catalogue-index tool entry that this view reads
 *  (see GalleryTool in gallery.ts for the fuller precedent) — kept local
 *  because the index shape is a build artifact, not a domain type the engine
 *  owns. */
...
export async function mountPlatform(viewEl: HTMLElement, _host: WebHost): Promise<void> {
  ...
  const isOpen = (flag: string, defaultOpen: boolean): boolean => params.has(flag) || defaultOpen;
  ...
  const w: Window & { __toolIndex?: { tools: PlatformTool[] } } = window;
  ...
  const stat = (n: number, label: string): string => `<span class="plat-stat"><strong>${n}</strong>${escape(label)}</span>`;
              ...
              <article class="plat-font">
                ...
                <div class="plat-font-specimen" style="font-family:${f.stack}">
                  ...
                  <div class="plat-font-weights">
                    ${WEIGHT_RAMP.map((wt) => `<span style="font-weight:${wt}">${wt}</span>`).join('')}
                  </div>
                </div>
                <dl class="plat-kv">
  ...
  viewEl.querySelectorAll<HTMLButtonElement>('.plat-swatch-chip[data-copy]').forEach((btn) => {
  ...
  function wireDevice(): void {
    const device = viewEl.querySelector<HTMLDetailsElement>('.plat-device');
    ...
    // The router (main.js's navigate()) calls view._cleanup() on view switch —
    // same convention as gallery.ts's cancellation hook.
  ...
  const mountEl = viewEl as HTMLElement & { _platMount?: number };
  const myMount = (mountEl._platMount = (mountEl._platMount || 0) + 1);
  function isCurrent<T extends Element>(node: T | null): node is T {
    return mountEl._platMount === myMount && !!node && viewEl.contains(node);
  }
  ...
  fetchAssetSummary()
    .then((assets) => {
      const block = viewEl.querySelector<HTMLElement>('[data-asset-block]');
      if (!isCurrent(block)) return;
      const stats = block.querySelector<HTMLElement>('[data-asset-stats]');
      if (!assets) {
      ...
      if (count) { count.textContent = String(assets.total); count.hidden = false; }
      if (stats) {
...
/** One entry in /catalog/assets/index.json — either an array of these, or an
 *  object with an `assets` array of these (see the `Array.isArray` check below). */
interface AssetIndexRecord {
  type?: string;
  [key: string]: unknown;
}

/** True at a real trust boundary (parsed JSON from a fetch): narrows `unknown`
 *  to the minimal shape fetchAssetSummary reads, without asserting more than
 *  that. */
function isAssetIndexRecord(v: unknown): v is AssetIndexRecord {
...
async function fetchAssetSummary(): Promise<AssetSummary | null> {
  try {
    ...
    const idx: unknown = await resp.json();
    const rawArr: unknown = Array.isArray(idx)
      ? idx
      : (isAssetIndexRecord(idx) && Array.isArray(idx.assets) ? idx.assets : []);
    const arr: AssetIndexRecord[] = Array.isArray(rawArr) ? rawArr.filter(isAssetIndexRecord) : [];
