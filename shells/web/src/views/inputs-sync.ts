 ...
 *
 * renderInputs() (in the tool view) rebuilds the whole control panel's innerHTML
 * and re-wires every listener — necessary on first render or a structural change,
 * but pure waste on a keystroke, where the only thing that changed is a value the
 ...
 * imports the tool view carries, so the decision is unit-testable under jsdom.
 *
 ...
 */
import type { InputControl, InputValue } from '@lolly/engine';
...
function isEditingBlockNumberField(el: HTMLElement): boolean {
  ...
  // Structural check, NOT instanceof: the unit tests drive this with plain
  // stubs (no DOM globals), and only <input type="number"> carries this shape.
  const a = active as Element & { type?: string; dataset?: { fieldId?: string } };
  return a.type === 'number' && !!a.dataset?.fieldId;
}
