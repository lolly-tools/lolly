 ...
 * driven only by the input declaration (the engine manifest shape).
 *
 ...
 * Keeping a focused renderer means the single-tool view stays untouched and the
 * feature can be lifted out cleanly.
 *
...
import type { InputValue, SelectOption, BlockFieldSpec } from '@lolly/engine';
 ...
 * Returns '' for types with no inline control (caller renders read-only).
 */
export function controlHtml(input: ControlSpec, value: InputValue | undefined, attrs = ''): string {
  ...
  switch (t) {
    ...
    case 'asset': {
      ...
      const url = asStr(readProp(value, 'url'));
      const id = asStr(readProp(value, 'id'));
      const isObj = !!value && typeof value === 'object';
      if (!isObj || !(url || id)) {
        return `<button type="button" ${common} data-asset-pick>Choose…</button>`;
      }
      const name = asStr(readProp(readProp(value, 'meta'), 'name')) || id || 'Selected';
      const thumb = url ? `<img class="pro-asset-thumb" src="${esc(url)}" alt="">` : '';
      // Trailing badge: ✓ turns into a clickable ✕ on hover, so an image
    ...
    case 'vector': {
      ...
      const vObj: object = (value && typeof value === 'object') ? value : {};
      const sub = fields.map(f => {
        const fv = readProp(vObj, f.id) ?? f.default ?? f.min ?? 0;
...
/** Value of a form control element, or '' if it isn't one. */
function elValue(el: HTMLElement): string {
...
export function readControlValue(el: HTMLElement, input: ControlSpec): InputValue {
  switch (input.type) {
    ...
    case 'number': {
      const raw = elValue(el);
      if (raw === '') return input.default ?? 0;
      const n = Number(raw);
    ...
    case 'vector': {
      ...
      root.querySelectorAll<HTMLInputElement>('[data-vec-field]').forEach(s => {
        const key = s.dataset.vecField;
        if (key) byId[key] = s;
      });
