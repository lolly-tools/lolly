...
export function attachReorder(container: HTMLElement, {
  ...
  const rowsIn = (tbody: HTMLElement): HTMLElement[] =>
    [...tbody.querySelectorAll<HTMLElement>(':scope > tr[data-row]')];
  ...
  function onDown(e: PointerEvent): void {
    ...
    if (!(tr instanceof HTMLElement) || !tbody) return;
  ...
  function onUp(e: PointerEvent): void {
    if (!drag) return;
    const d = drag; // narrow once — callbacks below can't see the null check
    const order = rowsIn(d.tbody).map(r => r.dataset.row ?? '');
    const changed = order.length !== d.startOrder.length
      || order.some((id, i) => id !== d.startOrder[i]);
    d.tr.classList.remove('pro-row-drag-active');
