 ...
 *
 * This module derives the batch grid's column set from the selected tools and
 * decides, per column, whether it can be "bulk written" (one value applied to
 * every row at once) or must be edited cell-by-cell.
 *
 * It is deliberately free of side effects so it can be unit-tested in isolation
 * (see model.test.js) and the whole /pro feature reasoned about — or
 ...
 *   member — a tool's declaration of that input (carries its constraints).
 *   bulk   — every member is compatible, so the column header is actionable.
 */
import type { InputSpec, InputManifest, SelectOption } from '@lolly/engine';
...
export interface Column {
  ...
  inline: boolean;
  /** Why not bulk (for header tooltip). */
  reason: string;
...
// Input types we can edit inline in a grid cell. `blocks` (repeating groups) and
// anything unknown are surfaced as read-only cells: they render with the
...
// twice). Mirrors RESERVED in engine/src/url-mode.ts.
 ...
 * @param rows
 * @returns columns in first-seen order
 *
 * Column = {
 *   key, label, type|'mixed',
 *   members: Map<toolId, inputDecl>,   // distinct tools that declare the id
 *   bulk: boolean,                     // header actionable?
 *   inline: boolean,                   // can any cell be edited inline?
 *   reason: string,                    // why not bulk (for header tooltip)
 *   spec: inputDecl|null,              // representative decl for bulk control
 * }
 */
export function deriveColumns(rows: BatchRow[]): Column[] {
  ...
  for (const row of rows) {
    ...
    for (const input of row.manifest.inputs ?? []) {
      ...
      // chrome and export-dimension feed — never their own column.
      ...
      // First non-empty label wins display; keyed by tool so repeated
...
function finalizeColumn(col: DraftColumn): Column {
  ...
  const uniformType: ColumnType = types.size === 1 && first ? first.type : 'mixed';
  ...
  return {
    ...
    bulk,
    reason,
    spec: bulk && first ? first : null,
  };
}

/**
 * The input declaration for a given (row, column) pair, or null if this row's
 * tool does not have the input (→ cell is greyed out / disabled).
 */
...
/**
 * Rows a bulk write to `column` should touch: those whose tool declares the
 * column AND whose cell is inline-editable. Returns row references.
 */
