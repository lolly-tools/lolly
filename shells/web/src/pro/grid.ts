 ...
 */
import { cellInput, isCellEditable, type Column } from './model.ts';
...
import type { InputValue, ToolManifest, Unit } from '@lolly/engine';
  ...
  const action = col.bulk
    ...
    : col.type === 'blocks'
      ...
      : col.reason
        ...
        : '';
  // label itself is the collapse control (distinct from the Fill button).
  return `<th class="pro-col" data-col="${esc(col.key)}" style="${dataWidthStyle(col.key, widths)}">
    <div class="pro-col-head">
      <button type="button" class="pro-col-label" data-collapse-col="${esc(col.key)}" title="Hide “${esc(col.label)}” — click its tag below to bring it back">${esc(col.label)}</button>
      ${action}
    </div>
  </th>`;
}
...
function exportHeaderCell(col: ExportCol, widths: Record<string, number> | undefined): string {
  return `<th class="pro-col-fixed" data-col="${esc(col.key)}" style="${widthStyle(col.key, widths)}">
    <div class="pro-col-head">
      <button type="button" class="pro-col-label" data-collapse-col="${esc(col.key)}" title="Hide “${esc(col.label)}” — restore it from the tag at the front of the bar below">${esc(col.label)}</button>
    </div>
  </th>`;
}

// + Row / +5 / Fill last — rendered directly after </table> (see
// renderGridHtml), not in the columns bar, so they can be positioned/styled
// independently. "Fill last" is disabled unless there's both a template-filled
// source row and an empty row to fill.
...
/** Bottom bar (always shown): add-row buttons at left — anchored where you'd
 * use them — then the "hide all" control + restorable column tags, and the
 * queued-renders count pushed to the right.
 * @param visible currently-shown data columns
 * @param hidden collapsed data columns (shown as restore tags)
 * @param queued rows with a template chosen (the render count)
 */
...
function dataCell(col: Column, row: GridRow, ctx: GridCtx): string {
  ...
  if (input.type === 'blocks') {
    ...
    const preview = n && firstField
      ? arr.slice(0, 2).map(r => readProp(r, firstField)).filter(v => v != null && v !== '').map(String).join(', ')
      : '';
  ...
  if (isCellEditable(col, row, { assetPicker: ctx.assetPicker })) {
...
export function bodyRow(row: GridRow, columns: Column[], ctx: GridCtx): string {
  ...
  const tool = ctx.tools.find(t => t.id === row.toolId);
  ...
  // Unit selector + DPI for the row. DPI only bites physical units + raster,
 ...
 * @param state    { rows, colWidths, ... }
 * @param columns  visible columns (from deriveColumns(), minus collapsed)
 * @param ctx      { tools, assetPicker }
 * @param hidden   collapsed columns, rendered as restorable tags below
 */
