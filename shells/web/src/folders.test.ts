 ...
 * Run directly:  node --experimental-strip-types --test shells/web/src/folders.test.ts
 */
...
// The profile record as the store sees it (folders + arbitrary sibling fields).
type TestProfile = { [key: string]: unknown; folders?: Folder[] };
...
function makeHost({ slots = [], images = [] }: { slots?: string[]; images?: string[] } = {}) {
  ...
  return {
    profile: {
      async get(): Promise<TestProfile> { return profile; },
      async set(p: TestProfile): Promise<void> { profile = p; },
    },
    state: { async list() { return slots.map(slot => ({ slot })); } },
    assets: { async _listUserAssets() { return images.map(id => ({ id })); } },
    _profile: (): TestProfile => profile,
  };
}
...
test('prune drops refs missing from both backing stores', async () => {
  ...
  const folder = await store.get(a.id);
  assert.ok(folder);
  const refs = folder.items.map(i => i.ref).sort();
...
test('mutations preserve sibling profile fields', async () => {
  const host = makeHost();
  await host.profile.set({ firstname: 'Ada', featureFlags: { 'pro-batch': true } });
  const store = createFolderStore(host);
