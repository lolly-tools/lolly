...
function toHex(value: unknown): string {
  ...
  if (value && typeof value === 'object' && 'ref' in value && typeof value.ref === 'string') {
    const cached = 'value' in value ? value.value : undefined;
...
export function wireColorField(scope: HTMLElement, { onChange = () => {}, onInteractStart, onInteractEnd }: WireColorFieldOpts = {}): void {
  ...
  function armOutside(field: HTMLElement, popover: HTMLElement): void {
    ...
    const handler = (e: PointerEvent) => {
      if (!(e.target instanceof Node) || !field.contains(e.target)) {
        popover.hidden = true; popover.style.cssText = '';
        field.querySelector('.color-trigger')?.setAttribute('aria-expanded', 'false');
        disarmOutside();
      }
    };
    outside = handler;
