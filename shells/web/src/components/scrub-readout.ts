...
function place(b: HTMLDivElement, targetX: number, topAvoid: number, bottomAvoid: number): void {
 ...
 * @param o.text      value to display
 * @param o.anchorEl  anchor above this element (desktop)
 * @param o.finger    track above this touch point (mobile)
 */
export function showScrubReadout(
  ...
  if (finger) {
    place(b, finger.x, finger.y - FINGER, finger.y + FINGER);
  } else if (anchorEl) {
    const r = anchorEl.getBoundingClientRect();
    place(b, r.left + r.width / 2, r.top, r.bottom);
  }
