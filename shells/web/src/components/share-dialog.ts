...
function shareUrlFromParts(parts: readonly string[], toolId?: string | null): string {
  const qs = parts.join('&');
  const query = qs ? '?' + qs : '';
  const id = toolId
...
/** The manifest slice that drives which share toggles are offered. */
interface ShareManifest {
  version?: string | number;
  render?: {
    export?: boolean;
    formats?: readonly string[];
    actions?: readonly unknown[];   // ToolRenderSpec.actions (schema enum; opaque here)
  };
}
...
export function openShareDialog({ toolId, baseParts = [], manifest = {}, currentFormat = '', title = 'Share this tool' }: ShareDialogOpts): HTMLDialogElement {
  ...
  // Static parts of the innerHTML above — always present.
  ...
  const flagParts = () => {
    ...
    for (const cb of checkboxes) {
      ...
      if (!flag) continue;
      parts.push(flag === '_v' ? `_v=${encodeURIComponent(String(version))}` : flag);
    }
    return parts;
  };
