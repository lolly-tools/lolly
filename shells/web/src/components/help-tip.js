...
export function wireHelpTips(scope) {
  if (scope._helpTipsWired) return;
  scope._helpTipsWired = true;

  const btnFor = (pop) =>
    pop.closest('.input-row, .block-control')?.querySelector('.help-tip-btn');

  const closeAll = (except) => {
    scope.querySelectorAll('.help-tip-pop:not([hidden])').forEach((pop) => {
  ...
  scope.addEventListener('click', (e) => {
    ...
    const pop = btn.closest('.input-row, .block-control')?.querySelector('.help-tip-pop');
    if (!pop) return;
  ...
  scope.addEventListener('keydown', (e) => {
    ...
    const pop = scope.querySelector('.help-tip-pop:not([hidden])');
    if (!pop) return;
    pop.hidden = true;
  ...
  scope._helpTipDismiss = (e) => {
    if (!e.target.closest('.help-tip-btn, .help-tip-pop')) closeAll(null);
  };
...
export function linkHelpDescriptions(scope) {
  scope.querySelectorAll('.help-tip-pop[id]').forEach((pop) => {
