...
import type { Profile, RuntimeHost } from '@lolly/engine';
 ...
 * render path needs (RuntimeHost — renderRowToBlob mounts a real runtime),
 * plus the previews store this module writes thumbnails into.
 */
interface PreviewHost extends RuntimeHost {
...
export function regeneratePreviews({ host, tools, sig, onThumb }: RegeneratePreviewsOpts): () => void {
  ...
  async function step() {
    ...
    try {
      ...
      host.log?.('warn', `Personalized preview failed for ${toolId}`, { error: msg });
    }
...
async function rasterToThumbnailDataUrl(blob: Blob, maxW = 720, maxH = 560): Promise<string> {
