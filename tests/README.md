...
Contract + integration tests for the engine and cross-cutting behaviour (node:test, no framework). `pnpm test` discovers this directory together with every co-located suite through `scripts/run-test-suite.ts`. Discovery is deterministic, and `tests/test-shards.test.ts` proves every test file belongs to exactly one non-empty shard.

```bash
pnpm test
pnpm run test:shards       # inventory and file counts
pnpm run test:unit:engine
...
| Promise | Gate |
|---|---|
| A constrained mobile visitor gets a usable first render | `pnpm run check:first-load <preview-url>` (Lighthouse mobile smoke; run on a deploy, not every local test) |
...
- `engine/src/**/*.test.ts` - co-located tests beside the engine module they cover (`audio-clean`, `chroma-key`); the `engine/src` root is discovered by `scripts/run-test-suite.ts`, so a new one runs in `pnpm test` with no registration.
...
`pnpm test` must stay green on a machine with nothing extra installed; these self-skip (or run reduced):
...
- **Private brand content:** suites that read SUSE tools gate on the SOURCE pack under `brands/suse/tools/`, never on a resolved content path: they skip cleanly when `brands/suse/tools` isn't mounted (public CI / lolly-start checkouts), but with the pack mounted a missing tool dir FAILS the suite - a renamed/deleted tool can't silently turn the tests green (`color-block`, `connector-geometry`, `export-size`, and the SUSE half of `parity-constants`). `community/` and `brands/lolly-start/` are directories in this repository, so their tools are present in any checkout and those suites (`deck-builder-freeform`, `deck-builder-markdown`, `rebrand-deck-tool`, the lolly-start half of `parity-constants`) assert existence unconditionally. (`deck-builder-style.test.ts` is the one that gates on the active content profile instead: it asks the resolver whether `deck-builder` is in the profile and skips when it is not.)
...
- **`CORE_PACK=1`:** `core-pack.test.ts` runs `scripts/pack-core.ts` end to end - compile `@lolly-tools/core` to ESM + `.d.ts`, `npm pack` it, install the tarball into a scratch project and assert it both runs and type-checks there. Two `tsc` runs and two `pnpm install`s (~1 min, network on a cold cache), so it skips by default; set `CORE_PACK=1` to run. This is the only check that the PUBLISHED package works - the checked-in `exports` point at raw `./src/*.ts` for the workspace's no-build loop, which Node refuses to type-strip under `node_modules`.
...
- **[`docs/parser-inventory.md`](../docs/parser-inventory.md)** - the generated view of every untrusted-input parser with its bound constants, tests, and fuzz target. Edit `security/parser-assurance.json`, then run `pnpm run build:parser-inventory`; CI checks both the generated view and the live `ALL_TARGETS` mapping.
