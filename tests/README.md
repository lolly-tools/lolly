...
- `engine/src/**/*.test.ts` - co-located tests beside the engine module they cover (`audio-clean`, `chroma-key`); the `engine/src` root is discovered by `scripts/run-test-suite.ts`, so a new one runs in `npm test` with no registration.
