 ...
 *    safe edit (the mistake layout-studio-fit-circle.test.ts made about
 *    `fitText`, and layout-studio-path.test.ts documents at length).
...
const BRANDS = (['lolly-start', 'suse'] as const).filter((b) =>
  existsSync(join(ROOT, 'brands', b, 'tools', 'layout-studio', 'tool.json')));
...
function fieldsOf(brand: string): any[] {
  const manifest = JSON.parse(readFileSync(
    join(ROOT, 'brands', brand, 'tools', 'layout-studio', 'tool.json'), 'utf8'));
...
test('strokeDashLen and strokeGapLen are APPENDED, in that order, immediately after blur', () => {
  assert.ok(BRANDS.length, 'no layout-studio fork is mounted');
...
test('absent or 0 fields leave the markup BYTE-IDENTICAL to the pre-change output', async () => {
  // This is the property that lets tests/fixtures/layout-studio-extras-baseline.json
