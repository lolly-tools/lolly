...
const fileRef = (o: Record<string, unknown>): any => ({ __file: true, url: null, ...o });
const pdfFile = (bytes: Uint8Array, name = 'report.pdf'): any =>
...
test('compress-pdf: manifest validates and is wired as a no-sidebar exportFile utility', () => {
  ...
  // the reserved 'dpi' id.
  const ids = manifest.inputs.map((i: any) => i.id);
