...
import type { InputFile, PdfAPI } from '../engine/src/bridge/host-v1.ts';
...
function compressTool(): LoadedTool {
  return {
    manifest: JSON.parse(readFileSync(join(ROOT, 'tools/compress-pdf/tool.json'), 'utf8')) as ToolManifest,
...
const fileRef = (over: Partial<InputFile> = {}): InputFile =>
  ({ __file: true, name: 'report.pdf', mime: 'application/pdf', size: 0, bytes: new Uint8Array(0), url: null, ...over });
const pdfFile = (bytes: Uint8Array, name = 'report.pdf'): InputFile =>
...
test('compress-pdf: exportFile returns a smaller, valid PDF with the page count preserved', async () => {
  ...
  const outBytes = bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes);
  assert.equal(mime, 'application/pdf');
  assert.equal(filename, 'report-compressed.pdf');
  assert.ok(outBytes.length <= pdf.length);
  assert.equal((await PDFDocument.load(outBytes, { updateMetadata: false })).getPageCount(), 40);
});
...
test('compress-pdf: degrades gracefully when the host has no compress capability', async () => {
  ...
  const legacyPdf: Pick<PdfAPI, 'analyze' | 'strip'> = {
    analyze: async () => ({ findings: [] }),
    strip: async (b) => ({ bytes: b }),
  };
  const legacy = { ...BARE_HOST, pdf: legacyPdf };
