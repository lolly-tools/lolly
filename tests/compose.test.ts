...
import type { AssetRef, ComposeSpec } from '../engine/src/bridge/host-v1.ts';

// A host tool that composes `qr-code`, binding the child's `url` to its own `url`.
const composeManifest = (over: Partial<ToolManifest> = {}): ToolManifest => ({
  id: 'host-tool',
  name: 'Host Tool',
  version: '1.0.0',
  engineVersion: '^1.0.0',
...
const composeTool = (over?: Partial<ToolManifest>): LoadedTool => ({
  manifest: composeManifest(over),
  template: '{{#if badgeQr}}<img src="{{asset badgeQr}}">{{else}}<span>no-qr</span>{{/if}}',
  styles: null,
  hooksSource: null, hooksUrl: null,
  textTemplates: {},
  textTemplateErrors: {},
});

const stubRef = (url = 'blob:STUB'): AssetRef => ({ source: 'remote', id: 'x', type: 'vector', format: 'svg', url });

// Host double whose compose.render is fully controllable + records calls.
function composeHost(render: (spec: ComposeSpec) => AssetRef | Promise<AssetRef>) {
  const calls: ComposeSpec[] = [];
  const warns: string[] = [];
  const host = {
    version: '1',
    profile: { get: async () => ({}) },
    log: (level: string, msg: string) => { warns.push(`${level}:${msg}`); },
    compose: { render: async (spec: ComposeSpec) => { calls.push(spec); return render(spec); } },
  };
...
test('compose: renders the child, binds {{url}}, threads the stack, exposes {{asset <id>}}', async () => {
  ...
  assert.equal(first.inputs.url, 'https://suse.com', '{{url}} hydrated against parent context');
  assert.equal(first.format, 'svg');
  assert.deepEqual([...(first._stack ?? [])], ['host-tool'], 'parent id pushed onto the recursion stack');
...
test('compose: an input value flows into the child render on change', async () => {
  const { host, calls } = composeHost((spec) => stubRef(`blob:${encodeURIComponent(String(spec.inputs.url))}`));
  const rt = await createRuntime(composeTool(), host, {});
...
test('compose: a child returning no url is treated as a miss (slot stays empty)', async () => {
  // A ref missing `url`, exactly as it would arrive from a buggy shell — a JSON
  // payload the engine must not trust, hence the parse rather than a typed literal.
  const { host } = composeHost(() => JSON.parse('{"source":"remote","id":"x","type":"vector","format":"svg"}'));
  const rt = await createRuntime(composeTool(), host, {});
  assert.match(rt.getHydrated(), /no-qr/);
});
...
test('resolveNestedRenders: returns { id: ref } and skips malformed entries', async () => {
  ...
  const model: InputModelItem[] = [
    { id: 'url', type: 'text', value: 'https://x', isDirty: false, control: 'text-input' },
  ];
...
const chainTool = (id: string, child: string | null): LoadedTool => ({
  manifest: composeManifest({
    id, name: id,
    render: { width: 10, height: 10, formats: ['svg'] }, inputs: [],
    ...(child
      ? { capabilities: ['compose'], composes: [{ id: 'child', tool: child }] }
...
function chainHost(tools: Record<string, ReturnType<typeof chainTool>>, MAX = 3) {
  const calls: { toolId: string; stack: string[] }[] = [];
  const rendered: string[] = [];
  const host = {
    version: '1', profile: { get: async () => ({}) }, log: () => {},
    compose: {
      async render(spec: ComposeSpec): Promise<AssetRef> {
        const { toolId, _stack = [] } = spec;
        ...
        // Chain tools declare no inputs, so the child mounts with an empty state.
        await createRuntime(t, host, {}, { composeStack: _stack }); // ancestor stack (M1)
...
test('compose: a success then a failing re-render CLEARS the slot (no stale embed)', async () => {
  ...
  const host = {
    version: '1', profile: { get: async () => ({}) }, log: () => {},
    compose: { render: async (spec: ComposeSpec) => { if (fail) throw new Error('boom'); return stubRef(`blob:${String(spec.inputs.url)}`); } },
  };
  const rt = await createRuntime(composeTool(), host, {});
...
test('compose: an out-of-order render from an older keystroke does not overwrite a newer value', async () => {
  ...
  const open = (k: string) => { const gate = gates[k]; assert.ok(gate, `no pending render for ${k}`); gate(); };
  const host = {
    version: '1', profile: { get: async () => ({}) }, log: () => {},
    compose: { render: (spec: ComposeSpec) => new Promise<AssetRef>((res) => { gates[String(spec.inputs.url)] = () => res(stubRef(`blob:${String(spec.inputs.url)}`)); }) },
  };
  const rtP = createRuntime(composeTool(), host, {});
  await new Promise((r) => setTimeout(r));
  open('https://suse.com'); // let the mount render finish
  const rt = await rtP;

  const a = rt.setInput('url', 'A');
  const b = rt.setInput('url', 'B'); // B is the newer value
  await new Promise((r) => setTimeout(r));
  open('B'); // newer resolves first…
  open('A'); // …older resolves last
