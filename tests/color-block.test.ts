...
// The tool's single blocks input — asserted once here so every later test can
// use it without re-guarding (the first test pins the manifest shape anyway).
const blocksInput = tool.manifest.inputs[0];
assert.ok(blocksInput, 'color-block declares at least one input');
const blockFields = blocksInput.fields ?? [];
  ...
  const host = {
    ...
    assets: {
      get: async (id: string): Promise<AssetRef> => {
        fetched.push(id);
        return { source: 'library', id, type: 'raster', format: 'png', url: 'asset:' + id };
      },
    },
    log: () => {},
  };
...
async function mount(initialState: Record<string, InputValue> = {}) {
...
const cellCount = (html: string) => (html.match(/class="cb-block /g) ?? []).length;
// Logo marks rendered, in order, by resolved asset URL.
const logoSrcs = (html: string) =>
  [...html.matchAll(/<img class="cb-logo-mark" src="([^"]*)"/g)].map(m => m[1]);
...
test('color-block: "logo" is a block kind that can be added like any other', () => {
  const kind = blockFields.find(f => f.id === 'kind');
  assert.ok(kind, 'blocks declare a kind discriminator');
  ...
  const fields = Object.fromEntries(blockFields.map(f => [f.id, f]));
  assert.deepEqual(fields.logoOrient?.showFor, ['logo']);
  assert.deepEqual(fields.logoColor?.showFor, ['logo']);
  const bgImageShowFor = fields.bgImage?.showFor;
  assert.ok(bgImageShowFor, 'bgImage is scoped to specific kinds');
  assert.ok(!bgImageShowFor.includes('logo'));
});
...
test('color-block: renders exactly one grid cell per block', async () => {
  ...
  const defaults = blocksInput.default;
  assert.ok(Array.isArray(defaults), 'blocks input ships a default list');
  const { html } = await mount();                       // ships the manifest default blocks
  assert.equal(cellCount(html), defaults.length);
