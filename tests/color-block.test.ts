...
async function mount(initialState: any = {}) {
  ...
  return { rt, fetched, html: rt.getHydrated() as string };
}
...
const logoSrcs = (html: string) => [...html.matchAll(/<img class="cb-logo-mark" src="([^"]*)"/g)].map(m => m[1]);
...
test('color-block: the only top-level content input is the block list (no corner-logo input)', () => {
  ...
  // a separate top-level input.
  assert.deepEqual(ids, ['blocks', 'width', 'height']);
...
test('color-block: "logo" is a block kind that can be added like any other', () => {
  ...
  const fields = Object.fromEntries(tool.manifest.inputs[0].fields.map((f: any) => [f.id, f]));
...
const LOGO_MATRIX: Array<[string, string, string, string]> = [
