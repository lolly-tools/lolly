...
test('the real multi-page-pdf manifest warns on landscape but not portrait', () => {
  const manifest = JSON.parse(readFileSync(join(ROOT, 'tools/multi-page-pdf/tool.json'), 'utf8')) as ToolManifest;
...
test('the real event-name-badge manifest wires its size select to export dims', () => {
  const manifest = JSON.parse(readFileSync(join(ROOT, 'tools/event-name-badge/tool.json'), 'utf8')) as ToolManifest;
  ...
  const sizeInput = manifest.inputs.find(i => i.id === 'size');
  assert.equal(sizeInput?.options?.every(o => d?.dims[o.value]), true);
});
