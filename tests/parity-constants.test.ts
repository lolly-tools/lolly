 ...
 * (tools/voice-recorder/hooks.js) recomputes the same verdict DOM-free — the hook ships
 * as tool DATA and cannot import from the shell, so it keeps its own copies. The decision
 * was to keep that duplication and guard it here, so re-tuning one file without the other
 * fails this test.
 *
...
test('audio-coaching BANDS match between shell core and voice-recorder hook', () => {
  for (const row of ['soft', 'normal', 'loud']) {
    assert.deepEqual(
      band(hook, row),
      band(core, row),
      `BANDS.${row} drifted between voice-recorder/hooks.js and audio-coach-core.ts`,
    );
  }
});
...
test('audio-coaching room/speech thresholds are mirrored in voice-recorder hook', () => {
  for (const name of ['NOISY_FLOOR_DBFS', 'HUM_RATIO', 'HISS_FLATNESS', 'SPEAKING_SNR_DB']) {
    ...
    assert.ok(
      ...
      `${name} (${value}) from audio-coach-core.ts is not mirrored in voice-recorder/hooks.js`,
    );
  }
});
