 ...
 *
 * Run with: node --test tests/tool-url-asset.test.js
 */
...
test('asset: a tool-URL id re-renders via host.compose.renderUrl, not the catalog', async () => {
  ...
  const host = {
    ...
    profile: { get: async () => ({}) },
    log: () => {},
    assets: { get: async (id) => { throw new Error('catalog lookup must not run for a tool URL: ' + id); } },
    compose: {
      render: async () => {},
      renderUrl: async (url, opts) => {
        calls.push({ url, opts });
...
test('asset: a plain library id still resolves through host.assets.get', async () => {
  ...
  const host = {
    ...
    profile: { get: async () => ({}) },
    log: () => {},
    assets: { get: async (id) => { got = id; return { source: 'library', id, type: 'vector', format: 'svg', url: 'blob:LIB' }; } },
