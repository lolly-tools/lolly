 ...
 * Run with: node --experimental-strip-types --test tests/css-box.test.ts
 */
...
import type { CornerInputs, CornerRadii, CornerPair } from '../engine/src/css-box.ts';
...
const CORNER_KEYS: (keyof CornerRadii)[] = ['topLeft', 'topRight', 'bottomRight', 'bottomLeft'];
// uniformRadius() narrowed to non-null for the tests that expect a pair.
const uniform = (r: CornerRadii): CornerPair => {
  const u = uniformRadius(r);
  assert.ok(u, 'expected a uniform radius pair');
  return u;
};
...
test('cornerRadii: huge uniform radius collapses to a pill (rx==ry==min(w,h)/2)', () => {
  ...
  assert.deepEqual(uniform(r).map(Math.round), [40, 40]);
});
...
test('cornerRadii: 50% stays an ellipse on a non-square box, a circle on a square one', () => {
  ...
  assert.deepEqual(uniform(ell).map(Math.round), [150, 40]);

  const circ = cornerRadii(corners('50%'), 200, 200);
  assert.deepEqual(uniform(circ).map(Math.round), [100, 100]);
});
...
test('insetCorners: shrinks by inset, clamped to 0', () => {
  const r: CornerRadii = { topLeft: [10, 10], topRight: [4, 4], bottomRight: [0, 0], bottomLeft: [10, 10] };
...
test('parseBoxShadow: single shadow (Chrome computed form, color first)', () => {
  ...
  const first = s[0];
  assert.ok(first);
  assert.deepEqual({ x: first.x, y: first.y, blur: first.blur, spread: first.spread }, { x: 0, y: 32, blur: 80, spread: 0 });
  assert.equal(first.color, 'rgba(0, 0, 0, 0.55)');
});
...
test('parseBoxShadow: blur/spread optional; negative spread kept; blur clamped ≥0', () => {
  ...
  const first = s[0];
  assert.ok(first);
  assert.deepEqual({ blur: first.blur, spread: first.spread }, { blur: 0, spread: 0 });
  const sp = parseBoxShadow('rgb(0,0,0) 0px 2px 6px -2px');
  const spFirst = sp[0];
  assert.ok(spFirst);
  assert.equal(spFirst.spread, -2);
});
