...
import {
  ...
  packEncrypted, unpackEncrypted, hasEncryptedState, isEncryptAvailable, ENC_PARAM,
...
test('packEncrypted/unpackEncrypted round-trips with the right password', async () => {
  ...
  const token = await packEncrypted(BIG_QUERY, pw);
  assert.ok(token, 'should mint a zx token');
  assert.equal(token![0], '1', 'tag byte versions the pbkdf2/aes-gcm codec');
  assert.match(token!, /^[A-Za-z0-9_-]+$/, 'URL-safe (no % escaping)');
  assert.equal(await unpackEncrypted(token!, pw), BIG_QUERY, 'decrypts back byte-for-byte');
});

test('unpackEncrypted returns null for a wrong/empty password (never fabricated state)', async () => {
  const token = await packEncrypted(BIG_QUERY, 'the-real-password');
  assert.equal(await unpackEncrypted(token!, 'wrong-password'), null);
  assert.equal(await unpackEncrypted(token!, ''), null);
});

test('unpackEncrypted returns null for a tampered token', async () => {
  const token = await packEncrypted('color=30BA78&format=pdf', 'pw12345');
  const i = token!.length - 5; // deep in the ciphertext/tag region
  const mangled = token!.slice(0, i) + (token![i] === 'A' ? 'B' : 'A') + token!.slice(i + 1);
  assert.equal(await unpackEncrypted(mangled, 'pw12345'), null);
});

test('encrypted tokens are salted: same input+password → different tokens, both decrypt', async () => {
  const a = await packEncrypted(BIG_QUERY, 'pw');
  const b = await packEncrypted(BIG_QUERY, 'pw');
  assert.notEqual(a, b, 'random salt + iv make each token unique');
  assert.equal(await unpackEncrypted(a!, 'pw'), await unpackEncrypted(b!, 'pw'));
});

test('the password never appears in the token; zx is not expanded by expandQuery', async () => {
  const pw = 'sup3r-secret-passphrase';
  const token = await packEncrypted('name=Andy&format=pdf', pw);
...
test('packEncrypted returns null without a password', async () => {
  assert.equal(await packEncrypted(BIG_QUERY, ''), null);
});
...
test('round-trip preserves compact block delimiters and %-encoded values exactly', async () => {
  ...
  const boxes = state.values.boxes as any[];
  assert.equal(boxes.length, 24);
  assert.equal(boxes[0].id, 'b0');
