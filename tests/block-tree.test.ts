...
/** Fetches an array element, throwing instead of silently yielding undefined. */
...
test('nestingActive gates on activeWhen with array membership', () => {
  const input = { id: 'x', type: 'blocks' as const, nesting: { parentField: 'parent', activeWhen: { diagramType: ['org', 'mindmap'] } } };
  ...
  assert.equal(nestingActive({ id: 'x', type: 'blocks' as const, nesting: { parentField: 'parent' } }, {}), true);   // no activeWhen ⇒ on
  assert.equal(nestingActive({ id: 'x', type: 'blocks' as const }, {}), false);               // no nesting ⇒ off
});
...
test('reparent INSIDE makes the dragged node a child of the target', () => {
  ...
  const cfo = at(out, keys.indexOf('cfo'));
...
test('reparent BEFORE makes the dragged node a preceding sibling of the target', () => {
  ...
  assert.ok(out);
  const keys = deriveBlockKeys(out, CFG);
  const qa = at(out, keys.indexOf('qa'));
...
test('reparent moves the whole subtree, not just the node', () => {
  ...
  assert.ok(out);
  const keys = deriveBlockKeys(out, CFG);
  assert.equal(slugRef(at(out, keys.indexOf('cto')).parent), 'cfo');
  const order = blockTreeOrder(out, blockParentIndex(out, keys, 'parent')).map(e => keys[e.idx]);
...
test('buildRefOptions excludes self and descendants for a parent picker', () => {
  ...
  const ceoOption = options.find(o => o.value === 'ceo');
  assert.ok(ceoOption);
  assert.equal(ceoOption.label, 'CEO');
});
...
test('drag-to-nest onto a blank-id card anchors the reference durably (no drift)', () => {
  ...
  assert.ok(slugRef(at(out, targetPos).nodeId), 'target nodeId was materialised');
});
...
test('reparent keeps duplicate-label references unambiguous after reorder', () => {
  ...
  assert.ok(slugRef(at(out, c1Pos).nodeId), 'first Lead got a durable id');
});
...
test('materializeRefTarget writes a durable key onto a referenced blank row', () => {
  ...
  assert.equal(at(out, 0).nodeId, 'eng-lead');           // materialised
  assert.equal(at(out, 1).nodeId, 'qa');                 // untouched
