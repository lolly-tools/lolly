...
import type { TokenSet, TokenEntry } from '../engine/src/bridge/host-v1.ts';
...
const entry = (ts: TokenSet, path: string): TokenEntry => {
  const e = ts.get(path);
  assert.ok(e, `missing token ${path}`);
  return e;
};

test('flattens groups, inherits $type, keeps dotted paths', () => {
  const ts = createTokenSet(BRAND);
  assert.equal(entry(ts, 'color.brand.jungle').type, 'color'); // inherited from group $type
  assert.equal(entry(ts, 'color.brand.jungle').value, '#30ba78');
  assert.equal(entry(ts, 'space.sm').type, 'dimension');       // explicit on the token
...
test('query by type and colour swatches', () => {
  ...
  assert.ok(jungle, 'missing jungle swatch');
  ...
  assert.deepEqual(jungle.cmyk, [70, 0, 65, 0]);     // CMYK rides in $extensions
