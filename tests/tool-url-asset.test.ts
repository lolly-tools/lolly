...
import type { AssetRef, ComposeUrlOpts } from '../engine/src/bridge/host-v1.ts';
...
// A compose.render that must never run — these tests only exercise renderUrl.
const unusedRender = async (): Promise<AssetRef> => { throw new Error('compose.render must not run'); };
...
test('asset: a tool-URL id re-renders via host.compose.renderUrl, not the catalog', async () => {
  const calls: { url: string; opts: ComposeUrlOpts }[] = [];
  const host = {
    version: '1',
    profile: { get: async () => ({}) },
    log: () => {},
    ...
    compose: {
      render: unusedRender,
      renderUrl: async (url: string, opts: ComposeUrlOpts): Promise<AssetRef> => {
        calls.push({ url, opts });
  ...
  const call = calls[0];
  assert.ok(call);
  assert.equal(call.url, TOOL_URL);
  ...
  assert.ok(call.opts._stack);
  assert.deepEqual([...call.opts._stack], ['doc'], 'current tool id pushed onto the recursion stack');
...
test('asset: a plain library id still resolves through host.assets.get', async () => {
  ...
  const host = {
    ...
    profile: { get: async () => ({}) },
    log: () => {},
    assets: { get: async (id: string): Promise<AssetRef> => { got = id; return { source: 'library', id, type: 'vector', format: 'svg', url: 'blob:LIB' }; } },
    compose: { render: unusedRender, renderUrl: async () => { throw new Error('renderUrl must not run for a library id'); } },
  };
