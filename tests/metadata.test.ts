 ...
 * Run with: node --experimental-strip-types --test tests/metadata.test.ts
 */
import { test } from 'node:test';
import assert from 'node:assert/strict';

import { buildExportMeta, type MetadataHost } from '../engine/src/metadata.ts';
import type { Profile } from '../engine/src/bridge/host-v1.ts';

const hostWith = (profile: Profile): MetadataHost => ({ profile: { get: async () => profile } });
...
test('buildExportMeta: missing/throwing profile is tolerated', async () => {
  const m1 = await buildExportMeta({}, { id: 'x' });                       // no profile API
  assert.equal(m1.author, '');
  assert.equal(m1.tool, 'x');
  const m2 = await buildExportMeta({ profile: { get: async () => { throw new Error('nope'); } } }, { name: 'T' });
