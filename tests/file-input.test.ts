 ...
 *
 * Run with: node --test tests/file-input.test.ts
 */
...
test('file input: default value is null, control is file-picker', () => {
  const [m] = buildInputModel(FILE_MANIFEST) as any;
...
test('file input: a loaded FileRef in initial state is kept; an unresolved/junk ref is dropped to null', () => {
  const loaded = (buildInputModel(FILE_MANIFEST, { initial: { photo: fileRef() } }) as any)[0];
  ...
  const unresolved = (buildInputModel(FILE_MANIFEST, { initial: { photo: { __file: true, path: './x.jpg', _unresolved: true } } }) as any)[0];
  assert.equal(unresolved.value, null);

  // A stray string → blank.
  const stray = (buildInputModel(FILE_MANIFEST, { initial: { photo: 'whatever' } }) as any)[0];
...
test('file input: constrain accepts a FileRef or null, rejects anything else', () => {
  ...
  const replaced = updateInput(model, 'photo', fileRef({ name: 'b.png' })) as any;
...
function toolWith({ manifest, hooksSource, template = '' }: { manifest: any; hooksSource?: string; template?: string }): any {
...
test('runtime.exportFile: runs the exportFile hook and returns its bytes; hasExportFile reflects the manifest', async () => {
  ...
  const res = await rt.exportFile() as any;
...
test('runtime.export: on-device tools embed NO provenance metadata and never watermark', async () => {
  ...
  const host: any = {
    ...BARE_HOST,
    export: { render: async (_node: any, _fmt: any, opts: any) => { captured = opts; return new Blob(['x']); } },
  };
  const onDevice: any = {
    manifest: {
...
function buildTextPng(): Uint8Array {
...
test('strip-data: strips APP1/EXIF from a JPEG losslessly (keeps APP0 + scan data)', async () => {
  ...
  const { bytes, filename, mime } = await rt.exportFile() as any;
  ...
  const hasExif = (b: any) => { for (let i = 0; i < b.length - 4; i++) if (b[i] === 0x45 && b[i + 1] === 0x78 && b[i + 2] === 0x69 && b[i + 3] === 0x66) return true; return false; };
...
test('strip-data: removes tEXt chunks from a PNG, keeps IHDR/IDAT/IEND', async () => {
  ...
  const { bytes, filename } = await rt.exportFile() as any;
  assert.equal(filename, 'art-clean.png');
  const typeAt = (b: any, type: string) => { for (let i = 8; i < b.length - 8; i++) { if (String.fromCharCode(b[i + 4], b[i + 5], b[i + 6], b[i + 7]) === type) return true; } return false; };
...
const svgFile = (text: string, name = 'logo.svg'): any => {
...
test('strip-data (svg): a non-SVG file is reported as such and handed back untouched', async () => {
  ...
  const { bytes } = await rt.exportFile() as any;
...
async function buildMetaPdf(): Promise<Uint8Array> {
...
const pdfFile = (bytes: Uint8Array, name = 'plan.pdf'): any =>
