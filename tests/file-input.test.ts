 ...
 *
 * Run with: node --experimental-strip-types --test tests/file-input.test.ts
 */
...
import { buildInputModel, updateInput, isFileValue } from '../engine/src/inputs.ts';
import type { InputManifest } from '../engine/src/inputs.ts';
import type { LoadedTool, ToolManifest } from '../engine/src/loader.ts';
import type { InputFile, ExportOpts } from '../engine/src/bridge/host-v1.ts';
...
const FILE_MANIFEST: InputManifest = { inputs: [{ id: 'photo', type: 'file', accept: ['image/jpeg'] }] };
const fileRef = (over: Partial<InputFile> = {}): InputFile => ({ __file: true, name: 'a.jpg', mime: 'image/jpeg', size: 3, bytes: new Uint8Array([1, 2, 3]), url: null, ...over });
...
test('file input: a loaded FileRef in initial state is kept; an unresolved/junk ref is dropped to null', () => {
  ...
  assert.ok(loaded && isFileValue(loaded.value));
...
test('file input: constrain accepts a FileRef or null, rejects anything else', () => {
  ...
  const replaced = updateInput(model, 'photo', fileRef({ name: 'b.png' }))[0];
  assert.ok(replaced && isFileValue(replaced.value));
  assert.equal(replaced.value.name, 'b.png');
  // null clears it.
  assert.equal(updateInput(model, 'photo', null)[0]?.value, null);
  // A garbage string is ignored — the prior value stays.
  const kept = updateInput(model, 'photo', 'nope')[0];
  assert.ok(kept && isFileValue(kept.value));
  assert.equal(kept.value.name, 'a.jpg');
});
...
test('url-mode: a file param parses to an unresolved path-ref (CLI loads the bytes)', () => {
  ...
  // url-mode.ts is not ported yet and infers as `{}` — read via typed entries.
  const byId = new Map<string, unknown>(Object.entries(values));
  assert.deepEqual(byId.get('photo'), { __file: true, path: './pic.jpg', _unresolved: true });
});
...
function toolWith({ manifest, hooksSource, template = '' }: { manifest: ToolManifest; hooksSource?: string; template?: string }): LoadedTool {
  return { manifest, hooksSource: hooksSource ?? null, hooksUrl: null, template, styles: null, textTemplates: {}, textTemplateErrors: {} };
}

// Narrow the exportFile transform result to the Uint8Array every hook under
// test actually returns, so byte-level assertions can index it.
function asBytes(b: Uint8Array | ArrayBuffer): Uint8Array {
  assert.ok(b instanceof Uint8Array, 'hook returned raw Uint8Array bytes');
  return b;
}
...
test('runtime.exportFile: runs the exportFile hook and returns its bytes; hasExportFile reflects the manifest', async () => {
  ...
  assert.deepEqual(Array.from(asBytes(res.bytes)), [2, 3, 4]);
...
test('runtime.export: on-device tools embed NO provenance metadata and never watermark', async () => {
  const captured: { opts: ExportOpts | null } = { opts: null };
  const host = {
    ...BARE_HOST,
    export: { render: async (_node: unknown, _fmt: string, opts: ExportOpts) => { captured.opts = opts; return new Blob(['x']); } },
  };
  const onDevice: LoadedTool = {
    manifest: {
    ...
    template: '<div></div>',
    styles: null, hooksSource: null, hooksUrl: null, textTemplates: {}, textTemplateErrors: {},
  };
  const rt = await createRuntime(onDevice, host, {});
  await rt.export({}, 'png', {});
  assert.ok(captured.opts);
  assert.equal(captured.opts.meta, undefined, 'on-device output must carry no provenance metadata');
  assert.ok(!captured.opts.watermark, 'on-device output must never be watermarked');

  // A normal (non-on-device) tool DOES get provenance assembled.
  const normal: LoadedTool = {
    manifest: {
  ...
  assert.ok(captured.opts?.meta && typeof captured.opts.meta === 'object', 'normal tools still embed provenance');
});
...
function stripDataTool(): LoadedTool {
  return {
    ...
    styles: null, textTemplates: {}, textTemplateErrors: {},
  };
}
...
test('strip-data: strips APP1/EXIF from a JPEG losslessly (keeps APP0 + scan data)', async () => {
  ...
  const { bytes: rawBytes, filename, mime } = await rt.exportFile();
  ...
  const hasExif = (b: Uint8Array) => { for (let i = 0; i < b.length - 4; i++) if (b[i] === 0x45 && b[i + 1] === 0x78 && b[i + 2] === 0x69 && b[i + 3] === 0x66) return true; return false; };
...
test('strip-data: removes tEXt chunks from a PNG, keeps IHDR/IDAT/IEND', async () => {
  ...
  const { bytes: rawBytes, filename } = await rt.exportFile();
  const bytes = asBytes(rawBytes);
  assert.equal(filename, 'art-clean.png');
  const typeAt = (b: Uint8Array, type: string) => { for (let i = 8; i < b.length - 8; i++) { if (String.fromCharCode(b[i + 4] ?? 0, b[i + 5] ?? 0, b[i + 6] ?? 0, b[i + 7] ?? 0) === type) return true; } return false; };
...
const svgFile = (text: string, name = 'logo.svg') => {
...
test('strip-data (svg): a non-SVG file is reported as such and handed back untouched', async () => {
  ...
  assert.deepEqual(Array.from(asBytes(bytes)), Array.from(original.bytes)); // byte-for-byte passthrough
});
...
const pdfFile = (bytes: Uint8Array, name = 'plan.pdf') =>
