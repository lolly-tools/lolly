...
function makePng({ width = 2, height = 1, pixel = [255, 0, 0, 255], idatSplit = 1, extraChunks = [] }: MakePngOptions = {}): Uint8Array {
...
const text = (s: string): Uint8Array => Uint8Array.from(Buffer.from(s, 'latin1'));
...
test('garbage input throws', () => {
  ...
  assert.throws(() => packApng(['nope'] as unknown as Uint8Array[]), /not a Uint8Array/);
