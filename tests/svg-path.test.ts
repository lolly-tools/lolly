 ...
 * Run with: node --test tests/svg-path.test.ts
 */
...
test('closed triangle → one closed subpath of M,L,L', () => {
  ...
  assert.deepEqual(sp[0]!.segments.map((s: any) => s.op), ['M', 'L', 'L']);
...
test('relative commands resolve to absolute', () => {
  ...
  assert.deepEqual(sub!.segments.map((s: any) => [s.x, s.y]), [[10, 10], [15, 10], [15, 15]]);
});
...
test('quadratic Q is normalized to cubic', () => {
  ...
  const seg: any = sub!.segments[1];
  assert.equal(seg.op, 'C');
...
test('multiple subpaths (holed glyph) yield multiple entries', () => {
  ...
  assert.ok(sp.every((s: any) => s.closed));
});
...
test('arc A decomposes into cubic segments', () => {
  ...
  assert.ok(sub!.segments.slice(1).every((s: any) => s.op === 'C'));
  // endpoint of the arc is reached
  const last: any = sub!.segments[sub!.segments.length - 1];
...
// ("A5 5 0 0110 0" = large-arc 0, sweep 1, x 10). The generic number tokenizer
// read "0110" as 110, so the arc had too few args and was silently dropped.
// The compact form must parse identically to the fully-spaced form.
