...
import { buildC2paManifest, embedC2paInPdf, generateSigner, encodeCbor, CborTag } from '../engine/src/c2pa.ts';

const te = new TextEncoder();
const td = new TextDecoder();
const hex = (b: any): string => Array.from(b as ArrayLike<number>, (x) => x.toString(16).padStart(2, '0')).join('');
const sha256 = async (b: any): Promise<Uint8Array> => new Uint8Array(await crypto.subtle.digest('SHA-256', b));
...
const binOf = (bytes: any): string => Array.from(bytes as ArrayLike<number>, (b) => String.fromCharCode(b)).join('');
const bytesOf = (s: string): Uint8Array => Uint8Array.from(s, (c) => c.charCodeAt(0) & 0xff);

// ─── independent CBOR decoder (definite lengths only) ─────────────────────────

function decodeItem(b: any, i: number): [any, number] {
  const ib = b[i++];
  const major = ib >> 5;
  let n = ib & 0x1f;
  if (n === 24) { n = b[i]; i += 1; }
...
function decodeCbor(bytes: any): any {
...
function parseSuperbox(bytes: any, box: any): any {
...
const uuidOf = (fourcc: string): string => hex(te.encode(fourcc)) + JUMBF_SUFFIX;

function storeParts(store: any): any {
  ...
  const [assertionStore, claim, signature] = manifest.children.map((c: any) => parseSuperbox(store, c));
  const [actions, hashData] = assertionStore.children.map((c: any) => parseSuperbox(store, c));
  const contentOf = (sub: any) => store.slice(sub.children[0].payloadStart, sub.children[0].end);
  return { store: s, manifest, assertionStore, claim, signature, actions, hashData, contentOf };
}

// ─── independent DER walker ───────────────────────────────────────────────────

function derTlv(b: any, i: number): any {
  const tag = b[i];
  let len = b[i + 1];
  let j = i + 2;
  if (len & 0x80) {
...
function derChildren(b: any, tlv: any): any[] {
  const kids: any[] = [];
...
async function verifyCose(store: any): Promise<any> {
  ...
  const bstrHead = (n: number): Uint8Array =>
  ...
  assert.equal(
    await crypto.subtle.verify({ name: 'ECDSA', hash: 'SHA-256' }, key, sigRaw, sigStructure as BufferSource),
...
function buildTestPdf({ withNames = false }: { withNames?: boolean } = {}): { bytes: Uint8Array; startxref: number } {
  let out = '%PDF-1.4\n%âãÏÓ\n';
...
test('CBOR encoder matches hand-computed byte vectors', () => {
  const eq = (v: unknown, expected: string) => assert.equal(hex(encodeCbor(v)), expected);
  ...
  eq(new Map(), 'a0');
  eq(new Map<number, number | Uint8Array>([[1, 2], [3, new Uint8Array([0xaa, 0xbb])]]), 'a201020342aabb');
  eq({ a: 1 }, 'a1616101');
  // tags + simple values
...
test('self-signed certificate DER parses structurally', async () => {
  const cert = (await generateSigner(DATES)).certDer!;
...
test('manifest length is deterministic across differing hash bytes', async () => {
  ...
  const mk = (fill: number) => buildC2paManifest({
