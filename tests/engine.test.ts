 ...
 *
 * Run with: node --test tests/engine.test.ts
...
// Narrow an InputValue (or unknown) to a plain string-keyed record for
// sub-field assertions — the shape blocks rows and asset refs take.
function isValueRecord(v: unknown): v is Record<string, InputValue | undefined> {
...
const SAMPLE_MANIFEST: InputManifest = {
  inputs: [
...
test('url-mode: RESERVED set matches the documented reserved-param list', () => {
  // Mirrors the header doc-comment in engine/src/url-mode.ts AND the table in
...
test('url-mode: compact blocks fold a raw comma into the last field (no shift)', () => {
  ...
  const manifest: InputManifest = { inputs: [{ id: 'rows', type: 'blocks', fields: [
  ...
  const row = rows[0];
  assert.ok(isValueRecord(row));
  assert.equal(row.a, 'x');
  assert.equal(row.b, 'y,z'); // overflow folded into the last field
});
...
test('inputs: number constraints clamp to min/max on update', () => {
  const manifest: InputManifest = { inputs: [{ id: 'opacity', type: 'number', min: 0, max: 100 }] };
...
test('inputs: text maxLength truncates on update', () => {
  const manifest: InputManifest = { inputs: [{ id: 'name', type: 'text', maxLength: 5 }] };
  let model = buildInputModel(manifest);
...
test('inputs: modelToValues produces a flat object for template hydration', () => {
  const model: InputModelItem[] = [
    { id: 'a', type: 'text', value: 'one', isDirty: false, control: 'text-input' },
    { id: 'b', type: 'number', value: 2, isDirty: false, control: 'text-input' },
  ];
...
test('template: data-format helpers (icsStamp, rfcText, csvCell) with raw hydration', () => {
  const raw = (src: string, values: Record<string, unknown>) => hydrate(src, values, { raw: true });
...
const TYPED_BLOCKS_MANIFEST: InputManifest = {
  inputs: [
    {
      id: 'blocks',
      type: 'blocks',
...
test('validate: accepts typed blocks (addMenu, select/asset/number/showFor)', () => {
  const manifest = {
    ...
    inputs: [
      {
        ...TYPED_BLOCKS_MANIFEST.inputs?.[0],
...
test('runtime: resolves asset sub-fields inside blocks (CLI/URL parity)', async () => {
  ...
  const host = {
    ...
    profile: { get: async () => ({}) },
    assets: {
      get: async (id: string): Promise<AssetRef> => {
        fetched.push(id);
        return { source: 'library', id, type: 'raster', format: 'png', url: 'blob:' + id };
      },
    },
    log: () => {},
  };
  ...
  const blocksInput = rt.getModel().find(i => i.id === 'blocks');
  assert.ok(blocksInput);
  const val = blocksInput.value;
  assert.ok(Array.isArray(val));
  const [first, second] = val;
  assert.ok(isValueRecord(first));
  const firstImg = first.img;
  assert.ok(isValueRecord(firstImg));
  assert.equal(firstImg.url, 'blob:suse/logo/primary');      // resolved
  assert.ok(isValueRecord(second));
  assert.equal(second.img, null);                            // left alone
