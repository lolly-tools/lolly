 ...
 * Run with: node --experimental-strip-types --test tests/color.test.ts
 */
...
const byte = (buf: Uint8Array, off: number): number => {
  const b = buf[off];
  assert.ok(b !== undefined, `read past end of buffer at ${off}`);
  return b;
};
const sig = (buf: Uint8Array, off: number): string =>
  String.fromCharCode(byte(buf, off), byte(buf, off + 1), byte(buf, off + 2), byte(buf, off + 3));
const u32 = (buf: Uint8Array, off: number): number =>
  ((byte(buf, off) << 24) | (byte(buf, off + 1) << 16) | (byte(buf, off + 2) << 8) | byte(buf, off + 3)) >>> 0;
...
test('srgbIccProfile: required tags are present and in-bounds', () => {
  ...
  const tag = (name: string): { offset: number; size: number } => {
    const e = tags.get(name);
    assert.ok(e, `missing tag ${name}`);
    return e;
  };
  for (const t of ['desc', 'wtpt', 'rXYZ', 'gXYZ', 'bXYZ', 'rTRC', 'gTRC', 'bTRC', 'cprt']) {
    const { offset, size } = tag(t);
  ...
  assert.equal(tag('rTRC').offset, tag('gTRC').offset);
  assert.equal(tag('gTRC').offset, tag('bTRC').offset);
});
