...
async function bundleFiles(blob: Blob): Promise<Record<string, Uint8Array>> {
  return unzipSync(new Uint8Array(await blob.arrayBuffer()));
}
function reseal(files: Record<string, Uint8Array>): Uint8Array {
...
function makeHost(): any {
  const profileBox: { value: any } = { value: null };
  const sessions = new Map<string, any>();   // slot → full record
  const assets = new Map<string, any>();     // id → record (with Blob)
  return {
    _peek: { profileBox, sessions, assets },
    profile: {
      async get() { return profileBox.value ?? {}; },
      async set(p: any) { profileBox.value = p; },
      bust() {},
    },
    state: {
      async list() {
        return [...sessions.values()].map((r: any) => ({
      ...
      async save(slot: string, data: any, thumb: any = null) {
        sessions.set(slot, {
    ...
    assets: {
      ...
      async _importUserAsset(rec: any) { assets.set(rec.id, rec); },
    },
  };
}

function makeStorage(seed: Record<string, string> = {}): any {
  const m = new Map<string, string>(Object.entries(seed));
  return {
    getItem: (k: string) => (m.has(k) ? m.get(k) : null),
    setItem: (k: string, v: any) => m.set(k, String(v)),
...
test('export → import reproduces all user data on a fresh device', async () => {
  ...
  const pic = restored.find((a: any) => a.id === 'user/upload/1700-pic.jpg');
...
test('import isolates a failed asset restore: counts it, keeps going, reports honestly', async () => {
  const src = await seedSource();
  ...
  const dst = { host: makeHost(), storage: makeStorage() };
  let n = 0;
  dst.host.assets._importUserAsset = async (rec: any) => {
    if (++n === 2) throw Object.assign(new Error('QuotaExceededError'), { code: 'STORAGE_FULL' });
    dst.host._peek.assets.set(rec.id, rec);
  };

  const isum = await importBackup({ host: dst.host, storage: dst.storage }, new Uint8Array(await blob.arrayBuffer()));
  assert.equal(isum.userAssets, 1, 'the surviving image is restored');
  assert.equal(isum.failedAssets, 1, 'the failed image is counted, not hidden');
  assert.equal(isum.skipped, 0, 'a failed restore is NOT conflated with newer-writer parts');
  assert.equal(dst.host._peek.assets.size, 1);
});
...
test('manifest declares the stable format id, reader version, and per-part integrity', async () => {
  ...
  assert.ok(Object.keys(manifest.integrity).some((k: string) => k.startsWith('assets/blobs/')));
...
test('rejects a bundle missing a part the manifest vouches for', async () => {
  ...
  const { blob } = await exportBackup({ host: src.host, storage: src.storage });
  const files = await bundleFiles(blob);
  const blobPart = Object.keys(files).find((p: string) => p.startsWith('assets/blobs/'));
