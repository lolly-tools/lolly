...
const box = (o: Partial<Box> = {}): Box => ({ x: 0, y: 0, w: 100, h: 100, rot: 0, ...o });
const near = (a: number, b: number, eps = 1e-6): void => assert.ok(Math.abs(a - b) <= eps, `${a} ≈ ${b}`);

/** Safely index an array under noUncheckedIndexedAccess, throwing if the slot is empty. */
function at<T>(arr: T[], i: number): T {
  const v = arr[i];
  if (v === undefined) throw new Error(`expected index ${i} to be present`);
  return v;
}
...
test('resizeRect on a ROTATED box preserves the fixed world corner', () => {
  ...
  const before = at(boxCorners(box(start), CFG), 0);
  ...
  const after = at(boxCorners({ x: r.x, y: r.y, w: r.w, h: r.h, rot: start.rot }, CFG), 0);
...
test('moveBoxes shifts only the selected indices', () => {
  ...
  assert.equal(at(next, 0).x, 0);
  assert.equal(at(next, 1).x, 15);
  assert.equal(at(next, 1).y, -3);
...
test('alignBoxes single box aligns to the artboard edges', () => {
  ...
  assert.equal(at(alignBoxes(boxes, [0], 'left', CFG, canvas), 0).x, 0);
  assert.equal(at(alignBoxes(boxes, [0], 'right', CFG, canvas), 0).x, 900);
  assert.equal(at(alignBoxes(boxes, [0], 'hcentre', CFG, canvas), 0).x, 450);
});
...
test('alignBoxes multi aligns to the selection bbox', () => {
  ...
  assert.equal(at(out, 0).x, 0);
  assert.equal(at(out, 1).x, 0); // both to the selection's left edge (minX = 0)
});
...
test('distributeBoxes equalises gaps along the horizontal axis', () => {
  ...
  assert.equal(at(out, 0).x, 0);   // extremes fixed
  assert.equal(at(out, 2).x, 500);
  assert.equal(at(out, 1).x, 250); // (span 550 - sizes 150)/2 = 200 gap → 0+50+200
});
...
test('reorderZ front/back/forward/backward', () => {
  const ids = (arr: Box[]): string => arr.map((b) => String(b.id)).join('');
...
test('seedBox merges defaults + kind seed + rect + id', () => {
  const b = seedBox(CFG, { bg: '#fff', rot: 0 }, { bg: '#30BA78', text: 'Hi' }, { x: 5.4, y: 6.6, w: 100, h: 50, rot: 0 }, 'z1');
...
test('snapMove snaps a near edge to a sibling edge and emits a guide', () => {
  const active = { minX: 203, minY: 100, maxX: 303, maxY: 200, w: 100, h: 100 };   // left edge 3px off 200
  const others = [{ minX: 200, minY: 400, maxX: 300, maxY: 500, w: 100, h: 100 }]; // sibling left at 200
...
test('snapMove snaps centre to the artboard centre', () => {
  const active = { minX: 402, minY: 402, maxX: 602, maxY: 602, w: 200, h: 200 };   // centre (502,502)
...
test('snapMove ignores targets beyond the threshold', () => {
  const active = { minX: 220, minY: 220, maxX: 320, maxY: 320, w: 100, h: 100 };
  const s = snapMove(active, [{ minX: 200, minY: 200, maxX: 300, maxY: 300, w: 100, h: 100 }], { w: 1000, h: 1000 }, 6);
  assert.equal(s.dx, 0);
  assert.equal(s.dy, 0);
...
test('snapPoint snaps a pointer to a sibling right edge', () => {
  const s = snapPoint(298, 55, [{ minX: 100, minY: 40, maxX: 300, maxY: 90, w: 200, h: 50 }], { w: 1000, h: 1000 }, 6);
  assert.equal(s.x, 300);            // 298 → sibling maxX 300
  assert.equal(s.y, 55);             // no y target near
  assert.ok(s.guides.some(g => g.x1 === 300));
});

const GCFG: BoxFieldConfig = { ...CFG, fontSizeField: 'fontSize', radiusField: 'radius' };
...
test('scaleGroup scales positions + sizes about the anchor, keeping the anchor fixed', () => {
  ...
  assert.deepEqual([at(out, 0).x, at(out, 0).y, at(out, 0).w, at(out, 0).h], [100, 100, 200, 200]);
  // b's centre (350,150) → (100,100)+2*(250,50)=(600,200); size 200 → x=500,y=100
  assert.deepEqual([at(out, 1).x, at(out, 1).y, at(out, 1).w, at(out, 1).h], [500, 100, 200, 200]);
  assert.equal(at(out, 1).fontSize, 80);            // text scaled
  assert.equal(at(out, 1).radius, 40);              // radius scaled
});
...
test('rotateGroup rotates member centres about the pivot and adds to each rotation', () => {
  ...
  const o0 = at(out, 0);
  const c = { x: num(o0.x) + num(o0.w) / 2, y: num(o0.y) + num(o0.h) / 2 };
  near(c.x, 0, 1e-6); near(c.y, 150, 1e-6);
  assert.equal(o0.rot, 100);                // 10 + 90
});
