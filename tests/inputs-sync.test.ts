 ...
 * inputs-sync.ts). This is the logic that lets a keystroke avoid rebuilding the
...
import type { InputControl } from '@lolly/engine';
...
function makePanel(html: string): HTMLElement {
  ...
  if (!el) throw new Error('panel element missing');
  return el;
}

const inp = (id: string, control: InputControl, value: SyncableInput['value'], extra: Partial<SyncableInput> = {}): SyncableInput =>
  ({ id, control, value, ...extra });
