...
export function parseSvgPathArgs(str) {
...
export function parseSvgPath(d) {
  ...
  const subpaths = [];
  let cur = null;                 // current subpath being built
  ...
  const open = (x, y) => {
    cur = { segments: [{ op: 'M', x, y }], closed: false };
    subpaths.push(cur);
  };
  const line = (x, y) => { if (cur) cur.segments.push({ op: 'L', x, y }); };
    ...
    switch (C) {
      case 'M':
        for (let i = 0; i + 1 < nums.length; i += 2) {
          ...
          if (i === 0) { open(x, y); sx = x; sy = y; }   // new subpath
 ...
 */
export function svgArcToBeziers(x1, y1, rx, ry, phi, fa, fs, x2, y2) {
