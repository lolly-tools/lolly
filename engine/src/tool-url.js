...
const FORMAT_EXT = { png: 'png', jpg: 'jpg', jpeg: 'jpg', webp: 'webp', svg: 'svg', pdf: 'pdf' };
 ...
 * @param {string} src
 * @returns {{ toolId: string, format: string|null, query: string } | null}
 */
export function parseToolUrl(src) {
  ...
  if (u.hash) {
    ...
    if (m && ID_RE.test(m[1])) return { toolId: m[1], format: null, query: hQuery };
  }
 ...
 * @param {{ toolId: string, format?: string, query?: string }} spec
 * @returns {string|null}
 */
export function buildEmbedUrl({ toolId, format, query = '' } = {}) {
