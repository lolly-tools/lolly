...
export const UNITS = ['px', 'pt', 'pc', 'mm', 'cm', 'in'];

// How many of each unit fit in one inch.
const PER_INCH = { px: 96, pt: 72, pc: 6, mm: 25.4, cm: 2.54, in: 1 };

export const isUnit = (u) => Object.prototype.hasOwnProperty.call(PER_INCH, u);
 ...
 * @param {string} [defaultUnit='px']  unit to assume when the string has none
 */
export function parseDimension(input, defaultUnit = 'px') {
...
export const toInches = (dim) => dim.value / PER_INCH[dim.unit];

/** True for any unit that carries a physical size (everything but raw px). */
export const isPhysical = (dim) => Boolean(dim) && dim.unit !== 'px';
 ...
 */
export function toPixels(dim, dpi = CSS_DPI) {
