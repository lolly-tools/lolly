 ...
 *
 * Sources (fetched 2026-07-08 — refresh periodically):
 *   • Google C2PA Root CA G3 — http://pki.goog/c2pa/root-g3.crt (Gemini / "Nano
 *     Banana"; NOT yet in the Adobe list, so vendored separately and verified:
 *     self-signed, and it signs the intermediate in Gemini's own x5chain).
 *   • The C2PA / Content Authenticity trust list —
 *     https://verify.contentauthenticity.org/trust/anchors.pem (Adobe, the
 *     camera makers — Canon/Nikon/Sony/Leica/Fujifilm — Microsoft, DigiCert,
 *     SSL.com, and AI providers Bria/Metaphysic, etc.).
 *
 * NOTE: verifyC2pa's chain check currently verifies ECDSA (P-256/384/521)
 * issuer steps only, so ECDSA-rooted signers (Google, the camera makers, …)
 * read as trusted today; RSA-rooted anchors (Adobe, Microsoft, DigiCert,
 * SSL.com) are carried here but their chains stay honestly "untrusted" until
 * RSA chain verification lands. Malformed/foreign entries are skipped, never
 * fatal. To refresh: re-fetch both sources and regenerate this file.
 */
...
export function c2paTrustAnchors(): Uint8Array[] {
  ...
  const blocks = C2PA_TRUST_ANCHORS_PEM.match(/-----BEGIN CERTIFICATE-----[\s\S]*?-----END CERTIFICATE-----/g) || [];
  const ders: Uint8Array[] = [];
  for (const b of blocks) { try { ders.push(pemToDer(b)); } catch { /* skip malformed anchor */ } }
  cache = ders;
  return ders;
}
