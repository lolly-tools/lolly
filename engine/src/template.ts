    ...
    .map(block => {
      ...
      if (lines.length && lines.every(l => BULLET.test(l))) {
        const items = lines.map(l => {
          const marker = l.match(BULLET)?.[1];  // every line matched above
          const arrowClass = marker === undefined ? undefined : ARROW_CLASSES[marker];
 ...
 * @param opts.raw  Disable HTML escaping of `{{x}}`. Used for
...
export function hydrate(
  ...
  const key = raw ? '\0raw\0' + templateSource : templateSource;
  ...
  if (compiled) {
    ...
    if (compileCache.size > COMPILE_CACHE_MAX) {
      ...
      if (oldest !== undefined) compileCache.delete(oldest); // evict oldest
    }
  }
