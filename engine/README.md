...
**106 modules** under `engine/src/` (excluding the `index.ts` barrel): 100 re-exported from `index.ts`, 90 with a dedicated `tests/*.test.ts`, 13 covered indirectly, 3 with no coverage under `tests/`, 16 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` — do not hand-edit between the markers.
...
| `bytes.ts` | 58 | Shared byte-level primitives for the engine's binary/crypto format modules (c2pa, c2pa-verify, seal, x509, zip-crypto, pdf-crypto-r6, …). | no | none | – |
| `c2pa-containers.ts` | 1153 | C2PA container placement — the per-format byte-splicing side of the writer: classic-xref PDF incremental update, the png/jpeg/gif/svg/tiff/webp embedders, ISO BMFF (mp4) with its own c2pa.hash.bmff.v2 binding, and the… | no | `tests/c2pa-containers.test.ts` | yes |
| `c2pa-extract.ts` | 782 | C2PA structural extraction — the read side's format-sniffing, CBOR decoding, JUMBF-store walking, and per-container manifest extraction (pdf/png/jpeg/gif/ svg/tiff/webp/mp4/webm), plus the ingredient-preparation helpers… | no | `tests/c2pa-extract.test.ts` | yes |
...
| `c2pa-verdict.ts` | 254 | C2PA verdict resolution — the single source of truth for (a) the check-code vocabulary verifyC2pa emits, (b) the flags→verdict ladder every surface renders, and (c) trust-anchor assembly. | yes | `tests/c2pa-verdict.test.ts` | – |
| `c2pa-verify.ts` | 957 | C2PA (Content Credentials) verifier — pure, DOM-free. | yes | `tests/c2pa-verify.test.ts` | yes |
| `c2pa.ts` | 763 | C2PA (Content Credentials) manifest builder + PDF embedder — pure, DOM-free. | yes | `tests/c2pa.test.ts` | – |
| `catalog-integrity.ts` | 240 | Catalog signing + runtime integrity verification — the SOVEREIGNTY.md "catalog origin is a trust anchor" gap, closed. | yes | `tests/catalog-integrity.test.ts` | – |
...
| `color-tools.ts` | 566 | Colour tools — perceptual metrics and ramp math on top of brand-derive's OKLab core. | yes | `tests/color-tools.test.ts` | – |
...
| `design-map.ts` | 1036 | Design-file → Layout Studio boxes (pure mapper). | yes | `tests/design-map.test.ts` | – |
...
| `gamut.ts` | 518 | Display-gamut classification for OKLCH colours — which of sRGB, Display-P3 or Rec.2020 can actually show a given lightness/chroma/hue. | yes | `tests/gamut.test.ts` | – |
...
| `hdr.ts` | 263 | HDR raster export — brand-colour highlight boost + PQ (SMPTE ST 2084) encoding. | yes | `tests/hdr.test.ts` | – |
...
| `pptx-patch.ts` | 352 | pptx-patch.ts — SURGICAL rebrand of an unzipped .pptx part map (Pipeline A, plans/fable-new-potential-pptx.md §2.2 / track E2). | yes | `tests/pptx-patch.test.ts` | yes |
...
| `strip-metadata.ts` | 334 | Embedded-metadata stripper | yes | `tests/strip-metadata.test.ts` | yes |
...
| `url-mode.ts` | 643 | URL mode. | yes | indirect | – |
| `url-pack.ts` | 308 | Packed URL state — the compact transport for large tool state. | yes | `tests/url-pack.test.ts` | yes |
| `validate.ts` | 66 | Validates a tool manifest against the JSON Schema. | yes | indirect | – |
...
| `wav.ts` | 152 | WAV reader — RIFF bytes in, Float32 channel data out. | yes | `tests/wav.test.ts` | yes |
| `webp-anim.ts` | 164 | Animated WebP packer — pure, DOM-free, platform-agnostic. | yes | `tests/webp-anim.test.ts` | – |
| `x509.ts` | 309 | DER / X.509 authority — pure, DOM-free (globalThis.crypto only; browsers and Node 18+). | yes | `tests/x509.test.ts` | yes |
