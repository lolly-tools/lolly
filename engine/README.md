...
**154 modules** under `engine/src/` (excluding the `index.ts` barrel): 140 re-exported from `index.ts`, 128 with a dedicated `tests/*.test.ts`, 20 covered indirectly, 6 with no coverage under `tests/`, 18 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` — do not hand-edit between the markers.
...
| `runtime.ts` | 1432 | Runtime — orchestrates the 5-step lifecycle for a single mounted tool. | yes | indirect | – |
