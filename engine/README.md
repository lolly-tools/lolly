...
**153 modules** under `engine/src/` (excluding the `index.ts` barrel): 140 re-exported from `index.ts`, 128 with a dedicated `tests/*.test.ts`, 20 covered indirectly, 5 with no coverage under `tests/`, 18 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` — do not hand-edit between the markers.
...
| `design-version.ts` | 482 | design-version.ts — the pure model behind versioned design systems (plans/97 §6a). | yes | `tests/design-version.test.ts` | – |
...
| `inputs.ts` | 639 | Builds a runtime input model from a tool manifest. | yes | indirect | – |
...
| `loader.ts` | 457 | Tool loader. | yes | indirect | – |
...
| `runtime.ts` | 1352 | Runtime — orchestrates the 5-step lifecycle for a single mounted tool. | yes | indirect | – |
