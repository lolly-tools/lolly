...
**111 modules** under `engine/src/` (excluding the `index.ts` barrel): 105 re-exported from `index.ts`, 95 with a dedicated `tests/*.test.ts`, 13 covered indirectly, 3 with no coverage under `tests/`, 16 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` — do not hand-edit between the markers.
...
| `png.ts` | 440 | PNG encoder — 8-bit and 16-bit truecolour, pure bytes, DOM-free. | yes | `tests/png.test.ts` | – |
...
| `tokens.ts` | 525 | Design tokens — a platform-agnostic DTCG model. | yes | `tests/tokens.test.ts` | – |
...
| `url-mode.ts` | 697 | URL mode. | yes | indirect | – |
