...
**116 modules** under `engine/src/` (excluding the `index.ts` barrel): 105 re-exported from `index.ts`, 100 with a dedicated `tests/*.test.ts`, 13 covered indirectly, 3 with no coverage under `tests/`, 16 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` — do not hand-edit between the markers.
