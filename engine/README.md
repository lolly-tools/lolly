...
**168 modules** under `engine/src/` (excluding the `index.ts` barrel): 151 re-exported from `index.ts`, 137 with a dedicated `tests/*.test.ts`, 23 covered indirectly, 8 with no coverage under `tests/`, 18 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `text-facts.ts` | 162 | Document facts - a NEUTRAL census of what a text observably contains, for the verify and catalog panels' interrogation surface. | yes | `tests/text-facts.test.ts` | – |
