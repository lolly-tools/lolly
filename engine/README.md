...
| `c2pa-extract.ts` | 1927 | C2PA structural extraction - the read side's format-sniffing, CBOR decoding, JUMBF-store walking, and per-container manifest extraction (pdf/png/jpeg/gif/ svg/tiff/webp/mp4/webm/mp3/wav, plus the C2PA 2.4 TEXT bindings… | no | `tests/c2pa-extract.test.ts` | yes |
...
| `c2pa-verify.ts` | 1641 | C2PA (Content Credentials) verifier - pure, DOM-free. | yes | `tests/c2pa-verify.test.ts` | yes |
...
| `clamp.ts` | 10 | The one numeric clamp. | yes | none | – |
...
| `compare-budget.ts` | 70 | Work and output limits shared by comparison algorithms. | no | none | – |
| `compare-structure.ts` | 88 | Structured field comparison with ordered arrays and optional stable-ID moves. | no | none | – |
| `compare-text.ts` | 51 | Bounded line and word alignment with original source locations. | no | none | – |
...
| `inputs.ts` | 805 | Builds a runtime input model from a tool manifest. | yes | indirect | – |
...
| `pdf-map.ts` | 2073 | PDF (and Adobe Illustrator .ai - an .ai IS a PDF) page content stream → DesignNodes. | yes | `tests/pdf-map.test.ts` | yes |
...
| `prepare-document.ts` | 189 | Bounded local document scopes and source-range edits for preparation jobs. | no | none | – |
...
| `prepare-text.ts` | 104 | Credential and personal-data suggestions with bounded declarative rules. | yes | none | – |
