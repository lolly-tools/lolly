...
**181 modules** under `engine/src/` (excluding the `index.ts` barrel): 162 re-exported from `index.ts`, 145 with a dedicated `tests/*.test.ts`, 26 covered indirectly, 10 with no coverage under `tests/`, 19 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `keyframes.ts` | 1687 | Keyframe tracks, the `kf` wire grammar, and the depth-camera projection - the shared, DOM-free maths every consumer of plans/104 trusts. | yes | `tests/keyframes.test.ts` | – |
...
| `pdf-artwork.ts` | 340 | Vector artwork detection - find the logos on a page full of shapes. | yes | `tests/pdf-artwork.test.ts` | – |
...
| `url-mode.ts` | 976 | URL mode. | yes | indirect | – |
