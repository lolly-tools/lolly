...
**177 modules** under `engine/src/` (excluding the `index.ts` barrel): 158 re-exported from `index.ts`, 141 with a dedicated `tests/*.test.ts`, 26 covered indirectly, 10 with no coverage under `tests/`, 19 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
