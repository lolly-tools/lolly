...
**166 modules** under `engine/src/` (excluding the `index.ts` barrel): 149 re-exported from `index.ts`, 135 with a dedicated `tests/*.test.ts`, 23 covered indirectly, 8 with no coverage under `tests/`, 18 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` — do not hand-edit between the markers.
