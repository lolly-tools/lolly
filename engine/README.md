...
**157 modules** under `engine/src/` (excluding the `index.ts` barrel): 142 re-exported from `index.ts`, 130 with a dedicated `tests/*.test.ts`, 22 covered indirectly, 5 with no coverage under `tests/`, 18 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` — do not hand-edit between the markers.
...
| `pptx.ts` | 756 | PPTX (PowerPoint / OOXML) builder. | yes | `tests/pptx.test.ts` | – |
...
| `svg-custgeom.ts` | 339 | Flat-SVG to native PowerPoint custom geometry. | yes | `tests/svg-custgeom.test.ts` | – |
