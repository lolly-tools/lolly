...
**165 modules** under `engine/src/` (excluding the `index.ts` barrel): 148 re-exported from `index.ts`, 134 with a dedicated `tests/*.test.ts`, 23 covered indirectly, 8 with no coverage under `tests/`, 18 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` — do not hand-edit between the markers.
...
| `inpaint.ts` | 405 | Telea inpainting: fill a brushed-out region of an RGBA frame from the pixels around it, by fast marching inward from the region boundary. | yes | `tests/inpaint.test.ts` | – |
