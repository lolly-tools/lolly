...
**164 modules** under `engine/src/` (excluding the `index.ts` barrel): 147 re-exported from `index.ts`, 133 with a dedicated `tests/*.test.ts`, 23 covered indirectly, 8 with no coverage under `tests/`, 18 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` — do not hand-edit between the markers.
...
| `claudisms.ts` | 347 | AI writing-tell patterns for the text-signal analyzer (engine/src/text-signals.ts). | no | none | – |
...
| `reword.ts` | 379 | Reword flagged text - the SEMANTIC half humanize.ts's header defers (plans/127). | yes | `tests/reword.test.ts` | – |
...
| `url-mode.ts` | 783 | URL mode. | yes | indirect | – |
