...
**120 modules** under `engine/src/` (excluding the `index.ts` barrel): 109 re-exported from `index.ts`, 101 with a dedicated `tests/*.test.ts`, 14 covered indirectly, 5 with no coverage under `tests/`, 16 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` — do not hand-edit between the markers.
...
| `c2pa-containers.ts` | 1176 | C2PA container placement — the per-format byte-splicing side of the writer: classic-xref PDF incremental update, the png/jpeg/gif/svg/tiff/webp embedders, ISO BMFF (mp4) with its own c2pa.hash.bmff.v2 binding, and the… | no | `tests/c2pa-containers.test.ts` | yes |
...
| `cmyk-palette.ts` | 118 | The brand-swatch → CMYK lookup every CMYK sink shares. | yes | none | – |
...
| `preflight.ts` | 1318 | Preflight — pre-export findings over a plain job description. | yes | `tests/preflight.test.ts` | – |
| `print-marks.ts` | 285 | Print-marks & bleed geometry — platform-agnostic, no DOM. | yes | `tests/print-marks.test.ts` | – |
| `provenance-defaults.ts` | 75 | Whether an export carries provenance marks WHEN NOBODY SAID — the one home for the two default-on answers every shell needs before it exports anything: | yes | none | – |
| `radiance.ts` | 639 | Radiance RGBE (`.hdr` / `.pic`) reader + writer — pure bytes, DOM-free. | no | `tests/radiance.test.ts` | – |
| `rate-card.ts` | 662 | The printer's own rate card — stored, validated, never a source of prices. | yes | indirect | – |
