...
**218 modules** under `engine/src/` (excluding the `index.ts` barrel): 187 re-exported from `index.ts`, 177 with a dedicated `tests/*.test.ts`, 38 covered indirectly, 3 with no coverage under `tests/`, 29 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `claudisms.ts` | 359 | AI writing-tell patterns for the text-signal analyzer (engine/src/text-signals.ts). | no | `tests/claudisms.test.ts` | – |
