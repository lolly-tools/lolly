...
**178 modules** under `engine/src/` (excluding the `index.ts` barrel): 159 re-exported from `index.ts`, 142 with a dedicated `tests/*.test.ts`, 26 covered indirectly, 10 with no coverage under `tests/`, 19 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `audio-loudness.ts` | 131 | audio-loudness.ts - ITU-R BS.1770-4 integrated loudness (plans/101 section 2.5, plans/165's deferred tier). | yes | `tests/audio-loudness.test.ts` | – |
