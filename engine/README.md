...
| `text-signals.ts` | 1063 | Text AI-likelihood signals - a string in, a tiered report of the signals that bear on "was this text generated by (or run through) an AI model" out. | yes | `tests/text-signals.test.ts` | – |
| `text-watermark.ts` | 177 | Statistical text watermark - the green-list scheme of Kirchenbauer et al., "A Watermark for Large Language Models" (arXiv:2301.10226), as Lolly's own generation paths embed it and /verify detects it. | yes | `tests/text-watermark.test.ts` | – |
