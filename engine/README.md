...
**129 modules** under `engine/src/` (excluding the `index.ts` barrel): 117 re-exported from `index.ts`, 107 with a dedicated `tests/*.test.ts`, 17 covered indirectly, 5 with no coverage under `tests/`, 18 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` — do not hand-edit between the markers.
...
| `media-sniff.ts` | 191 | Pure, DOM-free media classification from header bytes. | yes | `tests/media-sniff.test.ts` | yes |
...
| `runtime.ts` | 1260 | Runtime — orchestrates the 5-step lifecycle for a single mounted tool. | yes | indirect | – |
