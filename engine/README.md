...
**123 modules** under `engine/src/` (excluding the `index.ts` barrel): 112 re-exported from `index.ts`, 104 with a dedicated `tests/*.test.ts`, 14 covered indirectly, 5 with no coverage under `tests/`, 16 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` — do not hand-edit between the markers.
...
| `c2pa-containers.ts` | 1296 | C2PA container placement — the per-format byte-splicing side of the writer: classic-xref PDF incremental update, the png/jpeg/gif/svg/tiff/webp embedders, ISO BMFF (mp4) with its own c2pa.hash.bmff.v2 binding, and the… | no | `tests/c2pa-containers.test.ts` | yes |
| `c2pa-extract.ts` | 842 | C2PA structural extraction — the read side's format-sniffing, CBOR decoding, JUMBF-store walking, and per-container manifest extraction (pdf/png/jpeg/gif/ svg/tiff/webp/mp4/webm/mp3/wav), plus the ingredient-preparation… | no | `tests/c2pa-extract.test.ts` | yes |
...
| `c2pa.ts` | 774 | C2PA (Content Credentials) manifest builder + PDF embedder — pure, DOM-free. | yes | `tests/c2pa.test.ts` | – |
...
| `speech-text.ts` | 438 | Speech synthesis text machinery — the PURE half of Kokoro TTS. | yes | `tests/speech-text.test.ts` | – |
