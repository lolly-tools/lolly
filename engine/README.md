...
**109 modules** under `engine/src/` (excluding the `index.ts` barrel): 103 re-exported from `index.ts`, 93 with a dedicated `tests/*.test.ts`, 13 covered indirectly, 3 with no coverage under `tests/`, 16 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` — do not hand-edit between the markers.
...
| `pdf-map.ts` | 2024 | PDF (and Adobe Illustrator .ai — an .ai IS a PDF) page content stream → DesignNodes. | yes | `tests/pdf-map.test.ts` | yes |
