...
**167 modules** under `engine/src/` (excluding the `index.ts` barrel): 150 re-exported from `index.ts`, 136 with a dedicated `tests/*.test.ts`, 23 covered indirectly, 8 with no coverage under `tests/`, 18 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `c2pa.ts` | 1114 | C2PA (Content Credentials) manifest builder + PDF embedder - pure, DOM-free. | yes | `tests/c2pa.test.ts` | - |
...
| `claudisms.ts` | 350 | AI writing-tell patterns for the text-signal analyzer (engine/src/text-signals.ts). | no | none | - |
...
| `file-metadata.ts` | 1237 | Embedded-metadata reader | yes | `tests/file-metadata.test.ts` | yes |
...
| `pptx.ts` | 894 | PPTX (PowerPoint / OOXML) builder. | yes | `tests/pptx.test.ts` | - |
...
| `runtime.ts` | 1509 | Runtime - orchestrates the 5-step lifecycle for a single mounted tool. | yes | indirect | - |
...
| `text-signals.ts` | 1136 | Text AI-likelihood signals - a string in, a tiered report of the signals that bear on "was this text generated by (or run through) an AI model" out. | yes | `tests/text-signals.test.ts` | - |
| `text-watermark.ts` | 224 | Statistical text watermark - the green-list scheme of Kirchenbauer et al., "A Watermark for Large Language Models" (arXiv:2301.10226), as Lolly's own generation paths embed it and /verify detects it. | yes | `tests/text-watermark.test.ts` | - |
...
| `url-mode.ts` | 786 | URL mode. | yes | indirect | - |
