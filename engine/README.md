...
**174 modules** under `engine/src/` (excluding the `index.ts` barrel): 156 re-exported from `index.ts`, 139 with a dedicated `tests/*.test.ts`, 26 covered indirectly, 9 with no coverage under `tests/`, 19 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `inputs.ts` | 738 | Builds a runtime input model from a tool manifest. | yes | indirect | – |
