...
**200 modules** under `engine/src/` (excluding the `index.ts` barrel): 178 re-exported from `index.ts`, 155 with a dedicated `tests/*.test.ts`, 30 covered indirectly, 15 with no coverage under `tests/`, 28 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `runtime.ts` | 1744 | Runtime - orchestrates the 5-step lifecycle for a single mounted tool. | yes | indirect | – |
