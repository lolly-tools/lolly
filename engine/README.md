...
**191 modules** under `engine/src/` (excluding the `index.ts` barrel): 172 re-exported from `index.ts`, 153 with a dedicated `tests/*.test.ts`, 28 covered indirectly, 10 with no coverage under `tests/`, 28 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `document-api.ts` | 218 | Stable, transport-neutral document/compiler verbs. | yes | `tests/document-api.test.ts` | – |
...
| `penpot-file.ts` | 1993 | `.penpot` writer - a Lolly document (plus the brand's tokens) → the binfile-v3 archive Penpot itself exports and imports (plans/178). | yes | `tests/penpot-file.test.ts` | – |
...
| `runtime.ts` | 1629 | Runtime - orchestrates the 5-step lifecycle for a single mounted tool. | yes | indirect | – |
...
| `template.ts` | 509 | Template hydration. | yes | indirect | – |
