...
**180 modules** under `engine/src/` (excluding the `index.ts` barrel): 161 re-exported from `index.ts`, 144 with a dedicated `tests/*.test.ts`, 26 covered indirectly, 10 with no coverage under `tests/`, 19 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `penpot-file.ts` | 1869 | `.penpot` writer - a Lolly document (plus the brand's tokens) → the binfile-v3 archive Penpot itself exports and imports (plans/178). | yes | `tests/penpot-file.test.ts` | – |
...
| `pptx-read.ts` | 1375 | pptx-read.ts: PARSE an unzipped .pptx part map into a read-model. | yes | `tests/pptx-read.test.ts` | yes |
