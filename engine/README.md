...
**147 modules** under `engine/src/` (excluding the `index.ts` barrel): 134 re-exported from `index.ts`, 123 with a dedicated `tests/*.test.ts`, 19 covered indirectly, 5 with no coverage under `tests/`, 18 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` — do not hand-edit between the markers.
...
| `c2pa-containers.ts` | 1306 | C2PA container placement — the per-format byte-splicing side of the writer: classic-xref PDF incremental update, the png/jpeg/gif/svg/tiff/webp embedders, ISO BMFF (mp4) with its own c2pa.hash.bmff.v2 binding, and the… | no | `tests/c2pa-containers.test.ts` | yes |
| `c2pa-extract.ts` | 896 | C2PA structural extraction — the read side's format-sniffing, CBOR decoding, JUMBF-store walking, and per-container manifest extraction (pdf/png/jpeg/gif/ svg/tiff/webp/mp4/webm/mp3/wav), plus the ingredient-preparation… | no | `tests/c2pa-extract.test.ts` | yes |
...
| `inputs.ts` | 631 | Builds a runtime input model from a tool manifest. | yes | indirect | – |
...
| `loader.ts` | 456 | Tool loader. | yes | indirect | – |
...
| `runtime.ts` | 1310 | Runtime — orchestrates the 5-step lifecycle for a single mounted tool. | yes | indirect | – |
...
| `url-mode.ts` | 726 | URL mode. | yes | indirect | – |
