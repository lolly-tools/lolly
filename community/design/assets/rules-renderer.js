"use strict";var LollyDesignRules=(()=>{var S=Object.defineProperty;var A=Object.getOwnPropertyDescriptor;var $=Object.getOwnPropertyNames;var C=Object.prototype.hasOwnProperty;var F=(s,g)=>{for(var m in g)S(s,m,{get:g[m],enumerable:!0})},j=(s,g,m,r)=>{if(g&&typeof g=="object"||typeof g=="function")for(let c of $(g))!C.call(s,c)&&c!==m&&S(s,c,{get:()=>g[c],enumerable:!(r=A(g,c))||r.enumerable});return s};var O=s=>j(S({},"__esModule",{value:!0}),s);var L={};F(L,{designSelection:()=>R,designToolPolicy:()=>T,evaluateDesignTool:()=>E,validateDesignTool:()=>_,validateDesignValues:()=>D});function z(s,g,m,r){let c=s.stories.find(o=>o.id===g.textStory);if(!c)throw new Error("The template text story is missing.");if(m==="text"){let o=String(r??"");if(o===c.source)return!0;if(o.length>64e3||o.includes("\uFFFC"))throw new Error("The supplied template text exceeds its supported source limits.");let u=c.paragraphs[0],y=c.spans.find(l=>l.start===0),e=[];for(let l of o.matchAll(/\r\n|[\r\n\v\f\u0085\u2028\u2029]/g))e.push({start:l.index,length:l[0].length,kind:l[0]==="\u2028"?"soft":"paragraph"});let i=0,t=0,n=[],x=new Set([u.id]),p=()=>{if(!t)return u.id;let l=`${c.id.slice(0,96)}-field-${t}`;for(;x.has(l);)l+="x";return x.add(l),l};for(let l of e.filter(w=>w.kind==="paragraph"))n.push({...u,id:p(),start:i,end:l.start}),i=l.start+l.length,t++;return n.push({...u,id:p(),start:i,end:o.length}),c.source=o,c.breaks=e,c.paragraphs=n,c.inlines=[],c.spans=o&&y?[{...y,start:0,end:o.length}]:[],c.revision++,!0}let a={fontSize:"size",weight:"weight",fg:"color",font:"font"}[m];if(!a)return!1;let d=m==="fontSize"||m==="weight"?Number(r):r;if(m==="font"){let o=s.fonts.find(u=>u.id===r||u.family===r);if(!o)throw new Error("Choose a font embedded in this template.");d=o.id}let f={[a]:d};function b(o,u=new Set){if(!o)return{};if(u.has(o)||u.size>=32)throw new Error("The template text style inheritance is invalid.");u.add(o);let y=s.styles.find(e=>e.id===o);if(!y)throw new Error("The template text style is missing.");return{...b(y.basedOn,u),...y.paragraph?.character?.axes,...y.character?.axes}}if(m==="weight"){for(let o of c.paragraphs)({...b(c.defaultStyle),...b(o.style),...o.paragraph?.character?.axes}).wght!==void 0&&(o.paragraph={...o.paragraph,character:{...o.paragraph?.character,axes:{...o.paragraph?.character?.axes,wght:Number(d)}}});for(let o of c.spans)({...b(o.style),...o.character?.axes}).wght!==void 0&&(o.character={...o.character,axes:{...o.character?.axes,wght:Number(d)}})}return c.paragraphs=c.paragraphs.map(o=>({...o,paragraph:{...o.paragraph,character:{...o.paragraph?.character,...f}}})),c.spans=c.spans.map(o=>({...o,character:{...o.character,...f}})),c.revision++,!0}var k={text:["text","longtext","select"],image:["asset"],fontSize:["number"],font:["select"],weight:["select","number"],fg:["color","select"],fill:["color","select"],fit:["select"],imageFraming:["vector"]},N=new Set(["__proto__","prototype","constructor"]),M=new Set(["boxes","textDocument","exportVisibleText","customCss","background","transparentBg","font","fontSize","boxStyle","textStyle","mediaHtml","textHtml","designRows","designWidth","designHeight","designBackground","designIssues"]),P=/^[a-zA-Z][a-zA-Z0-9_-]{0,63}$/,v=(s,g)=>Object.hasOwn(s,g),V=s=>`${s.variantId}/${s.layerId}/${s.property}`;function T(s){return{schemaVersion:1,presentation:s.presentation,inputs:s.inputs,choices:s.choices,variants:s.variants.map(({id:g,label:m,width:r,height:c})=>({id:g,label:m,width:r,height:c})),defaultVariant:s.defaultVariant,formats:s.formats,...s.sourceTool?{sourceTool:s.sourceTool}:{}}}function _(s,g=[]){let m=[],r=(e,i,t,n)=>{m.push({code:e,message:i,inputId:t,layerId:n})};s.schemaVersion!==1&&r("version","This rules version is not supported."),/^[a-z0-9][a-z0-9-]{2,95}$/.test(s.id)||r("identity","Use a valid permanent tool id."),(!s.name.trim()||!/^\d+\.\d+\.\d+$/.test(s.version))&&r("identity","Add a name and a three-part version."),["sidebar","on-canvas"].includes(s.presentation)||r("presentation","Choose Sidebar or On-canvas."),(!s.formats.length||s.formats.some(e=>!["png","svg","pdf"].includes(e)))&&r("formats","Choose PNG, SVG or PDF."),(!s.variants.length||s.variants.length>24||s.inputs.length>64||s.choices.length>4||s.recipes.length>64)&&r("limits","Use up to 24 artboards, 64 inputs and four choices.");let c=new Map(s.variants.map(e=>[e.id,e]));(c.size!==s.variants.length||!c.has(s.defaultVariant))&&r("artboards","Choose a valid default artboard and unique artboard ids.");for(let e of s.variants)(!/^[a-zA-Z0-9][a-zA-Z0-9_-]{0,63}$/.test(e.id)||![e.width,e.height].every(i=>Number.isFinite(i)&&i>=1&&i<=16384))&&r("artboard-size","Artboards need a size between 1 and 16384 pixels."),(e.boxes.length>1e3||new Set(e.boxes.map(i=>i.id)).size!==e.boxes.length)&&r("layers","Layers need unique, permanent ids.");let h=new Map(s.inputs.map(e=>[e.input.id,e])),a=new Set(s.choices.map(e=>e.inputId));(h.size!==s.inputs.length||a.size!==s.choices.length)&&r("duplicate-id","Each input and choice needs a unique id.");let d=new Map,f=new Map,b=new Map,o=(e,i,t)=>{if(!v(k,e.property)){r("property","This property cannot be made editable.",i,e.layerId);return}c.get(e.variantId)?.boxes.find(p=>p.id===e.layerId)||r("missing-layer","The linked object was removed. Relink this input.",i,e.layerId),t&&!k[e.property].includes(t)&&r("type","The input type does not match its property.",i,e.layerId);let x=d.get(V(e));x&&x!==i&&r("ownership","Two controls change the same property. Choose one owner.",i,e.layerId),d.set(V(e),i)};for(let e of s.inputs){let i=e.input;if((!P.test(i.id)||N.has(i.id)||M.has(i.id)||g.includes(i.id)||i.id.startsWith("__"))&&r("input-id","Use a unique input id that is not a reserved setting.",i.id),["text","longtext","number","select","color","asset","vector",...s.sourceTool?["boolean","url","date","time","datetime-local"]:[]].includes(i.type)||r("input-type","This input type is not supported in a locked tool.",i.id),i.type==="select"&&(!i.options?.length||i.options.some(t=>!t.value||!t.label)||new Set(i.options.map(t=>t.value)).size!==i.options.length)&&r("options","Give each option a label and a unique value.",i.id),i.type==="number"&&(![i.min,i.max,i.step,i.default].every(t=>typeof t=="number"&&Number.isFinite(t))||Number(i.min)>Number(i.max)||Number(i.step)<=0)&&r("range","Set a finite minimum, maximum, default and positive step.",i.id),i.type==="asset"&&i.assetType!=="image"&&r("media","Editable images must use the image asset type.",i.id),i.type==="vector"){let t=i.fields;(!t?.length||t.length>3||new Set(t.map(n=>n.id)).size!==t.length||t.some(n=>!["x","y","zoom"].includes(n.id)||![n.min,n.max,n.step].every(Number.isFinite)||n.min>n.max||n.step<=0||n.min<(n.id==="zoom"?1:0)||n.max>(n.id==="zoom"?1e3:100)))&&r("framing-range","Set valid X/Y percentages and a bounded zoom range.",i.id)}!e.targets.length&&!s.sourceTool?.inputs[i.id]&&!a.has(i.id)&&!s.recipes.some(t=>t.parts.some(n=>"inputId"in n&&n.inputId===i.id))&&r("unlinked","Link this input to an object.",i.id),e.common&&(e.common.key==="headshot"?i.type!=="asset":!["text","longtext"].includes(i.type))&&r("common-type","Use an image for a headshot and text for other common fields.",i.id),e.image&&(i.type!=="asset"||[e.image.minWidth,e.image.minHeight].some(t=>t!==void 0&&(!Number.isInteger(t)||t<1||t>32768))||e.image.formats&&(!e.image.formats.length||e.image.formats.some(t=>!["png","jpeg","webp","avif","svg"].includes(t))))&&r("image-rule","Choose image formats and positive minimum pixel dimensions.",i.id),e.common?.source==="profile"&&(!["person","recipient","presenter"].includes(e.common.subject)||!["firstname","lastname","email"].includes(e.common.key))&&r("profile","Profile prefill is only available for a person\u2019s name or email.",i.id);for(let t of e.targets)o(t,i.id,i.type),t.text&&(!(t.text.min>0)||!Number.isFinite(t.text.max)||t.text.max<t.text.min)&&r("text-fit","Review this object\u2019s font-size range.",i.id,t.layerId);e.text&&(!(e.text.min>0)||!Number.isFinite(e.text.max)||e.text.min>e.text.max||e.text.maxLines!==void 0&&(!Number.isInteger(e.text.maxLines)||e.text.maxLines<1))&&r("text-fit","Set a valid font-size range and line limit.",i.id)}if(s.sourceTool){let e=Object.entries(s.sourceTool.inputs);(!s.sourceTool.id||!s.sourceTool.version||e.length!==s.inputs.length||new Set(e.map(([,i])=>i)).size!==e.length||e.some(([i,t])=>!h.has(i)||!t||N.has(t)||t.startsWith("__")))&&r("source-inputs","Choose distinct inputs from the source tool.")}let u=1,y=0;for(let e of s.choices){let i=h.get(e.inputId);(i?.input.type!=="select"||i.targets.length)&&r("choice","A choice needs its own select input.",e.inputId),u*=e.options.length,(!e.options.length||new Set(e.options.map(n=>n.value)).size!==e.options.length)&&r("choice-options","Give each choice option a unique value.",e.inputId),e.options.some(n=>n.variantId)&&y++;let t=new Set;for(let n of e.options){i?.input.options?.some(p=>p.value===n.value)||r("choice-option","Declare each choice option in the input.",e.inputId),n.variantId&&!c.has(n.variantId)&&r("choice-artboard","Choose an existing artboard.",e.inputId);for(let p of new Set([...n.fixedInputs??[],...Object.keys(n.defaults??{})])){let l=h.get(p);(!l||a.has(p))&&r("choice-field","Choose an existing content input.",e.inputId);let w=b.get(p);if(w&&w!==e.inputId&&r("ownership","Two choices set the same input. Choose one owner.",p),b.set(p,e.inputId),l&&v(n.defaults||{},p))for(let I of D({...T(s),choices:[],inputs:[l]},{[p]:n.defaults[p]}))m.push(I)}let x=new Set;for(let p of n.writes){let l=V(p),w=f.get(l);w&&w!==e.inputId&&r("ownership","Two choices control the same property.",e.inputId,p.layerId),f.set(l,e.inputId),x.has(l)&&r("choice-write","An option cannot write the same property twice.",e.inputId,p.layerId),x.add(l);let I=d.get(l);I&&I!==e.inputId&&!(n.fixedInputs??[]).includes(I)&&r("ownership","This option and a reader input both control this property.",I,p.layerId),I||o(p,e.inputId),t.add(l)}}}(u>128||y>1)&&r("choice-limits","Use one artboard choice and up to 128 option combinations.");for(let e of s.recipes){(e.target.property!=="text"||!e.parts.length||e.parts.length>16)&&r("recipe","Joined text needs a text target and up to 16 parts."),o(e.target,`recipe:${V(e.target)}`,"text");for(let i of e.parts)"inputId"in i&&!["text","longtext"].includes(h.get(i.inputId)?.input.type??"")&&r("recipe-input","Joined text can use only declared text inputs.",i.inputId)}for(let e of D(T(s),{},!0))m.push(e);return m}function R(s,g){let m=s.defaultVariant,r=new Set,c={},h=[];for(let a of s.choices){let d=v(g,a.inputId)?g[a.inputId]:s.inputs.find(b=>b.input.id===a.inputId)?.input.default,f=a.options.find(b=>b.value===d);if(f){f.variantId&&(m=f.variantId);for(let b of f.fixedInputs??[])r.add(b);Object.assign(c,f.defaults),h.push(...f.writes)}}return{variantId:m,fixed:r,defaults:c,writes:h}}function D(s,g,m=!1){let r=[],c=R(s,g);for(let h of s.inputs){let a=h.input;if(c.fixed.has(a.id))continue;let d=v(g,a.id)?g[a.id]:v(c.defaults,a.id)?c.defaults[a.id]:a.default,f=o=>{r.push({code:"value",message:`${a.label||a.id}: ${o}`,inputId:a.id})};if(d==null||d===""){m&&a.required&&f("add a value.");continue}if(["text","longtext","url","date","time","datetime-local"].includes(a.type))typeof d!="string"?f("enter text."):a.maxLength!==void 0&&d.length>a.maxLength&&f(`use up to ${a.maxLength} characters. The full value has been kept.`);else if(a.type==="boolean")typeof d!="boolean"&&f("choose on or off.");else if(a.type==="number"){let o=Number(d);(!Number.isFinite(o)||o<Number(a.min)||o>Number(a.max)||Math.abs((o-Number(a.min))/Number(a.step)-Math.round((o-Number(a.min))/Number(a.step)))>1e-7)&&f(`use ${a.min} to ${a.max}, in steps of ${a.step}.`)}else if(a.type==="select"&&!a.options?.some(o=>o.value===d))f("choose an approved option.");else if(a.type==="color"&&(typeof d!="string"||!/^#[\da-f]{3,8}$/i.test(d)))f("choose a valid colour.");else if(a.type==="asset"&&typeof d=="object"&&"type"in d&&(!["raster","vector"].includes(String(d.type))||"format"in d&&["gif","apng","html"].includes(String(d.format))))f("choose a still image.");else if(a.type==="vector"){let o=a.fields;if(!d||typeof d!="object"||!o?.length)f("set valid image positioning.");else for(let u of o){let y=Number(d[u.id]??a.default?.[u.id]);(!Number.isFinite(y)||y<u.min||y>u.max||u.step&&Math.abs((y-u.min)/u.step-Math.round((y-u.min)/u.step))>1e-7)&&f(`${u.id} must be between ${u.min} and ${u.max}, using its declared step.`)}}let b=typeof d=="object"&&d&&"id"in d?String(d.id):String(d);h.approved?.length&&!h.approved.includes(b)&&f("choose a designer-approved value.")}return r}function E(s,g){let m=T(s),r=R(m,g),c=s.variants.find(t=>t.id===r.variantId);if(!c)throw new Error("The selected artboard is unavailable.");let h=JSON.parse(JSON.stringify(c)),a={},d={},f={},b={},o={},u={},y=D(m,g,!0),e=new Set(y.map(t=>t.inputId)),i=(t,n)=>{if(t.variantId!==h.id)return;let x=h.boxes.find(p=>p.id===t.layerId);if(x&&v(k,t.property)){if(x.textStory&&h.textDocument&&z(h.textDocument,x,t.property,n))return;x[t.property]=n,t.property==="text"&&(x.plainText=!0)}};for(let t of s.inputs){let n=t.input,x=!e.has(n.id)&&v(g,n.id)?g[n.id]:v(r.defaults,n.id)?r.defaults[n.id]:n.default;if(a[n.id]=r.fixed.has(n.id)?r.defaults[n.id]??n.default:x,!r.fixed.has(n.id))for(let p of t.targets)i(p,x),p.variantId===h.id&&(f[p.layerId]??=n.id,p.property==="imageFraming"&&(b[p.layerId]=n.id),p.property==="image"&&t.image&&(u[p.layerId]=t.image),p.property==="text"&&t.text?.sharedSize&&(o[p.layerId]=n.id),t.text&&p.property==="text"&&(d[p.layerId]=p.text||t.text))}for(let t of s.recipes)if(i(t.target,t.parts.map(n=>"literal"in n?n.literal:String(a[n.inputId]??"")).join("")),t.target.variantId===h.id){let n=t.parts.find(x=>"inputId"in x);n&&"inputId"in n&&(f[t.target.layerId]=n.inputId),t.text&&(d[t.target.layerId]=t.text)}for(let t of r.writes)i(t,t.value);for(let t of s.inputs)if(t.input.type==="number"&&!r.fixed.has(t.input.id))for(let n of t.targets)n.variantId===h.id&&n.property==="fontSize"&&d[n.layerId]&&(d[n.layerId]={...d[n.layerId],min:Number(t.input.min),max:Number(t.input.max)});for(let t of h.boxes){let n=d[String(t.id)];if(t.fitText=!1,t.textStory&&h.textDocument&&n){let x=JSON.parse(String(t.textFrame));if(n.mode==="shrink"){if(h.textDocument.stories.find(l=>l.id===t.textStory).frameIds.length!==1)throw new Error("A linked story cannot shrink to fit. Use a fixed text rule.");x.shrink={minSize:n.min}}else delete x.shrink;t.textFrame=JSON.stringify(x);continue}n&&(t.fontSize=Math.min(n.max,Math.max(n.min,Number(t.fontSize)||n.max)))}return{variant:h,values:a,findings:y,textRules:d,inputMap:f,framingMap:b,fitGroups:o,imageRules:u}}return O(L);})();
