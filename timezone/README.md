...
- A shared table with City first, optional detail columns, spreadsheet/list paste, copy and a resizable pop-out. New work starts with London, Nuremberg, Prague, Sofia, San Francisco and Noosa examples; clearing the rows stays empty. Existing saved objects, row ids and shared URLs remain compatible. Any number of ordered location rows: a label, city or IANA identifier, annotation, colour, optional timezone-region fill, and optional exact coordinates. Reorder the rows to set the tour route. City names resolve offline; add a country/province or timezone override for ambiguous names. Unknown locations stay visible as errors instead of silently becoming UTC. An explicit IANA identifier uses its representative city unless coordinates are supplied.
...
Offline lookup supplements: Nuremberg aliases the existing Nürnberg record; Noosa / Noosa Heads uses the [Queensland place-names register, record 24497](https://spatial-gis.information.qld.gov.au/arcgis/rest/services/Location/QldPlaceNames/MapServer/0/query?where=REF_NO%3D24497&outFields=*&f=pjson). Country names as well as codes can disambiguate cities.
