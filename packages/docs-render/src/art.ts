...
/**
 * `::: figure <id>` — parse the fence label to the figure id, or null.
 *
 * The id line is CANONICAL: it is the same token in all 27 locale copies of the page,
 * so a translator never has to know what it means and an edit to it never strands 26
...
export function figureBlock(parts: { art: string; caption: string; credential: string; src: string }): string {
  ...
  return `<figure class="docs-figure" data-art="${parts.src}">`
    ...
    // An empty <figcaption> promises a caption and delivers none: a figure whose artifact
    // carries no readable credential and whose fence held no prose gets the artwork alone.
