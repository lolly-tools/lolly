 ...
 * field specs, and so on) are deliberately left to the schema. {@link InputSpec}
 * carries an index signature so those extra members type-check while the schema
 * validates them.
 */
...
export interface RenderSpec {
   ...
   *  default) is a vertical rail beside the canvas, or right for tall documents.
...
export interface ToolHookFlags {
   ...
   * a declared `format`. For example, color-palette returns an Adobe `.ase` swatch file
...
export interface ToolManifest {
  ...
  hooks?: ToolHookFlags;
  /**
   * Request Worker execution for a verified built-in tool's hooks (engine
   ...
   * touch DOM globals (document/window/Image/canvas). The receiving shell owns
   * the actual trust decision: sideloaded/remote hooks are forced into strict
   * isolation and fail closed when it is unavailable, regardless of this hint.
   */
  isolate?: boolean;
}
