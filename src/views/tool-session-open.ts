...
export async function openToolSession(state: StateAPI, toolId: string, url: UrlState, carriedSlot?: string | null): Promise<{
  ...
  const remembered = !url.slot && !carriedSlot ? localHistorySlot(state, toolId) : undefined;
  const slot = url.slot ?? carriedSlot ?? remembered;
  if (!slot) return { url, values: url.values };
  const history = toolId === 'design' && !getCollabSessionSource() ? (state as WebStateAPI).history : undefined;
  ...
  const values = saved ? remembered ? { ...url.values, ...saved } : { ...saved, ...url.values } : url.values;
  if (remembered && saved) url = { ...url, values: {}, filename: null, format: null, width: null, height: null, unit: null, dpi: null, profile: null, bleed: null, marks: null };
