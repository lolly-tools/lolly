...
A tool's URL is not a wider door than its sidebar. `engine/src/url-mode.ts` parses a query string against the tool's own declared inputs, and anything it does not recognise is ignored rather than guessed at. Only one group of parameters means something without being an input, and that group is closed and explicit - the exported `RESERVED` set in `engine/src/url-mode.ts`, covering output concerns such as `format`, `width`, `height`, `unit`, `dpi`, `profile`, `bleed`, `marks` and the provenance switches.
...
That is a statement about the adapter, not about every line of code a tool can run. Whether a hook can reach *around* `host.net` depends on how it is executed, and there are two modes (`engine/src/runtime.ts` takes the executor as its `hookExecutor` option; `engine/src/loader.ts` lists the trust levels):
