...
Trabalho criativo repetível é previsível demais para justificar mãos especializadas toda vez e sensível demais em qualidade para ser delegado sem parâmetros de proteção. Deixado à própria sorte, ele segue um de três caminhos: lento, inconsistente ou preso aos templates de um fornecedor.

Esta plataforma é a resposta direta:

> **Criação e conteúdo programáticos em escala** - assets gerados a partir de entradas, com as regras definidas uma vez, para quem precisar deles.

A Lolly não é onde um sistema de design é inventado - é onde ele é produzido. Pense nela como uma máquina de vendas automática para design: faça uma seleção, receba um resultado. Toda vez. O engine busca a maior qualidade que cada formato consegue produzir no hardware à sua frente, e o mesmo engine gera o mesmo arquivo em cada superfície para a qual ele é distribuído.

O resultado é **abundância**: todo evento tem a sinalização correta, todo alerta de CVE segue o estilo da casa, toda etiqueta imprime limpa, toda assinatura de e-mail está atualizada - tudo a partir de ferramentas que já carregam as regras. A plataforma trata do criativo operacionalizado e recorrente. Deliberadamente não é uma ferramenta criativa sob medida - os designers continuam donos do trabalho de destaque.
...
O Lolly traça a mesma linha. Explore probabilisticamente: um modelo, um designer, uma ideia solta, um prompt que vai para onde ninguém planejou. Depois escale deterministicamente - o que alcança muitos resultados é uma *ferramenta*, e uma ferramenta renderiza da mesma forma todas as vezes a partir de entradas que você consegue ler. A exploração continua livre porque nada depois dela depende de acontecer igual duas vezes. O resultado ganha confiança porque não é um palpite. Levar a experimentação com IA a resultados previsíveis e reprodutíveis não é uma disciplina nova; é a mesma divisão de trabalho que tornou o material impresso confiável desde o início.
...
![Um gráfico de área empilhada com título, suas três séries em faixas de uma paleta fria, com eixos, legenda e título posicionados pelo modelo em vez de manualmente](/t/url-shot?url=%2F%23%2Ftool%2Fchart%3FchartType%3Darea%26stackMode%3Dstacked%26palette%3Dcool%26heading%3DProduct%2520mix%2520by%2520quarter%26full&width=1440&height=900&dpi=192&waitMs=2600&walker=1&format=svg&cropSelector=%23tool-canvas&dark=1&filename=ov2-lifecycle-chart)
...
## Uma ferramenta, muitas saídas

As regras de marca vivem na ferramenta, não em cada arquivo que ela produz (veja [Como o Lolly se compara](/info/positioning.html)). Acerte a ferramenta uma vez e toda saída herda o mesmo tipo, cor e espaçamento, seja um único card ou uma planilha de linhas em uma dúzia de idiomas. Como você confere seu trabalho, e quem faz isso, continua sendo escolha sua; o Lolly torna a coisa a ser conferida menor e a saída mais rápida de produzir.

A mesma ferramenta determinística alcança essa escala de três formas, todas produzindo saída idêntica:

- <!--i:people--> **Uma pessoa, no app.** A grade de lote `/pro`: cole ou importe as linhas, receba um asset finalizado por linha, baixe o zip. Sem habilidade de design, sem espera.
- <!--i:code--> **Um desenvolvedor, pela linha de comando.** A CLI roda o *mesmo* engine e o *mesmo* caminho de renderização sem interface, então a ferramenta pode ser sequenciada por todas as linhas em um script ou pipeline noturno. Uma chamada `lolly <tool> --field=…` em um loop é toda a integração.
- <!--i:cpu--> **Um sistema ou agente de IA, via MCP.** A mesma ferramenta operada programaticamente, com a mesma fidelidade, para quantas linhas o trabalho tiver.

![O modo Batch em uma instalação nova: uma linha vazia esperando por uma ferramenta, com toda a superfície de planilha e seu botão Render já no lugar antes de qualquer dado chegar](/t/url-shot?url=%2F%23%2Fbatch&width=1440&height=900&dpi=192&waitMs=3500&walker=1&format=svg&dark=1&filename=ov2-batch-grid)

Um conjunto de restrições de marca, fixado uma vez por um designer; três caminhos para a mesma saída.
...
|---|---|---|
| `host.tokens` | 1.0 | Tokens de design DTCG - os primitivos da própria marca |
| `host.text` | 1.0 | Texto para caminho (text-to-path) via HarfBuzz WASM (a flag de capacidade `wasm` sinaliza as ferramentas que dependem disso) |
| `host.media` | 1.4 | Quadros de câmera ao vivo acionando o hook `onFrame`. Aprimoramento progressivo, deliberadamente *não* condicionado pela flag `camera` - essa ferramenta continua funcionando como uma ferramenta comum de imagem estática |
| `host.color` | 1.40 | Matemática de cor perceptual: ΔEOK, contraste WCAG + APCA, rampas OKLab, class-breaks, paletas categóricas, esquemas de harmonia (1.60), mistura CSS Color 4 e bake de gradiente (1.68). Pura e síncrona - os shells anexam o `makeColorApi()` do engine em vez de implementar qualquer coisa, então não pode haver divergência |
| `host.images` | 1.60 | Decodifica / redimensiona / recodifica bytes no dispositivo - o caminho de conversão (HEIC → JPEG, compressão para WebP, downscale). Entregue no shell web como uma fachada preguiçosa (lazy facade), para que o decodificador HEIC nunca entre no chunk de boot |
| `host.geom` | 1.64 | Geometria vetorial exata: booleanos de caminho, offsetting, stroke-to-fill, rebaixamento de splines, simplificação, hit testing. Também pura, síncrona e anexada a partir do engine (`makeGeomApi()`); falhas são *retornadas*, nunca lançadas (thrown) |
...
Esta é uma resposta estrutural ao risco de percepção de que toda ferramenta na galeria carrega o mesmo peso. Qualquer processo que uma equipe monte em torno do catálogo vem por cima disso.
...
- **Sem EJS / sem JS arbitrário em templates.** A superfície de XSS é zero. A lógica fica em `hooks.js`.
- **Sem CMS de ativos obrigatório.** Cada pessoa importa seus próprios arquivos criativos direto para Ativos dentro do aplicativo ([Ativos](/info/using.html#assets-your-library) e o Brand Studio) - sem servidor, sem console de administração. O trabalho é repassado como uma **sessão**: um link de compartilhamento carrega todo o estado, e a mesma sessão viaja em um backup ou por uma sessão de colaboração. Quem controla a implantação pode então travar uma sessão compartilhada como um **template** - abrir o link, registrar seus valores como uma entrada de template no diretório dessa ferramenta no pacote de marca e fazer o commit - depois disso ela aparece no seletor "New from template" da ferramenta e pode ser acessada por link direto como `?template=<id>`. O Git é a etapa de travamento de quem detém a implantação, nunca do criador. Para um catálogo *compartilhado e governado*, uma organização **pode** gerenciar o diretório de ativos da mesma forma e condicionar atualizações à revisão de PR - um modelo de governança disponível, não uma exigência do aplicativo.
- **Sem RBAC forçado.** O aplicativo aberto é de acesso público por padrão; o risco de marca é gerenciado por tags de maturidade + marcas d'água. Uma organização que quer controle mais rígido adiciona sua própria autenticação e o catálogo revisado por Git acima.
- **Sem banco de dados central.** Todo o estado do usuário é por dispositivo. A integração com SUSE ID está no roadmap, mas não é um bloqueador de lançamento.
- **Sem caminho de código compartilhado entre ferramentas/engine.** O engine é de código aberto, assim como as ferramentas agnósticas de marca em `community/`; um pacote de marca como o `brands/suse/` privado carrega suas próprias ferramentas e catálogo sob seus próprios termos. De qualquer forma a separação é aplicada (nenhuma importação cruzada de `engine/` para conteúdo de ferramenta), para que a divisão permaneça limpa.
