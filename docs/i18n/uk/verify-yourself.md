...
**6. Кінцева точка рендерингу відповідає лише публічними даними.** Єдина серверна функція, яка розміщує введені користувачем дані в URL - рендеринг за прямим посиланням - тут працює, і в [політиці конфіденційності](/info/privacy.html) пояснено, що це означає для даних, які ви вкладаєте в посилання:
...
Перемикач діє на рівні розгортання (`LOLLY_DISABLE_RENDER_GET=1`): на екземплярі, де його встановлено, той самий запит повертає `404` - ця відмінність є результатом роботи прапорця, а не непослідовністю.
