...
**6. Кінцева точка рендерингу відповідає лише публічними даними.** Єдина функція сервера, яка розміщує введені користувачем дані в URL - рендери hot-link - працює тут, і [політика конфіденційності](/info/privacy.html) пояснює, що це означає для даних, які ви вводите в посилання:
...
Цей перемикач діє на рівні розгортання (`LOLLY_DISABLE_RENDER_GET=1`): на інстансі, де його встановлено, той самий запит повертає `404` - ця відмінність означає, що прапорець працює, а не є несумісністю.
...
Запустіть їх самостійно у клоні (Node 22.18+; для цього файлу `pnpm install` не потрібен):
...
Повний набір тестів (`pnpm install && pnpm test`) запускає ще кілька тисяч, включно зі змагальними криптографічними тестами, описаними в розділі [Безпека та верифікація](/info/security.html).
