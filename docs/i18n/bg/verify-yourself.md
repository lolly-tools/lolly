...
**6. Крайната точка за рендиране отговаря само с публични данни.** Единствената функционалност на сървъра, която поставя въведени от потребителя данни в URL адрес - hot-link рендиране - е активна тук, а [политиката за поверителност](/info/privacy.html) обяснява какво означава това за данните, които поставяш в линк:
...
Превключвателят е за отделно разгръщане (`LOLLY_DISABLE_RENDER_GET=1`): на инстанция, която го задава, същата проверка връща `404` - тази разлика е знак, че флагът работи, а не несъответствие.
...
Изпълни ги сам в клонинга (Node 22.18+; за този файл не е нужен `pnpm install`):
...
Пълният набор (`pnpm install && pnpm test`) изпълнява още няколко хиляди, включително състезателните криптографски тестове, описани в [Сигурност и верификация](/info/security.html).
