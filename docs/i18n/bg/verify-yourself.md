...
**6. Крайната точка за визуализация отговаря само с публични данни.** Единствената сървърна функция, която поставя въведени от потребителя данни в URL адрес - визуализации чрез директен линк - е активна тук, а [политиката за поверителност](/info/privacy.html) обяснява какво означава това за данните, които поставяш в линк:
...
Превключвателят е за всяко разгръщане поотделно (`LOLLY_DISABLE_RENDER_GET=1`): на инстанс, който я зададе, същата проверка връща `404` - тази разлика е знак, че флагът работи, а не несъответствие.
