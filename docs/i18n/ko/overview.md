...
반복되는 크리에이티브 작업은 매번 숙련된 인력을 투입할 만큼 예측 불가능하지는 않지만, 가드레일 없이 넘기기에는 품질에 너무 민감해요. 그대로 두면 다음 세 가지 중 하나로 흘러가요: 느려지거나, 일관성이 없어지거나, 벤더의 템플릿에 갇혀요.

이 플랫폼은 그에 대한 직접적인 답입니다:

> **대규모 프로그래매틱 크리에이티브와 콘텐츠** - 에셋은 입력에서 생성되고, 규칙은 한 번만 정하면 필요한 사람은 누구나 쓸 수 있어요.
...
그 결과는 **풍요**예요: 모든 행사에 올바른 사이니지가 준비되고, 모든 CVE 경고가 하우스 스타일을 따르고, 모든 라벨이 깔끔하게 인쇄되고, 모든 이메일 서명이 최신 상태를 유지해요 - 이 모든 것이 이미 규칙을 담고 있는 도구에서 나와요. 이 플랫폼은 반복되는 운영형 크리에이티브를 처리해요. 맞춤형 크리에이티브 도구가 아니라는 점은 의도된 거예요 - 플래그십 작업은 여전히 디자이너의 몫이에요.
...
Lolly도 같은 선을 그어요. 확률적으로 탐색하세요: 모델, 디자이너, 러프한 아이디어, 아무도 계획하지 않은 곳으로 향하는 프롬프트. 그런 다음 결정론적으로 규모를 키우세요 - 다수의 결과물을 만들어내는 것은 *도구*이고, 도구는 읽을 수 있는 입력으로부터 매번 같은 방식으로 렌더링돼요. 탐색은 자유로운 채로 남아요. 다운스트림의 어떤 것도 그것이 두 번 같은 결과로 떨어지는 데 의존하지 않기 때문이에요. 결과물은 신뢰를 얻어요. 그것이 추측이 아니기 때문이에요. AI 실험을 예측 가능하고 재현 가능한 결과로 옮기는 일은 새로운 원칙이 아니에요. 그것은 애초에 인쇄물을 신뢰할 만하게 만들었던 것과 같은 분업이에요.
...
브랜드 규칙은 도구가 만드는 개별 파일이 아니라 도구 자체에 담겨 있어요([Lolly는 어떻게 비교되는가](/info/positioning.html) 참고). 도구를 한 번 제대로 만들어 두면, 카드 한 장이든 십여 개 언어로 된 행이 담긴 스프레드시트든, 모든 결과물이 동일한 타이포그래피, 색상, 간격을 물려받아요. 작업을 어떻게 확인할지, 누가 확인할지는 여전히 당신의 몫이에요; Lolly는 확인해야 할 대상을 줄이고 결과물을 더 빠르게 만들어 낼 뿐이에요.

동일한 결정론적 도구가 세 가지 방식으로 그 규모에 도달하며, 모두 동일한 결과물을 만들어내요:

- <!--i:people--> **앱 안에서, 사람이.** `/pro` 배치 그리드: 행을 붙여넣거나 가져오면 행마다 완성된 에셋이 하나씩 나오고, zip으로 다운로드해요. 디자인 기술도, 기다림도 필요 없어요.
- <!--i:code--> **명령줄에서, 개발자가.** CLI는 *동일한* 엔진과 *동일한* 렌더링 경로를 헤드리스로 실행하므로, 스크립트나 야간 파이프라인에서 모든 행에 걸쳐 도구를 순차적으로 적용할 수 있어요. 반복문 안의 `lolly <tool> --field=…` 호출 하나가 통합의 전부예요.
- <!--i:cpu--> **MCP를 통해, 시스템이나 AI 에이전트가.** 같은 도구가 같은 충실도로, 작업에 포함된 행 수가 얼마든 프로그래밍 방식으로 동작해요.
...
디자이너가 한 번 정한 하나의 브랜드 제약 세트, 동일한 결과물로 가는 세 가지 경로예요.
...
### 7. 성숙도 태그는 설계를 통해 도구가 무엇인지 알려줘요
...
이는 갤러리의 모든 도구가 동일한 무게를 지닌 것처럼 보일 수 있다는 인식상의 위험에 대한 구조적인 답이에요. 팀이 카탈로그 주위에 어떤 절차를 두든 그것은 이 위에 얹혀요.
