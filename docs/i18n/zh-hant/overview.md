...
可重複的創意工作，規律性高到不值得每次都動用專業人力，但對品質的要求又高到無法在沒有防護機制的情況下外包。放著不管，只會走向三種結局之一：緩慢、不一致，或被鎖在供應商的範本裡。

這個平台就是直接的解答：

> **規模化的程式化創意與內容產出** - 素材由輸入產生，規則只需設定一次，供任何需要的人使用。

Lolly 不是設計系統誕生的地方 - 而是它被產出的地方。把它想成一台設計自動販賣機:選好項目,拿到成品。每次都是如此。引擎會為眼前的硬體,盡力產出每種格式所能達到的最高品質,而且同一套引擎在其發行的每個平台上,都會產出相同的檔案。

結果就是**豐足**：每一場活動都有正確的標示、每一則 CVE 警示都符合公司風格、每一張標籤都印刷乾淨、每一個電子郵件簽名檔都是最新的 - 而且全部出自早已承載這些規則的工具。這個平台處理的是重複、可作業化的創意工作，刻意不做成客製化的創意工具 - 旗艦作品仍由設計師掌控。
...
Lolly 也劃出同樣的界線。以機率性的方式探索：一個模型、一位設計師、一個粗略的想法、一個走向沒人預料到之處的提示詞。接著以確定性的方式擴展 - 能變成許多輸出的，是一個*工具*，而工具每次都會依可讀的輸入，以相同方式算繪。探索可以保持自由，因為下游沒有任何東西仰賴它每次都得出相同結果。輸出之所以值得信任，是因為它不是猜測。讓 AI 的實驗轉化為可預測、可重現的成果，並不是什麼新的學問；這正是當初讓印刷品值得信賴的那套分工方式。
...
品牌規則存在於工具中，而不在它產出的每個檔案裡（詳見[Lolly 與其他方案的比較](/info/positioning.html)）。把工具一次做對，之後每一份產出都會繼承相同的字體、顏色與間距，無論是一張卡片，還是一份跨十幾種語言、逐行排列的試算表。你如何檢查自己的工作、由誰來檢查，依然由你決定;Lolly 只是讓值得檢查的東西變小，產出做得更快。

同一個確定性工具能以三種方式達到這樣的規模，而且都會產出完全相同的結果：

- <!--i:people--> **一個人，在應用程式中操作。** `/pro` 批次網格：貼上或匯入資料列，每一列產出一份完成的素材，下載為 zip 檔。不需要設計技能、不需要等待。
- <!--i:code--> **一位開發人員，透過命令列。** CLI 以無頭模式執行*相同*的引擎與*相同*的算繪路徑，因此可以在腳本或夜間管線中逐列依序執行該工具。在迴圈中呼叫 `lolly <tool> --field=…` 就是整個整合工作。
- <!--i:cpu--> **一個系統或 AI 代理，透過 MCP。** 同一個工具以程式化方式操作，保持相同的精確度，任務有多少列就處理多少列。
...
這是對「圖庫中每個工具份量相同」這種觀感風險的結構性解答。團隊圍繞目錄建立的任何流程都疊加於其上。
...
- **範本中沒有 EJS,也沒有任意 JS。** XSS 攻擊面為零。邏輯全部放在 `hooks.js` 中。
- **沒有強制性的素材 CMS。** 個人可以直接在應用程式內,將自己的創意檔案匯入素材中([素材](/info/using.html#assets-your-library)與 Brand Studio) - 不需要伺服器,也不需要管理主控台。作品是以**工作階段(session)**的形式交接:分享連結攜帶完整狀態,同一個工作階段也能透過備份或協作工作階段傳遞。部署的掌控者可以將某個已分享的工作階段鎖定為**範本(template)** - 開啟連結、將其數值記錄為該工具在品牌包目錄下的範本項目,再提交(commit) - 之後它便會出現在該工具的「New from template」選單中,並可透過 `?template=<id>` 深層連結存取。Git 是部署擁有者的鎖定步驟,絕非創作者的步驟。若要打造*共享且受管理*的目錄,組織**可以**以相同方式管理素材目錄,並透過 PR 審查來把關更新 - 這是一種可選的治理模式,而非應用程式的強制要求。
- **沒有強制性的 RBAC。** 開放版應用程式預設為公開存取;品牌風險透過成熟度標籤與浮水印來管理。若組織需要更嚴格的控管,可在其上疊加自己的驗證機制,以及上述經 Git 審查的目錄。
- **沒有中央資料庫。** 所有使用者狀態皆以裝置為單位儲存。SUSE ID 整合已列入路線圖,但並非上線的先決條件。
- **沒有共用的工具/引擎程式碼路徑。** 引擎是開放原始碼,`community/` 中與品牌無關的工具也是;像私有的 `brands/suse/` 這樣的品牌包,則依其自身條款攜帶自己的工具與目錄。無論哪種情況,這種區隔都是強制執行的(`engine/` 絕不會反向匯入工具內容),以確保此區分維持乾淨。
