...
**6. تجيب نقطة نهاية العرض (render endpoint) ببيانات عامة فقط.** الميزة الخادمية الوحيدة التي تضع مدخلات كتبها المستخدم داخل عنوان URL - عمليات العرض عبر الرابط المباشر (hot-link renders) - مفعّلة هنا، وتوضح [سياسة الخصوصية](/info/privacy.html) ما يعنيه ذلك بالنسبة للمدخلات التي تضعها في رابط:
...
هذا المفتاح خاص بكل عملية نشر (`LOLLY_DISABLE_RENDER_GET=1`): في نسخة تُفعّله، يُعيد نفس الفحص (probe) الرمز `404` - وهذا الاختلاف هو عمل العلامة (flag) كما ينبغي، لا تناقضاً.
...
شغّلها بنفسك في النسخة المستنسَخة (Node 22.18+؛ لا حاجة إلى `pnpm install` لهذا الملف):
...
تُشغِّل المجموعة الكاملة (`pnpm install && pnpm test`) عدة آلاف إضافية من الاختبارات، بما في ذلك اختبارات التشفير العدائية (adversarial) الموصوفة في [الأمان والتحقق](/info/security.html).
