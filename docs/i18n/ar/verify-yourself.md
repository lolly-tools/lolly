...
**6. نقطة نهاية العرض تجيب ببيانات عامة فقط.** ميزة الخادم الوحيدة التي تضع مدخلات كتبها المستخدم في رابط - عمليات العرض بالربط المباشر - مُفعّلة هنا، وتشرح [سياسة الخصوصية](/info/privacy.html) ما يعنيه ذلك للمدخلات التي تضعها في رابط:
...
المفتاح لكل نشر على حدة (`LOLLY_DISABLE_RENDER_GET=1`): على نسخة تضبط هذا المتغير، يُعيد نفس الفحص `404` - هذا الفرق هو عمل العلَم كما يجب، لا تضاربًا.
