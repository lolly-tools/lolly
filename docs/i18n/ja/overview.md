...
反復的なクリエイティブ作業は、毎回熟練者の手を借りるほどではないほど予測可能でありながら、ガードレールなしに任せるにはあまりに品質に敏感です。放っておくと、遅くなる、一貫性がなくなる、ベンダーのテンプレートに縛られる、という3つのいずれかに行き着きます。

このプラットフォームはその直接的な答えです。

> **プログラムによる大規模なクリエイティブとコンテンツ** - アセットは入力から生成され、ルールは一度設定するだけで、必要とする人なら誰でも使えます。
...
その結果得られるのは**豊富さ**です。あらゆるイベントに正しいサイネージが用意され、あらゆるCVEアラートが自社のスタイルに準拠し、あらゆるラベルがきれいに印刷され、あらゆるメール署名が最新の状態を保つ - そのすべてが、すでにルールを備えたツールから生まれます。このプラットフォームが担うのは、反復的でオペレーション化されたクリエイティブです。あえてオーダーメイドのクリエイティブツールにはしていません - フラッグシップの制作は引き続きデザイナーが担います。
...
Lollyも同じ線引きをしています。探索は確率的に行います - モデル、デザイナー、ラフなアイデア、誰も計画していなかった場所へ行き着くプロンプト。そしてスケールは決定論的に行います - 多数の出力を生み出すのは*ツール*であり、ツールは読み取り可能な入力から毎回同じようにレンダリングします。探索が自由でいられるのは、それが2回とも同じ結果に着地することに何も依存していないからです。出力が信頼を勝ち得るのは、それが当て推量ではないからです。AIによる実験を予測可能で再現性のある結果へと落とし込むことは、新しい規律ではありません。そもそも印刷物を信頼に足るものにしたのと同じ、分業の姿なのです。
...
ブランドルールはツールの中に宿り、それが作る個々のファイルの中にはありません([Lollyの比較](/info/positioning.html)を参照)。ツールを一度正しく作れば、1枚のカードであれ、十数言語にわたる行を持つスプレッドシートであれ、あらゆる出力が同じタイプ、色、スペーシングを引き継ぎます。作業をどう確認するか、誰が確認するかはあなた次第です。Lollyは確認すべきものを小さくし、出力をより速く作れるようにします。

同じ決定論的なツールが、3つの方法でその規模に到達し、いずれも同一の出力を生み出します:

- <!--i:people--> **人が、アプリの中で。** `/pro`バッチグリッド:行を貼り付けるかインポートし、1行につき1件の完成したアセットを得て、zipをダウンロードします。デザインスキルも待ち時間も不要です。
- <!--i:code--> **開発者が、コマンドラインから。** CLIは*同じ*エンジンと*同じ*レンダーパスをヘッドレスで実行するため、スクリプトや夜間パイプラインの中であらゆる行にわたってツールを連続実行できます。ループの中で`lolly <tool> --field=…`を呼び出すだけで統合は完結します。
- <!--i:cpu--> **システムやAIエージェントが、MCP経由で。** 同じツールを同じ忠実度でプログラムから操作でき、ジョブに含まれる行数がいくつであっても対応します。
...
デザイナーによって一度定められた1組のブランド制約。同一の出力へと至る3つの経路です。
...
これは、ギャラリー内のすべてのツールが同じ重みを持つように見えてしまう、という認識上のリスクへの構造的な答えです。チームがカタログの周りにどのようなプロセスを敷くとしても、それはこの上に重なります。
...
- **テンプレートにEJSなし・任意のJSなし。** XSSの攻撃面はゼロです。ロジックは`hooks.js`に置かれます。
- **必須のアセットCMSなし。** 個人は自分のクリエイティブファイルを、アプリ内([アセット](/info/using.html#assets-your-library)ビューとブランドスタジオ)から直接アセットへ取り込みます - サーバーも管理コンソールも不要です。作業は**セッション**として引き継がれます: 共有リンクが状態全体を保持し、同じセッションはバックアップや共同作業セッションを通じても運ばれます。デプロイメントを管理する者は、共有セッションを**テンプレート**として固定できます - リンクを開き、その値をブランドパック内のそのツールのディレクトリにテンプレートエントリとして記録し、コミットします - すると、それがそのツールの「New from template」選択肢に現れ、`?template=<id>`としてディープリンク可能になります。Gitはデプロイメント所有者の固定手段であり、制作者のものではありません。*共有され統治された*カタログを求めるなら、組織はアセットディレクトリを同じ方法で管理し、PRレビューによって更新をゲートすることも**できます** - これは利用可能な統治モデルであって、アプリの要件ではありません。
- **強制RBACなし。** オープンなアプリは既定で公開アクセスであり、ブランドリスクは成熟度タグとウォーターマークで管理します。より厳格な制御を求める組織は、独自の認証と上記のGitレビュー済みカタログを上に重ねます。
- **中央データベースなし。** すべてのユーザー状態はデバイスごとです。SUSE ID連携はロードマップにありますが、公開の前提条件ではありません。
- **共有されたツール/エンジンのコードパスなし。** エンジンはオープンソースであり、`community/`内のブランド非依存のツールも同様です。私設の`brands/suse/`のようなブランドパックは、独自の条件のもとで自身のツールとカタログを持ちます。いずれの場合も分離は強制され(`engine/`からツールコンテンツへのクロスインポートは禁止)、この分割は常にクリーンな状態を保ちます。
