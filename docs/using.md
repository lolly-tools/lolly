...
- **Before you export.** The inspector's Document section checks the saved layer structure, then reads the settled canvas for clipped text and flat-colour contrast. It also asks the same font registry used by SVG/PDF outlining whether each text run has embeddable font bytes; image and gradient backgrounds are named as visual checks instead of being given a made-up contrast score.
...
Choose **3D scene** from the add menu on the tool rail and drag out a frame: 3D Studio opens straight away on the new box, and what you set there comes back to the canvas. In every other way a scene box is an ordinary box. Move it, resize it, rotate it, give it a shadow, put it on a slide or on the timeline, and it behaves like the rest of them.

**A scene box keeps the recipe, not a picture.** An image box holds a rendered file; a scene box holds one setting, the scene itself, written as 3D Studio's own link query, with every value still at the studio's default left out. That is why a scene is about a hundred bytes rather than the few kilobytes a whole recipe costs, why the same string works in a share link and in the editor door, and why a new studio control needs no change in Design. It is also why the box re-renders at whatever size and moment the document asks for instead of being enlarged from a picture taken earlier. Pictures a scene uses stay assets and travel by id, so an upload inside a scene goes into a `.lolly` file with the rest of the document.

**Edit it in the studio.** Select the box and the Inspector shows a **3D scene** section: a line naming what the scene is made of, a second naming its lighting studio once you have chosen one, and one button, **Edit in 3D Studio**. The button opens the studio on that box's scene with every control the tool has. Apply, and the edited scene is written back as a single step, so one undo returns the box to the scene you started from; close the studio without applying and nothing changes. Everything else about the box - its place on the artboard, how big it is, its shadow, when it arrives on a slide - stays in the sections it always used. A scene box takes no image of its own and no caption: its picture comes from the studio, and its words are set there too.

**One live scene, a poster on every other box.** Every 3D box in a document shows a poster: a still picture of the scene, drawn off screen through the shared renderer pool at the size the box occupies. A document with twenty scenes costs one drawing context, not twenty. Select a scene box and it becomes the document's one live scene; deselect it and the frame that was on screen becomes its poster, so nothing jumps. Only one scene is live at a time, and selecting two scene boxes at once leaves both as posters. In this release the live scene is for looking, not for orbiting: change a scene through **Edit in 3D Studio**. A device that cannot open a floating-point graphics context keeps the poster and says why inside the box rather than showing a blank rectangle, and the rest of the document is unaffected. Opening a Design document with no 3D box loads no 3D code at all.

**On the timeline**, a scene box follows the playhead like a video clip: its start, clip-in and speed move the scene through its own animation, and the scene's length is the one you set in 3D Studio, so trimming a box shorter shows less of the scene rather than speeding it up. Only the selected scene box is live; every other one is a still picture, and a still picture does not scrub.

**In an export**, each scene is drawn fresh at the size the file needs, through the same renderer the studio uses. A video renders one frame per scene per moment; a PNG, SVG or PDF embeds one picture per box at the box's own pixel size. Nothing is photographed off the screen, so an export does not depend on which box you had selected. A scene that cannot be drawn fails the export and tells you why, in the studio's own words.
...
![The editing stage: the artboard front and centre, the tool rail on the left and the zoom HUD in the corner](/t/url-shot?url=%2F%23%2Ftool%2Fdesign%3Fz%3D11dZBb5swFADgX8MOiRYZB0J76GFpNO2wnbr7ZMwDrBg7s01C8usngmNwSqJszaT2aD8_G54_PUgJXRdK1iJ7CvAcpSHG6FMqG9BPQbwMkmWAMcsCjIP5lwDjUsp1O8DPAcZrJvpIKhsXaLpZ1I323mjXjcJHbCdKO4Ee7ISSxsvQJdmAO0cBNe6gtHDzQbKkkks101ARYRidaaBSZETtg2TlMgw0xuX8LBXANCN7PTVyWki3Kr-6b61yQmG4ay6FeWEHOL1K1E0TzgrhdqIgDCiXs_WjFcsyDi66A1aU_ZMuEPIOcwFNhHYRzgR8GySGs9CW0BDlFzWrVTe2qdCwftOcZP2TtJEfuovFyKZzIvor0fD7WKhVGzsXo2ALhH8UMxvFqmtiXmQFpmSimArYBfGzYHpKZcVEcS87-OHedpK72cHndmYWunu6rtxM-3wuwGqTXskacov-nhs15KNYG7HgWZtMvpNa0LJtUJNJi-2rYhnZ3ybtuNUVZuj9MotOrAbQFmPQbuB0uxwud4NX9-ycrmWIJw59Pg9h5AF6RGd-5vgWPhvG-2b5xs8bl5zvZ0ZK3tf_bd0pxu_lw2YDG2Jv6VRdj9Hi__WrKB7pV3OELuBKIRunReqM9f8d1tbnKPFsJVHs2Zqf9SaMLrSmASCjiNAbokD0lD0t_95Wxu-MVaQ4uT6WQ8ta0V46Z6lq9br1fVEOh7ypju-FFyjBC7fmVy0UaMm3YBcbVYMt-dhfTlUbe2BOuD6ujFd_AA&width=1440&height=900&dpi=192&waitMs=7000&waitSelector=.tl-clip&css=.fc-toolbar%7Bopacity%3A1!important%7D&format=svg&walker=1&tolerance=0.03&dark=1&filename=seq-studio-stage)
...
- **Sub-slide stacks.** Right-click an artboard and choose **Stack under the previous slide** and it becomes a step of that slide rather than a slide of its own: the overview shows one card, the deck walks through the stack in order, and the inspector's **Stack** row says which slide it belongs to.
- **Morph.** When two consecutive slides both carry a box with the same **Morph match** name (right-click a box, or the inspector's **Morph match** row - `hero`, say), the transition moves that box from where it was to where it is, resizing and recolouring on the way, instead of cutting. A deck-wide **Morph** transition does the same for every matched pair.
- **Narration.** Each artboard's **Speaker notes** can be read aloud. In the inspector's **Document** section pick a **Voice**, optionally a second voice to **Blend with**, the reading **Speed**, and a **Lead-in** and **Tail** in milliseconds around each slide; turn on **Show captions when presenting** and the words appear as they are spoken. The voice runs on your device. The same notes become the film in a video export, real slide audio in a PowerPoint export, and the narrated film inside a [SCORM package](/info/create/exporting.html#scorm-course-packages).
...
**Download project (.lolly)**, in a folder's menu in **Projects**, writes that folder as a shared project, so someone else can open it and carry on with every session in it. Each session travels as its own part (`sessions/<key>.json`, with its tile under `thumbs/`), the folder tree is listed in `manifest.json`, and uploads and catalogue art travel under the same rules as a single shared design. Batch sessions are not tool sessions and stay behind; the notice says how many. **Download originals**, beside it, is unchanged: a plain zip of each item as its own file.
