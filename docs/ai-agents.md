...
| File | What it is for |
|---|---|
| [`/agents.md`](/agents.md) | The short version of this page for a machine reader: entry points, the URL grammar, how to get bytes and the rules that save a round trip. |
| [`/llms.txt`](/llms.txt) | The docs index in the [llms.txt](https://llmstxt.org) convention - one line per page, linking its markdown twin. |
| [`/llms-full.txt`](/llms-full.txt) | Every docs page in one file, in the index's order, for a reader that wants the whole corpus in one fetch. |
| [`/.well-known/lolly.json`](/.well-known/lolly.json) | The discovery record: where the catalog, the manifests, the render route, the MCP endpoints and the files above live on this deployment. |
| [`/openapi.json`](/openapi.json) | The HTTP surface in OpenAPI 3.1 - the hot-link render route with its refusals, the catalog and manifest reads and the MCP endpoint's transport. |
| [`/info/capabilities.json`](/info/capabilities.json) | Format claims: what Lolly reads and writes per format and what each format's own metadata carries. |
| [`/catalog/tools/index.json`](/catalog/tools/index.json) | The generated tool index; `/tools/<id>/tool.json` is one tool's manifest with every input, default and format. |
| [`/info/skills/lolly/SKILL.md`](/info/skills/lolly/SKILL.md) | The agent skill: one page an agent loads to drive Lolly across every surface, with references for URL mode, the surfaces, the whole catalogue, the three document tools (chart, design, deck) and renovating an old deck into the design system (Rebrand). |
...
## Fast path for a model

When a model points at one Lolly instance, use the smallest path that answers the task:

1. Read the instance discovery record and request only the relevant tool or format.
2. Describe the chosen tool or template and build a stack plan from its declared inputs.
3. Validate and render in one workflow when the instance supports that combined call.
4. Return the artifact, editable recipe, checks and one next action.

For a known recipe, do not enumerate the whole catalog or repeat validation as a separate call. For a revision, send stable layer patches or changed inputs instead of the whole document. A result is complete when its declared checks pass. Ask for human review only when the task states a requirement this instance cannot measure, such as physical print appearance or subjective approval. Otherwise report the result and continue.

Keep these outcomes distinct: `complete`, `partial`, `approximate`, `blocked`, `failed` and `unknown`. Say what changed, what was preserved, what was checked and what remains unknown. Never describe an unchecked property as passed.
