...
const TOOL_COUNT = catalogIndex.tools.length;
...
function getBulletIcon(raw: string): string {
...
function getIcon(h2: string): string {
...
function tabLabel(h2: string): string {
  const t = h2.toLowerCase();
...
function mdToHtml(md: string): string {
  ...
  // Every access below is preceded by an explicit `i < lines.length` (or
  // `i + 1 < lines.length`) bounds check, so the index is always in range —
  // the `?? ''` fallback is provably unreachable. This keeps the original
  // indexing style intact under noUncheckedIndexedAccess without non-null
  // assertions.
  ...
  while (i < lines.length) {
    ...
    if (hm) {
      // Both capture groups are mandatory in the pattern, so they're always
      // present on a match; the fallback is provably unreachable.
    ...
    if (line.startsWith('> ')) {
      const ql = [];
      while (i < lines.length && at(i).startsWith('> ')) { ql.push(at(i).slice(2)); i++; }
    ...
    if (line.includes('|') && i + 1 < lines.length && /^\|?[-|: ]+\|/.test(at(i + 1))) {
      const headers = parseCells(line);
      i += 2;
      const rows = [];
      while (i < lines.length && at(i).trim().startsWith('|')) { rows.push(parseCells(at(i))); i++; }
    ...
    if (/^\s*[-*] /.test(line)) {
      ...
      while (i < lines.length && /^\s*[-*] /.test(at(i))) {
        out.push(`<li>${inline(at(i).replace(/^\s*[-*] /, ''))}</li>`); i++;
      }
    ...
    if (/^\d+\. /.test(line)) {
      ...
      while (i < lines.length && /^\d+\. /.test(at(i))) {
        out.push(`<li>${inline(at(i).replace(/^\d+\. /, ''))}</li>`); i++;
      }
    ...
    while (
      ...
      !at(i).startsWith('#') && !at(i).startsWith('```') &&
      !at(i).startsWith('> ') && !/^\s*[-*] /.test(at(i)) &&
      !/^\d+\. /.test(at(i)) && !/^-{3,}$/.test(at(i).trim()) &&
      !(at(i).includes('|') && i + 1 < lines.length && /^\|?[-|: ]+\|/.test(at(i + 1)))
    ) { para.push(at(i)); i++; }
...
function loadFaqs(): Faq[] {
  ...
  for (const line of md.split('\n')) {
    ...
    if (m) {
      ...
      // The capture group is mandatory in the pattern, so it's always present
      // on a match; the fallback is provably unreachable.
      cur = { q: (m[1] ?? '').trim(), a: [] };
    } else if (cur) {
      cur.a.push(line);
    }
...
function buildLandingContent(md: string): string {
  const rawSections = md.split(/\n---\n/);
  // `split` always returns at least one element, so index 0 and the last
  // index are always in range; the `?? ''` fallback is provably unreachable.
  ...
  function parseAudienceCard(section: string): AudienceCard {
    ...
    if (h3Idx >= 0) {
      for (let j = h3Idx + 1; j < lines.length; j++) {
        // j < lines.length is the loop bound, so the index is always in
        // range; the `?? ''` fallback is provably unreachable.
...
${cardData.map(({ h2 }, i) => `  <button class="audience-tab" role="tab" aria-selected="${i === 0 ? 'true' : 'false'}" data-panel="${i}" data-slug="${toSlug(h2)}">
  ...
  const cardsHtml = cardData.map(({ h2, h3, intro, bullets, codeBlocks }, i) => {
    const firstBlock = codeBlocks[0];
    return `<div class="audience-card${i === 0 ? ' tab-active' : ''}" id="${toSlug(h2)}" data-panel="${i}">
  <div class="card-main">
    ...
    ${firstBlock ? `<pre><code class="language-${esc(firstBlock.lang)}">${esc(firstBlock.code)}</code></pre>` : ''}
  </div>
  ...
  const platformFeatures: PlatformFeature[] = tailLines
    .slice(platformIdx + 1, whatsIdx >= 0 ? whatsIdx : undefined)
    .filter(l => l.startsWith('**'))
    .map((l): PlatformFeature | null => {
      const m = l.match(/^\*\*([^*]+)\*\*\.?\s*(.*)/);
      return m ? { title: m[1] ?? '', desc: m[2] ?? '' } : null;
    }).filter((f): f is PlatformFeature => f !== null);
  ...
  function renderPlatformFeature(f: PlatformFeature, idx = 0): string {
...
/* Dark mode */
.dark{--text:#cce8da;--muted:#7aaa90;--border:#1b3d2c;--pale:#0d2419;--blue:#5d9eff}
...
function buildNav(activeHref: string, isLanding: boolean | undefined): string {
  const link = (n: NavLink): string =>
...
function wrapPage(page: Page, content: string, ogSlugs: Set<string> | undefined): string {
...
if (process.argv.includes('--watch')) {
  ...
  const scheduleRebuild = (label: string): void => {
    ...
    timer = setTimeout(() => {
      ...
      try { build(); } catch (err) { console.error('✗  Rebuild failed:', err instanceof Error ? err.message : String(err)); }
    }, 120);
  };
  watch(__dirname, { recursive: true }, (_event: string, file: string | null) => scheduleRebuild(file || 'docs'));
