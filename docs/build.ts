...
// mastheads read [data-theme] per plans/212-docs-masthead-gemini-prompt.md; once the bank is
...
// site.json), generated by `pnpm run translate --corpus site` (see
...
async function build() {
  ...
  // The agent skill (plans/220): skills/lolly/ published verbatim at
  // /info/skills/lolly/ so an agent can fetch SKILL.md and its references the same
  // way it reads /agents.md and /llms.txt. The tables inside are generated by
  // scripts/gen-agent-skill.ts (tests/agent-skill.test.ts fails on drift); this
  // build just serves the source. Mirror, don't accumulate, so a removed
  // reference does not linger in the gitignored output.
  const skillSrc = resolve(repoRoot, 'skills', 'lolly');
  if (existsSync(skillSrc)) {
    rmSync(resolve(outDir, 'skills', 'lolly'), { recursive: true, force: true });
    cpSync(skillSrc, resolve(outDir, 'skills', 'lolly'), { recursive: true });
    console.log('✓  /info/skills/lolly (agent skill: SKILL.md + references)');
  }

  // Docs screenshots - committed baselines captured by `pnpm run docs:shots` from
...
// /info pages stay current during `pnpm run dev:web`. Sources are everything under
